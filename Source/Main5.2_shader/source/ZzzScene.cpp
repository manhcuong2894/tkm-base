...
bool MoveMainCamera()
{
	...
	if ((World == WD_73NEW_LOGIN_SCENE || World == WD_77NEW_LOGIN_SCENE) && g_CameraMove->IsTourMode())
	{
	...
	if (CameraTopViewEnable)
	{
		...
		CameraPosition[0] = Hero->GetPositionX();
		CameraPosition[1] = Hero->GetPositionY();
		CameraPosition[2] = CameraViewFar;
	}
	else if (World == WD_6STADIUM && (FindText(Hero->ID, "webzen") || FindText(Hero->ID, "webzen2")))
	{
		float Matrix[3][4];
		vec3_t pos, Position, TransformPosition;
		...
		pos[2] = 200.0;
		CameraViewFar = 5000.0;

		Vector(0.f, -CameraDistance, 0.f, Position);//-750
		AngleMatrix(CameraAngle, Matrix);
		VectorIRotate(Position, Matrix, TransformPosition);

		CameraPosition[0] = pos[0] + TransformPosition[0];
		CameraPosition[1] = pos[1] + TransformPosition[1];
		CameraPosition[2] = pos[2] + 800.0;
		CameraAngle[0] = -70.0;
	}
	else
	{
		...
		else
		{
			if (g_shCameraLevel == 0)
			{
				if (SceneFlag == LOG_IN_SCENE)
				{
					if (g_CameraMove->IsTourMode() == false)
					{
						if (gmProtect->SceneLogin != 0)
						{
		...
		else if (g_CameraMove->IsTourMode())
		{
			g_CameraMove->UpdateTourWayPoint();
			g_CameraMove->GetCurrentCameraPos(Position);
			CameraViewFar = 390.f * g_CameraMove->GetCurrentCameraDistanceLevel();
		}
		...
		else if (g_CameraMove->IsTourMode() == false)
		{
		...
		if (SceneFlag == MAIN_SCENE)
			CameraPosition[2] += CameraDistance - vTargetDistance;
		else
		...
		if (g_CameraMove->IsTourMode())
		{
			if (gmProtect->SceneLogin != 3)
			{
				g_CameraMove->SetAngleFrustum(-112.5f);
				CameraAngle[0] = g_CameraMove->GetAngleFrustum();
			}
			else
			{
				CameraAngle[0] = -78.5;
			...
			CameraAngle[1] = 0.0f;
			CameraAngle[2] = g_CameraMove->GetCameraAngle();
		}
	...
	{
		CameraDistanceTarget = 1100.f * g_CameraMove->GetCurrentCameraDistanceLevel() * 0.1f;
