...
HWND CWINHANDLE::Create(HINSTANCE hCurrentInst, uint32_t RenderSizeX, uint32_t RenderSizeY)
{
	WNDCLASS wc = {};
...
void CWINHANDLE::InitSize(uint32_t RenderSizeX, uint32_t RenderSizeY)
{
...
void CWINHANDLE::SetFontSize(uint32_t FontSize)
{
...
void CWINHANDLE::Resize(uint32_t RenderSizeX, uint32_t RenderSizeY)
{
	if (!this->GethWnd())
		return;
	...
	if (this->CheckWndMode())
	{
		...
		// Centrar la ventana dentro del área de trabajo
		screenX = rcWorkArea.left + (rcWorkArea.right - rcWorkArea.left - rc.right) / 2;
		screenY = rcWorkArea.top + (rcWorkArea.bottom - rcWorkArea.top - rc.bottom) / 2;
	}

	SetWindowPos(this->GethWnd(), NULL, screenX, screenY, rc.right, rc.bottom, SWP_NOZORDER | SWP_NOACTIVATE);
}
...
void CWINHANDLE::SetWndActive(bool bActive)
{
	WndActive = bActive;
}

bool CWINHANDLE::CheckWndActive()
{
	return WndActive;
}
...
void CWINHANDLE::SetWndMode(bool bActive)
{
	WndMode = bActive;
}

bool CWINHANDLE::CheckWndMode()
{
	return WndMode;
}

void CWINHANDLE::SetDisplayIndex(uint8_t index, bool Fontload)
{
	wndIndex = index;
...
uint8_t CWINHANDLE::GetDisplayIndex()
{
	return wndIndex;
}

uint8_t CWINHANDLE::GetDisplayIndex(const std::string text_name)
{
	return res_setting.FindInfoByIndex(text_name);
}

float CWINHANDLE::GetScreenX()
{
	return iWinWidth;
}

float CWINHANDLE::GetScreenY()
{
	return iWinHight;
}
...
void CWINHANDLE::Change_State(bool bActive)
{
...
bool CWINHANDLE::CheckWndIconic()
{
	return WndIconic && !IsWindowVisible(hWnd);
}

bool CWINHANDLE::CheckPerformance()
{
