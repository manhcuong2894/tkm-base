...
void OGL330MODEL::SendUniform(GLuint shaderID, const mvec4& bodyLight, const mvec4& lightPosition, const mvec4& meshUV, const mvec4& setting1, const mvec4& setting2, const bool enableLight, const bool shadow, vec3_t vBodyOrigin)
{
	...
	GLint loc;
	if ((loc = glGetUniformLocation(shaderID, "uProj")) != -1)
		glUniformMatrix4fv(loc, 1, GL_FALSE, ProjMatrix);

	if ((loc = glGetUniformLocation(shaderID, "uView")) != -1)
		glUniformMatrix4fv(loc, 1, GL_FALSE, ViewMatrix);

	if ((loc = glGetUniformLocation(shaderID, "u_bodyLight")) != -1)
		glUniform4f(loc, bodyLight.x, bodyLight.y, bodyLight.z, bodyLight.w);

	if ((loc = glGetUniformLocation(shaderID, "u_lightPosition")) != -1)
		glUniform4f(loc, lightPosition.x, lightPosition.y, lightPosition.z, lightPosition.w);

	if ((loc = glGetUniformLocation(shaderID, "u_meshUV")) != -1)
		glUniform4f(loc, meshUV.x, meshUV.y, meshUV.z, meshUV.w);

	if ((loc = glGetUniformLocation(shaderID, "u_setting1")) != -1)
		glUniform4f(loc, setting1.x, setting1.y, setting1.z, setting1.w);

	if ((loc = glGetUniformLocation(shaderID, "u_setting2")) != -1)
		glUniform4f(loc, setting2.x, setting2.y, setting2.z, setting2.w);

	if ((loc = glGetUniformLocation(shaderID, "u_enableLight")) != -1)
		glUniform1i(loc, enableLight ? 1 : 0);

	if ((loc = glGetUniformLocation(shaderID, "uTexture")) != -1)
		glUniform1i(loc, 0);

	if ((loc = glGetUniformLocation(shaderID, "u_shadowMode")) != -1)
		glUniform1i(loc, shadow ? 1 : 0);

	//if ((loc = glGetUniformLocation(shaderID, "u_shadowProj")) != -1)
	//	glUniform2f(loc, 2000.0, 4000.0);

	if ((loc = glGetUniformLocation(shaderID, "u_bodyOrigin")) != -1)
		glUniform3f(loc, vBodyOrigin[0], vBodyOrigin[1], vBodyOrigin[2]);
}
