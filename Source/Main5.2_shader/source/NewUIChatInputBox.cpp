...
bool SEASON3B::CNewUIChatInputBox::Create(CNewUIManager* pNewUIMng, CNewUIChatLogWindow* pNewUIChatLogWnd, int x, int y)
{
	...
	m_pChatInputBox->SetPlaceHolder("Escribe aqui...");
	...
	m_bOnlySystemMessage = !m_pNewUIChatLogWnd->GetMsgVisible(TYPE_SYSTEM_MESSAGE);
...
void SEASON3B::CNewUIChatInputBox::SetWndPos(int x, int y)
{
	...
	this->SetTextPosition(m_WndPos.x + 100, m_WndPos.y + 30);

	this->SetBuddyPosition(m_WndPos.x + 25, m_WndPos.y + 30);
#elif MAIN_UPDATE > 303
	if (gmProtect->checkold_school())
	{
...
bool SEASON3B::CNewUIChatInputBox::UpdateMouseEvent()
{
	...
	float RenderFrameX = m_WndPos.x + 29.f;
	float RenderFrameY = m_WndPos.y + 8.f;

	m_iTooltipType = INPUT_TOOLTIP_NOTHING;

	for (int i = 0; i < 10; ++i)
	{
		if (CheckMouseIn(RenderFrameX + i * 22.5, RenderFrameY, 18.f, 16.f))
		{
			m_iTooltipType = i;
			break;
		}
	...
	for (int i = 0; i < 4; ++i)
	{
		if (CheckMouseIn(RenderFrameX + i * 22.5, RenderFrameY, 18.f, 16.f) && SEASON3B::IsRelease(VK_LBUTTON))
		{
			SetInputMsgType(INPUT_CHAT_MESSAGE + i);
			PlayBuffer(SOUND_CLICK01);
			return false;
		}
	}

	if (CheckMouseIn(RenderFrameX + 4 * 22.5, RenderFrameY, 18.f, 16.f) == true && SEASON3B::IsRelease(VK_LBUTTON))
	{
	...
	if (CheckMouseIn(RenderFrameX + 5 * 22.5, RenderFrameY, 18.f, 16.f) == true && SEASON3B::IsRelease(VK_LBUTTON))
	{
		m_bShowChatLog = !m_bShowChatLog;
		if (m_bShowChatLog)
		{
			m_pNewUIChatLogWnd->SetAllMsgEnable();
			m_pNewUIChatLogWnd->SetMsgVisible(TYPE_SYSTEM_MESSAGE, false);
	...
	if (CheckMouseIn(RenderFrameX + 6 * 22.5, RenderFrameY, 18.f, 16.f) == true && SEASON3B::IsRelease(VK_LBUTTON))
	{
	...
	if (CheckMouseIn(RenderFrameX + 7 * 22.5, RenderFrameY, 18.f, 16.f) == true && SEASON3B::IsRelease(VK_LBUTTON))
	{
	...
	if (m_pNewUIChatLogWnd->IsShowFrame())
	{
		if (CheckMouseIn(RenderFrameX + 8 * 22.5, RenderFrameY, 18.f, 16.f) == true && SEASON3B::IsRelease(VK_LBUTTON))
		{
		...
		if (CheckMouseIn(RenderFrameX + 9 * 22.5, RenderFrameY, 18.f, 16.f) == true && SEASON3B::IsRelease(VK_LBUTTON))
		{
...
#ifdef WISSPER_CHAT_BLOCKED
		if (UpdateMouseActionChatBlocked())
		{
			return false;
		}
#endif // WISSPER_CHAT_BLOCKED
...
bool SEASON3B::CNewUIChatInputBox::UpdateKeyEvent()
{
	...
	{
#ifdef WISSPER_CHAT_BLOCKED
		if (m_bBlockedChat && m_iChatBlocked.HaveFocus() && SEASON3B::IsPress(VK_RETURN))
		{
			m_iChatBlocked.EnterInstance();
			return false;
		}
#endif // WISSPER_CHAT_BLOCKED
...
bool SEASON3B::CNewUIChatInputBox::RenderFrame()
{
	...
	SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY, (float)WND_WIDTH, (float)WND_HEIGHT, 0.0, 0.0, 798.f, 152.f);
#elif MAIN_UPDATE > 303
	if (gmProtect->checkold_school())
	{
...
{
#if MAIN_UPDATE > 603
	float RenderFrameX = m_WndPos.x;
	float RenderFrameY = m_WndPos.y;

	RenderFrameX += 29.f;
	for (int i = 0; i < 4; i++)
	{
		SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY + 8.f, 18.f, 16.f, (i * 64.f), (m_iInputMsgType == i)? 153.f : 204.f, 58.f, 51.f);

		RenderFrameX += 22.5;
	}
	SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY + 8.f, 18.f, 16.f, (4 * 64.f), (m_bBlockWhisper) ? 153.f : 204.f, 58.f, 51.f);
	RenderFrameX += 22.5;

	SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY + 8.f, 18.f, 16.f, (5 * 64.f), (m_bShowChatLog) ? 153.f : 204.f, 58.f, 51.f);
	RenderFrameX += 22.5;

	SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY + 8.f, 18.f, 16.f, (6 * 64.f), (m_bOnlySystemMessage) ? 153.f : 204.f, 58.f, 51.f);
	RenderFrameX += 22.5;

	if (m_pNewUIChatLogWnd->IsShowFrame())
	{
		SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY + 8.f, 18.f, 16.f, (7 * 64.f), 153.f, 58.f, 51.f);
		RenderFrameX += 22.5;

		SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY + 8.f, 18.f, 16.f, (8 * 64.f), 153.f, 58.f, 51.f);
		RenderFrameX += 22.5;

		SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY + 8.f, 18.f, 16.f, (9 * 64.f), 153.f, 58.f, 51.f);
		RenderFrameX += 22.5;
	}
	else
	{
		SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY + 8.f, 18.f, 16.f, (7 * 64.f), 204.f, 58.f, 51.f);
		RenderFrameX += 22.5;

		SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY + 8.f, 18.f, 16.f, (8 * 64.f), 204.f, 58.f, 51.f);
		RenderFrameX += 22.5;

		SEASON3B::RenderImageF(BITMAP_chat_input_box, RenderFrameX, RenderFrameY + 8.f, 18.f, 16.f, (9 * 64.f), 204.f, 58.f, 51.f);
		RenderFrameX += 22.5;
	}
...
void SEASON3B::CNewUIChatInputBox::OpenningProcess()
{
	...
	//gfxChat::OpenEditText();
	g_pNewUICommandServer->filterClear();

#if MAIN_UPDATE <= 303
	/*if (m_pNewUIChatLogWnd->IsShowFrame())
	{
		m_pNewUIChatLogWnd->SetPosition(PositionX_In_The_Mid(186.f), (int)GetWindowsY - 70);
	}
	else
	{
		m_pNewUIChatLogWnd->SetPosition(0, (int)GetWindowsY - 50);
	}*/
#endif
	//m_pNewChatLogWindow = new CNewUIChatLogWindow;
	//if (false == m_pNewChatLogWindow->Create(m_pNewUIMng, PositionX_In_The_Mid(186.f), (int)GetWindowsY - 52, 6))
	//	return false;
#ifdef WISSPER_CHAT_BLOCKED
	m_bBlockedChat = false;
#endif // WISSPER_CHAT_BLOCKED
...
void SEASON3B::CNewUIChatInputBox::ClosingProcess()
{
	...
	/*if (m_pNewUIChatLogWnd->IsShowFrame())
	{
		m_pNewUIChatLogWnd->SetPosition(PositionX_In_The_Mid(186.f), (int)GetWindowsY - 50);
	}
	else
	{
		m_pNewUIChatLogWnd->SetPosition(0, (int)GetWindowsY - 50);
...
bool SEASON3B::CNewUIChatInputBox::RenderSystemMessage()
{
	return m_bOnlySystemMessage;
}
...
#ifdef WISSPER_CHAT_BLOCKED
bool SEASON3B::CNewUIChatInputBox::CheckBlocked(const unicode::t_char* pzText)
{
	return m_iChatBlocked.CheckInstance(pzText);
}

void SEASON3B::CNewUIChatInputBox::SetWhsprIdBlocked(const unicode::t_char* pzText)
{
	if (m_bBlockedChat)
	{
		m_iChatBlocked.SetText(pzText);
	}
	else
	{
		this->SetWhsprID(pzText);
	}
}
#endif // WISSPER_CHAT_BLOCKED
...
#ifdef WISSPER_CHAT_BLOCKED
void SEASON3B::CNewUIChatInputBox::initListBlocked()
{
	m_iChatBlocked.Create(150, pos_botton(25));
}

void SEASON3B::CNewUIChatInputBox::RenderChatBlocked()
{
	if (m_bBlockedChat)
	{
		m_iChatBlocked.Render();
	}
}

bool SEASON3B::CNewUIChatInputBox::UpdateChatBlocked()
{
	if (m_bBlockedChat)
	{
		m_iChatBlocked.Update(this);
	}
	return false;
}

bool SEASON3B::CNewUIChatInputBox::UpdateMouseActionChatBlocked()
{
	if (m_bBlockedChat)
	{
		return m_iChatBlocked.UpdateMouseEvent();
	}
	return false;
}
#endif // WISSPER_CHAT_BLOCKED
