...
void CUIPhotoViewer::CopyPlayer(CHARACTER* pCharacter)
{
	if (m_bIsInitialized == FALSE)
		return;

	if (m_PhotoChar.Class != pCharacter->Class)
	{
		m_PhotoChar.Class = pCharacter->Class;
		SetChangeClass(&m_PhotoChar);
	}
	...
	if (pCharacter->Change == FALSE)
	{
		for (i = 0; i < MAX_BODYPART; ++i)
		{
			if (CompareItemEqual(&m_PhotoChar.BodyPart[i], &pCharacter->BodyPart[i]) == FALSE)
			{
				bChangeArmor = TRUE;
				break;
			}
		...
		for (i = 0; i < 2; ++i)
		{
			if (CompareItemEqual(&m_PhotoChar.Weapon[i], &pCharacter->Weapon[i]) == FALSE)
			{
				bChangeWeapon = TRUE;
				break;
			}
		}
		if (CompareItemEqual(&m_PhotoChar.Wing, &pCharacter->Wing) == FALSE)
			bChangeWing = TRUE;

#ifdef SHUTDOWN_CORE_WING4
		if (CompareItemEqual(&m_PhotoChar.Core, &pCharacter->Core) == FALSE)
			bChangeWing = TRUE;
#endif // SHUTDOWN_CORE_WING4

#ifdef SYSTEM_FLAG_NAT
		if (CompareItemEqual(&m_PhotoChar.Flag, &pCharacter->Flag) == FALSE)
			bChangeFlag = TRUE;
#endif // SYSTEM_FLAG_NAT

		if (CompareItemEqual(&m_PhotoChar.Helper, &pCharacter->Helper) == FALSE)
			bChangeHelper = TRUE;
	}
	else	// º¯½Å »óÅÂ
	{

		if (CompareItemEqual(&m_PhotoChar.BodyPart[BODYPART_HELM], &CharacterMachine->Equipment[EQUIPMENT_HELM], MODEL_BODY_HELM + gCharacterManager.GetSkinModelIndex(pCharacter->Class)) == FALSE)
			bChangeArmor = TRUE;
		else if (CompareItemEqual(&m_PhotoChar.BodyPart[BODYPART_ARMOR], &CharacterMachine->Equipment[EQUIPMENT_ARMOR], MODEL_BODY_ARMOR + gCharacterManager.GetSkinModelIndex(pCharacter->Class)) == FALSE)
			bChangeArmor = TRUE;
		else if (CompareItemEqual(&m_PhotoChar.BodyPart[BODYPART_PANTS], &CharacterMachine->Equipment[EQUIPMENT_PANTS], MODEL_BODY_PANTS + gCharacterManager.GetSkinModelIndex(pCharacter->Class)) == FALSE)
			bChangeArmor = TRUE;
		else if (CompareItemEqual(&m_PhotoChar.BodyPart[BODYPART_GLOVES], &CharacterMachine->Equipment[EQUIPMENT_GLOVES], MODEL_BODY_GLOVES + gCharacterManager.GetSkinModelIndex(pCharacter->Class)) == FALSE)
			bChangeArmor = TRUE;
		else if (CompareItemEqual(&m_PhotoChar.BodyPart[BODYPART_BOOTS], &CharacterMachine->Equipment[EQUIPMENT_BOOTS], MODEL_BODY_BOOTS + gCharacterManager.GetSkinModelIndex(pCharacter->Class)) == FALSE)
			bChangeArmor = TRUE;
	...
	if (pCharacter->Change == FALSE)
	{
		...
		{
			DeleteCloth(&m_PhotoChar, NULL, NULL);
			memcpy(&m_PhotoChar.BodyPart, &pCharacter->BodyPart, sizeof(PART_t) * MAX_BODYPART);
			for (i = 0; i < MAX_BODYPART; ++i)
			{
				m_PhotoChar.BodyPart[i].m_pCloth[0] = NULL;
				m_PhotoChar.BodyPart[i].m_pCloth[1] = NULL;
		...
		if (bChangeWeapon == TRUE)
		{
			memcpy(&m_PhotoChar.Weapon, &pCharacter->Weapon, sizeof(PART_t) * 2);
		}
		if (bChangeWing == TRUE)
		{
			memcpy(&m_PhotoChar.Wing, &pCharacter->Wing, sizeof(PART_t));
#ifdef SHUTDOWN_CORE_WING4
			memcpy(&m_PhotoChar.Core, &pCharacter->Core, sizeof(PART_t));
#endif // SHUTDOWN_CORE_WING4
			DeleteCloth(NULL, &m_PhotoChar.Object, NULL);
		}
		...
		if (bChangeFlag == TRUE)
		{
			memcpy(&m_PhotoChar.Flag, &pCharacter->Flag, sizeof(PART_t));
		}
#endif // SYSTEM_FLAG_NAT
		if (bChangeHelper == TRUE)
		{
			memcpy(&m_PhotoChar.Helper, &pCharacter->Helper, sizeof(PART_t));
		}
	}
	else
	{
		...
		{
			DeleteCloth(&m_PhotoChar, NULL, NULL);
			m_PhotoChar.BodyPart[BODYPART_HEAD].Type = MODEL_BODY_HELM + pCharacter->GetSkinModelIndex();
			SetItemToPhoto(&m_PhotoChar.BodyPart[BODYPART_HELM], &CharacterMachine->Equipment[EQUIPMENT_HELM], MODEL_BODY_HELM + pCharacter->GetSkinModelIndex());
			SetItemToPhoto(&m_PhotoChar.BodyPart[BODYPART_ARMOR], &CharacterMachine->Equipment[EQUIPMENT_ARMOR], MODEL_BODY_ARMOR + pCharacter->GetSkinModelIndex());
			SetItemToPhoto(&m_PhotoChar.BodyPart[BODYPART_PANTS], &CharacterMachine->Equipment[EQUIPMENT_PANTS], MODEL_BODY_PANTS + pCharacter->GetSkinModelIndex());
			SetItemToPhoto(&m_PhotoChar.BodyPart[BODYPART_GLOVES], &CharacterMachine->Equipment[EQUIPMENT_GLOVES], MODEL_BODY_GLOVES + pCharacter->GetSkinModelIndex());
			SetItemToPhoto(&m_PhotoChar.BodyPart[BODYPART_BOOTS], &CharacterMachine->Equipment[EQUIPMENT_BOOTS], MODEL_BODY_BOOTS + pCharacter->GetSkinModelIndex());
...
void CUIPhotoViewer::SetEquipmentPacket(uint32_t* Equipment)
{
	if (m_bIsInitialized == FALSE)
		return;
