...
CMultiLanguage::CMultiLanguage(std::string strSelectedML)
{
	...
	else if (_stricmp(strSelectedML.c_str(), "TAI") == 0)
	{
		byLanguage = 6;
		iCodePage = CP_THAI;
		iNumByteForOneCharUTF8 = 2;
...
void CMultiLanguage::ConvertANSIToUTF8OrViceVersa(std::string& strDest, LPCSTR lpString)
{
	...
	//if(byLanguage >= 3)
		iDestType = (IsCharUTF8(lpString)) ? this->GetCodePage() : CP_UTF8;
	//else
	//	iDestType = (IsCharUTF8(lpString)) ? CP_UTF8 : this->GetCodePage();

	ConvertCharToWideStr(wstrUTF16, lpString);

	for (int i = 0; i < wstrUTF16.size(); i++)
	{
		wstrUTF16[i] = this->ConvertFulltoHalfWidthChar(wstrUTF16[i]);
	}
