...
bool SEASON3B::CNewUIMyInventory::UpdateMouseEvent()
{
	for (int i = 0; i < this->GetInvenEnableCnt(); i++)
	{
		if (i > 0 && g_pInvenExpansion->CheckExpansionInventory() == false)
		{
			continue;
		}
	...
	if (this->EquipmentWindowProcess()
		|| this->InventoryProcess()
		|| this->BtnProcess())
	{
		return false;
	}
	...
	if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_NPCSHOP)
		...
		|| g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_MIX_EXPANSION)
		)
	{
		ResetMouseLButton();
		return false;
...
void SEASON3B::CNewUIMyInventory::SetEquipmentSlotInfo()
{
	...
	else
	{
		...
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_HELM], EQUIPMENT_HELM, IMAGE_INVENTORY_ITEM_HELM, m_Pos.x + 79, m_Pos.y + 44, 46, 46);

		SetEquipmentInfo(&m_Equipment[EQUIPMENT_WING], EQUIPMENT_WING, IMAGE_INVENTORY_ITEM_WING, m_Pos.x + 128, m_Pos.y + 44, 61, 46);

		//--
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_WEAPON_RIGHT], EQUIPMENT_WEAPON_RIGHT, IMAGE_INVENTORY_ITEM_RIGHT, m_Pos.x + 15, m_Pos.y + 91, 46, 66);

		SetEquipmentInfo(&m_Equipment[EQUIPMENT_ARMOR], EQUIPMENT_ARMOR, IMAGE_INVENTORY_ITEM_ARMOR, m_Pos.x + 79, m_Pos.y + 91, 46, 66);

		SetEquipmentInfo(&m_Equipment[EQUIPMENT_WEAPON_LEFT], EQUIPMENT_WEAPON_LEFT, IMAGE_INVENTORY_ITEM_LEFT, m_Pos.x + 143, m_Pos.y + 91, 46, 66);

		//--
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_GLOVES], EQUIPMENT_GLOVES, IMAGE_INVENTORY_ITEM_GLOVES, m_Pos.x + 15, m_Pos.y + 158, 46, 46);

		SetEquipmentInfo(&m_Equipment[EQUIPMENT_PANTS], EQUIPMENT_PANTS, IMAGE_INVENTORY_ITEM_PANTS, m_Pos.x + 79, m_Pos.y + 158, 46, 46);

		SetEquipmentInfo(&m_Equipment[EQUIPMENT_BOOTS], EQUIPMENT_BOOTS, IMAGE_INVENTORY_ITEM_BOOT, m_Pos.x + 143, m_Pos.y + 158, 46, 46);

		//--
		SetEquipmentInfo(&m_Equipment[EQUIPMENT_RING_LEFT], EQUIPMENT_RING_LEFT, IMAGE_INVENTORY_ITEM_RING, m_Pos.x + 123, m_Pos.y + 158, 22, 22);

		SetEquipmentInfo(&m_Equipment[EQUIPMENT_AMULET], EQUIPMENT_AMULET, IMAGE_INVENTORY_ITEM_NECKLACE, m_Pos.x + 60, m_Pos.y + 90, 22, 22);

		SetEquipmentInfo(&m_Equipment[EQUIPMENT_RING_RIGHT], EQUIPMENT_RING_RIGHT, IMAGE_INVENTORY_ITEM_RING, m_Pos.x + 60, m_Pos.y + 158, 22, 22);
	}
...
void SEASON3B::CNewUIMyInventory::RenderEquippedItem()
{
	...
	if (m_iPointedSlot != -1 && m_pNewUI3DRenderMng)
	{
		...
		{
			g_ConsoleDebug->Write(5, "buscando informacion: (%d : %d)", m_iPointedSlot, m_iBackPointed);
	...
	if (SEASON3B::IsRelease(VK_LBUTTON))
	{
		...
		if (pPickedItem)
		{
			...
			if (this->IsEquipable(EquipmentClientToServer(m_iPointedSlot), pItemObj))
			{
				...
				{
					int iSourceIndex = pPickedItem->GetSourceLinealPos();
					SendRequestEquipmentItem(REQUEST_EQUIPMENT_MYSHOP, INVENTORY_SHOP_INDEX + iSourceIndex, pItemObj, REQUEST_EQUIPMENT_INVENTORY, iTargetIndex);
					return true;
				}
		...
		if (InvenIndex == -1)
		{
			...
			if (g_pStorageInventory->IsInvenctrlSame(pPickedItem->GetOwnerInventory()) == -1)
			{
				if (pPickedItem->GetOwnerInventory() == g_pTrade->GetMyInvenCtrl())
				{
					...
					iTargetIndex = this->GetPickItemTarget(pPickedItem);
				...
				else if (pPickedItem->GetOwnerInventory() == g_pLuckyItemWnd->GetInventoryCtrl())
				{
					iSourceIndex = pPickedItem->GetSourceLinealPos();
					iTargetIndex = this->GetPickItemTarget(pPickedItem);
					iSrcType = g_pLuckyItemWnd->SetWndAction(eLuckyItem_Move);

					if (iTargetIndex != -1 && m_pNewInventoryCtrl[m_InvenIndex]->CanMove(iTargetIndex, pPickItem))
					{
						iTargetIndex = this->SlotCntSendDstInvenIndex(iTargetIndex, m_InvenIndex);
						SendRequestEquipmentItem(iSrcType, iSourceIndex, pPickItem, REQUEST_EQUIPMENT_INVENTORY, MAX_EQUIPMENT_INDEX + iTargetIndex);
						return true;
					}
				}
				else if (pPickedItem->GetOwnerInventory() == g_pMixInventory->GetInventoryCtrl())
				{
					iSourceIndex = pPickedItem->GetSourceLinealPos();
					iTargetIndex = this->GetPickItemTarget(pPickedItem);
					iSrcType = g_MixRecipeMgr.GetMixInventoryEquipmentIndex();
				...
				else if (pPickedItem->GetOwnerInventory() == g_pMyShopInventory->GetInventoryCtrl())
				{
					iSourceIndex = pPickedItem->GetSourceLinealPos();
					iTargetIndex = this->GetPickItemTarget(pPickedItem);
					iSrcType = REQUEST_EQUIPMENT_MYSHOP;

					if (iTargetIndex != -1 && m_pNewInventoryCtrl[m_InvenIndex]->CanMove(iTargetIndex, pPickItem))
					{
						iTargetIndex = this->SlotCntSendDstInvenIndex(iTargetIndex, m_InvenIndex);
						SendRequestEquipmentItem(REQUEST_EQUIPMENT_MYSHOP, INVENTORY_SHOP_INDEX + iSourceIndex, pPickItem, REQUEST_EQUIPMENT_INVENTORY, iTargetIndex + MAX_EQUIPMENT_INDEX);
						return true;
					}
				}
				else if (pPickItem->ex_src_type == ITEM_EX_SRC_EQUIPMENT)
				{
					iSourceIndex = pPickedItem->GetSourceLinealPos();
					iTargetIndex = this->GetPickItemTarget(pPickedItem);
					iSrcType = REQUEST_EQUIPMENT_INVENTORY;

					if (iTargetIndex != -1 && m_pNewInventoryCtrl[m_InvenIndex]->CanMove(iTargetIndex, pPickItem))
					{
						iTargetIndex = this->SlotCntSendDstInvenIndex(iTargetIndex, m_InvenIndex);
						SendRequestEquipmentItem(iSrcType, iSourceIndex, pPickItem, REQUEST_EQUIPMENT_INVENTORY, iTargetIndex + MAX_EQUIPMENT_INDEX);
						return true;
					}
				}
			}
			...
			{
				iSourceIndex = pPickedItem->GetSourceLinealPos();
				iTargetIndex = GetPickItemTarget(pPickedItem);
...
void SEASON3B::CNewUIMyInventory::RenderItemToolTip(int iSlotIndex)
{
	if (m_iPointedSlot != -1 && CNewUIInventoryCtrl::GetPickedItem() == NULL)
	{
		...
		if (pEquipmentItemSlot->Type != -1)
		{
			...
			{
				g_pNewItemTooltip->RenderItemTooltip(iTargetX, iTargetY, pEquipmentItemSlot, false, UNKNOWN_TOOLTIP_TYPE);
			}
			else
			{
...
bool SEASON3B::CNewUIMyInventory::MoveItemProcess()
{
	...
	if (SEASON3B::IsPress(VK_RBUTTON) && this->GetRepairMode() != REPAIR_MODE_ON && EquipmentItem == false)
	{
		...
		{		
			...
			{
				targetX = g_pMixInventory->GetPos().x;
				targetY = g_pMixInventory->GetPos().y;

				int iMixType = g_MixRecipeMgr.GetMixInventoryEquipmentIndex();

				if (SEASON3B::CheckMouseIn(targetX, targetY, WIN_WINDOW_SIZEX, WIN_WINDOW_SIZEY))
				{
			...
			{
				targetX = g_pLuckyItemWnd->GetPos().x;
				targetY = g_pLuckyItemWnd->GetPos().y;
...
#endif
			else if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_MIX_EXPANSION) && g_pMixInvenExpansion->GetMixState() == CNewUIMixInventory::MIX_READY)
			{
				targetX = g_pMixInvenExpansion->GetPos().x;
				targetY = g_pMixInvenExpansion->GetPos().y;
				...
				if (!SEASON3B::CheckMouseIn(targetX, targetY, WIN_WINDOW_SIZEX, WIN_WINDOW_SIZEY))
				{
					...
					if (pItemObj)
					{
						...
						int iDstIndex = g_pMixInvenExpansion->FindEmptySlot();

						if (iDstIndex != -1)
						{
							CNewUIInventoryCtrl* pSrc = this->GetInventoryCtrl(InvenIndex);

							if (g_pMixInvenExpansion->IsMixItem(pItemObj) && pSrc != NULL)
							{
								SetItemAutoMove(true);
								m_bInventoryBase = NULL;
								SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, iSrcIndex + MAX_EQUIPMENT_INDEX, pItemObj, 23, iDstIndex);
								return true;
							}
						}
					}
				...
				if (pItemObj)
				{
					...
					REPEAT_COM:
						...
						else
						{
							...
							if (this->IsEquipable(iDstIndex, pItemObj) && pSrc != NULL)
							{
								SetItemAutoMove(true);
								m_bInventoryBase = NULL;
