...
namespace battleCastle
{
	...
	bool AreOnSameTeam(CHARACTER* a, CHARACTER* b)
	{
		const int teamA = a->EtcPart;
		const int teamB = b->EtcPart;

		// Ataque equipo 1
		if ((teamA == PARTS_ATTACK_KING_TEAM_MARK || teamA == PARTS_ATTACK_TEAM_MARK) &&
			(teamB == PARTS_ATTACK_KING_TEAM_MARK || teamB == PARTS_ATTACK_TEAM_MARK))
		{
			return true;
		}

		// Ataque equipo 2
		if ((teamA == PARTS_ATTACK_KING_TEAM_MARK2 || teamA == PARTS_ATTACK_TEAM_MARK2) &&
			(teamB == PARTS_ATTACK_KING_TEAM_MARK2 || teamB == PARTS_ATTACK_TEAM_MARK2))
		{
			return true;
		}

		// Ataque equipo 3
		if ((teamA == PARTS_ATTACK_KING_TEAM_MARK3 || teamA == PARTS_ATTACK_TEAM_MARK3) &&
			(teamB == PARTS_ATTACK_KING_TEAM_MARK3 || teamB == PARTS_ATTACK_TEAM_MARK3))
		{
			return true;
		}

		// Defensa
		if ((teamA == PARTS_DEFENSE_KING_TEAM_MARK || teamA == PARTS_DEFENSE_TEAM_MARK) &&
			(teamB == PARTS_DEFENSE_KING_TEAM_MARK || teamB == PARTS_DEFENSE_TEAM_MARK))
		{
			return true;
		}
	...
	bool SettingBattleFormation(CHARACTER* c, uint32_t state)
	{
	...
	void DeleteBattleFormation(CHARACTER* c, uint32_t state)
	{
		if (gMapManager->InBattleCastle() == false)  return;
	...
	bool AttackEffect_BattleCastleMonster(CHARACTER* c, OBJECT* o, BMD* b)
	{
		...
		{
			tc = GetCharacterDataUnsafe(c->TargetCharacter);
			to = &tc->Object;
		}
	...
	bool    RenderBattleCastleMonsterObjectMesh(OBJECT* o, BMD* b)
	{
		...
		case MODEL_NPC_CAPATULT_ATT:
		{
			...
			BMD* linkBmd = GetModelDataUnsafe(MODEL_FLY_BIG_STONE1);
