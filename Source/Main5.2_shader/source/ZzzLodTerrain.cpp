...
uint8_t TerrainMappingLayer1[TERRAIN_SIZE * TERRAIN_SIZE];
uint8_t TerrainMappingLayer2[TERRAIN_SIZE * TERRAIN_SIZE];
...
static float g_fFrustumRange = -300.f;
...
int OpenTerrainAttribute(char* FileName)
{
	...
	uint32_t TerrainSize = TERRAIN_SIZE * TERRAIN_SIZE;

	if (extAtt)
	{
		memcpy(TerrainWall, &blockchain[4], TerrainSize * sizeof(WORD));
	}
	else
	{
		std::vector<uint8_t>TWall(TerrainSize);

		memcpy(TWall.data(), &blockchain[4], TerrainSize);
	...
	for (int i = 0; i < TERRAIN_SIZE * TERRAIN_SIZE; i++)
	{
		TerrainWall[i] = TerrainWall[i];
		TerrainWall[i] &= 0xFF;
		if ((uint8_t)TerrainWall[i] >= 128)
			Error = true;
	}
...
{
	if ((gmProtect->SceneLogin == 1 && SceneFlag == LOG_IN_SCENE)
		|| (gmProtect->SceneCharacter == 1 && SceneFlag == CHARACTER_SCENE))
	{
		return 0.f;
	}
...
void RenderTerrainBitmapTile(float xf, float yf, float lodf, int lodi, vec3_t c[4], bool LightEnable, float Alpha, float Height = 0.f, bool terrainback = true)
{
	int xi = (int)xf;
	int yi = (int)yf;
	...
	float TileScale = lodf * TERRAIN_SCALE;
	float sx = xf * TERRAIN_SCALE;
	float sy = yf * TERRAIN_SCALE;
	TerrainIndex1 = TERRAIN_INDEX(xi, yi);
	TerrainIndex2 = TERRAIN_INDEX(xi + lodi, yi);
	...
	{
		Vector(sx, sy, Height, TerrainVertex[0]);
		Vector(sx + TileScale, sy, Height, TerrainVertex[1]);
		Vector(sx + TileScale, sy + TileScale, Height, TerrainVertex[2]);
		Vector(sx, sy + TileScale, Height, TerrainVertex[3]);
	}
...
void RenderTerrainAlphaBitmap(int Texture, float xf, float yf, float SizeX, float SizeY, vec3_t Light, float Rotation, float Alpha, float Height, bool terrainback)
{
	if (Alpha == 1.f)
		glColor3fv(Light);
	else
	...
	for (float y = -Size; y <= Size; y += 1.f)
	{
		for (float x = -Size; x <= Size; x += 1.f)
		{
			...
			RenderTerrainBitmapTile((float)mxi + x, (float)myi + y, 1.f, 1, p2, false, Alpha, Height, terrainback);
		}
	}
}
...
void RenderDebugFrustum()
{
	...
	glColor3f(0.f, 1.f, 0.f); // Verde para líneas del frustum

	glBegin(GL_LINES);

	for (int i = 1; i <= 4; i++)
	{
		glVertex3fv(FrustrumVertex[0]);
		glVertex3fv(FrustrumVertex[i]);
	}

	glVertex3fv(FrustrumVertex[1]); glVertex3fv(FrustrumVertex[2]);
	glVertex3fv(FrustrumVertex[2]); glVertex3fv(FrustrumVertex[3]);
	glVertex3fv(FrustrumVertex[3]); glVertex3fv(FrustrumVertex[4]);
	glVertex3fv(FrustrumVertex[4]); glVertex3fv(FrustrumVertex[1]);
...
bool TestFrustrum2D(float x, float y, float Range)
{
	if (!((gmProtect->SceneLogin == 1 && SceneFlag == LOG_IN_SCENE) || (gmProtect->SceneCharacter == 1 && SceneFlag == CHARACTER_SCENE)))
	{
		...
		{
			float distance = (FrustrumX[i] - x) * (FrustrumY[j] - y) - (FrustrumX[j] - x) * (FrustrumY[i] - y);
...
bool CheckedFrustrum2D(float x, float y, float Range)
{
	if (!((gmProtect->SceneLogin == 1 && SceneFlag == LOG_IN_SCENE) || (gmProtect->SceneCharacter == 1 && SceneFlag == CHARACTER_SCENE)))
	{
		if (SceneFlag == SERVER_LIST_SCENE || SceneFlag == WEBZEN_SCENE || SceneFlag == LOADING_SCENE)
		...
		float collision = Range + gCameraController->GetFrustrumCollision();
...
bool TestFrustrumAABB(vec3_t min, vec3_t max)
{
	for (int i = 0; i < 5; ++i) // 5 planos del frustum
	{
		int outside = 0;
		...
		for (int x = 0; x < 2; ++x)
			for (int y = 0; y < 2; ++y)
				for (int z = 0; z < 2; ++z)
				{
					...
					float d = DotProduct(point, FrustrumFaceNormal[i]) + FrustrumFaceD[i];
					if (d < 0.0f)
						++outside;
				}

		if (outside == 8) // todos los vértices están fuera de este plano
			return false;
	}
	return true; // al menos un vértice está dentro de todos los planos
}
...
{
	int   xi, yi = FrustrumBoundMinY;
	float xf, yf = (float)yi;
	...
	for (; yi <= FrustrumBoundMaxY; yi += 4, yf += 4.f)
	{
		...
		{
			if (CheckedFrustrum2D(xf + 2.f, yf + 2.f, g_fFrustumRange) || CameraTopViewEnable)
			{
