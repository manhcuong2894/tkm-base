...
int CNewUILuckyItemWnd::SetWndAction(eLUCKYITEM _eType)
{
	...
	case eLuckyItemType_Trade:
		if (m_eWndAction == eLuckyItem_Move)
			return 15;
		else if (m_eWndAction == eLuckyItem_Act)
			return 51;
		break;
...
void CNewUILuckyItemWnd::Process_InventoryCtrl_DeleteItem(int iIndex)
{
	if (m_pNewInventoryCtrl)
	{
		...
		if (pItem != NULL) m_pNewInventoryCtrl->RemoveItem(pItem);
	}
}

bool CNewUILuckyItemWnd::Check_LuckyItem_InWnd(void)
{
	return (GetInventoryCtrl()->GetNumberOfItems() > 0);
}
...
{
	return _pItem->Kind3 == ItemKind3::LUCKY;
