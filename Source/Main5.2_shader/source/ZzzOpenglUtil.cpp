...
void RenderCooldownPie(float x, float y, float Width, float Height, float percent)
{
	...
	float angleMax = 2.0f * 3.1415926f * percent;

	float cx = ConvertX(x);
	float cy = ConvertY(y);
	float halfWidth = ConvertX(Width * 0.5f);
	float halfHeight = ConvertY(Height * 0.5f);
	...
	for (int i = 0; i <= NUM_SEGMENTS * percent; ++i)
	{
		float angle = 1.5707963f + ((float)i / NUM_SEGMENTS) * angleMax;
		float px = cx + std::cos(angle) * radius;
		float py = cy + std::sin(angle) * radius;

		// recortar dentro de límites
