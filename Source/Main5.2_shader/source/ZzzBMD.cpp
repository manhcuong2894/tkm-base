...
bool _VAOMesh::SendIndexBone(GLuint Shaderid, const float* Bone, bool bTrans, vec3_t vTrans, float Scale, bool AppScale, float ReqScale)
{
	...
	if (AppScale == true)
	{
		...
		for (int i = 0; i < BoneContainer.size(); ++i)
		{
			...
			for (int j = 0; j < 3; ++j)
			{
				...
				if (loc >= 0)
				{
					glUniform4f(loc + i * 3 + j, *vTarget * ResultScale, *(vTarget + 1) * ResultScale, *(vTarget + 2) * ResultScale, *(vTarget + 3) * ScalePre + vResult[j]);
				}
			}
		}
	}
	else
	{
		for (int i = 0; i < BoneContainer.size(); ++i)
		{
			...
			for (int j = 0; j < 3; ++j)
			{
				...
				if (loc >= 0)
				{
					glUniform4f(loc + i * 3 + j, *vTarget * ResultScale, *(vTarget + 1) * ResultScale, *(vTarget + 2) * ResultScale, *(vTarget + 3) * ResultScale + vResult[j]);
				}
			}
		}
	}
