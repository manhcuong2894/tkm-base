...
bool MoveBug(OBJECT* o, bool bForceRender)
{
	if (o->Live)
	{
		...
		case Movement::Dinorant:
		{
			...
			else if ((o->Owner->CurrentAction >= PLAYER_ATTACK_FIST && o->Owner->CurrentAction <= PLAYER_ATTACK_RIDE_CROSSBOW) || IsAliceRideAction_UniDino(o->Owner->CurrentAction) == true)
			{
				if (o->movementType == Movement::Dinorant)
				{
					if (World == WD_8TARKAN || World == WD_10HEAVEN || g_Direction.m_CKanturu.IsMayaScene())
...
bool RenderBug(OBJECT* o, bool bForceRender)
{
	...
	{
		OBJECT* to = o->Owner;
		o->Visible = (bForceRender == FALSE ? TestFrustrum2D(o->GetPositionX(), o->GetPositionY(), -10.f) : true);
		...
		if (o->Visible && o->Alpha > 0)
		{
			...
			default:
			{
				...
				if (pModel)
				{
					...
					gItemEffect->RenderPetEffects(o->Type, pModel, o);
				}
			}
			break;
			}
...
void RenderBoids(bool bAfterCharacter)
{
	for (int i = 0; i < MAX_BOIDS; i++)
	{
		...
		if (o->Live)
		{
			o->Visible = TestFrustrum2D(o->GetPositionX(), o->GetPositionY(), -10.f);
	...
	for (int i = 0; i < MAX_FISHS; i++)
	{
		...
		if (o->Live)
		{
			...
			if ((o->Type >= MODEL_FISH01 && o->Type <= MODEL_FISH01 + 10) || o->LifeTime > 0)
			{
				...
				VectorRotate(Position, o->Matrix, Direction);
				VectorAddDelta(o->Position, Direction, o->Position);
