...
using FunctionMap = std::unordered_map<std::string, uintptr_t>;
...
bool CreateOpenglWindow()
{
	...
	pfd.cColorBits = 32;   // color 24-bit
	pfd.cDepthBits = 24;   // z-buffer 24-bit
...
HWND GethWnd()
{
	return gwinhandle->GethWnd();
}

FunctionMap& GetExportedFunctions()
{
	static FunctionMap map = {
		{ "CreateNotice", reinterpret_cast<uintptr_t>(&CreateNotice) },
		{ "GethWnd", reinterpret_cast<uintptr_t>(&GethWnd) },
		// agrega más funciones aquí
	};
	return map;
}

extern "C" __declspec(dllexport) uintptr_t GetFunctionOffset(const char* funcionName)
{
	const auto& map = GetExportedFunctions();

	auto it = map.find(funcionName);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int nCmdShow)
{
	...
	if (m_MusicOnOff)
	{
		if (wzAudioCreate(gwinhandle->GethWnd()) == -1)
		{
	...
	if (m_SoundOnOff)
	{
		...
		if (InitDirectSound(gwinhandle->GethWnd()) < 0)
		{
			//SetEnableSound(false);
			//m_MusicOnOff = 0;
			m_SoundOnOff = 0;
