...
void SEASON3B::CNewUICharacterInfoWindow::RenderTableTexts()
{
	...
	{
		unicode::_sprintf(strExp, GlobalText[201], CharacterAttribute->Experience, CharacterAttribute->NextExperience);
	}
...
void SEASON3B::CNewUICharacterInfoWindow::RenderAttribute()
{
	...
	else
	{
		...
		g_pRenderText->RenderText(m_Pos.x + 12, RenderPosY + HEIGHT_STRENGTH + 6, GlobalText[166], 74, 0, RT3_SORT_CENTER);
...
#ifdef SHUTDOWN_CHARACTER_INFO
	if (CharacterAttribute->Work_DefenseSuccessRate > 0)
...
#ifdef SHUTDOWN_CHARACTER_INFO
	if (CLASS_WIZARD == iBaseClass || CLASS_SUMMONER == iBaseClass)
	{
		unicode::_sprintf(strBlocking, GlobalText[64], CharacterAttribute->MagicSpeed);
	}
	else
	{
		unicode::_sprintf(strBlocking, GlobalText[64], CharacterAttribute->AttackSpeed);
	}
#else
...
void SEASON3B::CNewUICharacterInfoWindow::ReceiveAdvanceChar(BYTE* ReceiveBuffer)
{
	...
	m_advanceChar.push_back(CHARACTER_ADVANCE_DESC("ATK Info", TRUE, TRUE, 0));
	...
	m_advanceChar.push_back(CHARACTER_ADVANCE_DESC("DEF Info", TRUE, TRUE, 0));
	...
	m_advanceChar.push_back(CHARACTER_ADVANCE_DESC("Recovery Offen HP", FALSE, Data->OffFullHPRestoreRate));
	m_advanceChar.push_back(CHARACTER_ADVANCE_DESC("Recovery Defen HP", FALSE, Data->DefFullHPRestoreRate));
	m_advanceChar.push_back(CHARACTER_ADVANCE_DESC("Recovery Offen MP", FALSE, Data->OffFullMPRestoreRate));
	m_advanceChar.push_back(CHARACTER_ADVANCE_DESC("Recovery Defen MP", FALSE, Data->DefFullMPRestoreRate));
	m_advanceChar.push_back(CHARACTER_ADVANCE_DESC("Recovery Offen SD", FALSE, Data->OffFullSDRestoreRate));
	m_advanceChar.push_back(CHARACTER_ADVANCE_DESC("Recovery Defen SD", FALSE, Data->DefFullSDRestoreRate));
	m_advanceChar.push_back(CHARACTER_ADVANCE_DESC("Recovery Offen BP", FALSE, Data->OffFullBPRestoreRate));
	m_advanceChar.push_back(CHARACTER_ADVANCE_DESC("Recovery Defen BP", FALSE, Data->DefFullBPRestoreRate));
}
