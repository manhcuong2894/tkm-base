	...
	else if ((pItem->Type < ITEM_WING + 3 || pItem->Type > ITEM_WING + 6) && pItem->Type != ITEM_WING + 42)
	{
		if (pItem->Type != ITEM_HELPER + 30 && pItem->Type != ITEM_WING + 49)
		{
				...
				|| pItem->Type == ITEM_WING + 50)
			{
				...
				{
					pItem->SpecialValue[pItem->SpecialNum] = 5;
					pItem->Special[pItem->SpecialNum++] = AT_DAMAGE_REFLECTION;
				}
				if ((excelWing >> 2) & 1)
				{
					pItem->SpecialValue[pItem->SpecialNum] = 5;
...
{
	Character.MagicDamageMin = Character.Work_MagicDamageMin;
	Character.MagicDamageMax = Character.Work_MagicDamageMax;
	/*DWORD Energy;
	Energy = Character.Energy + Character.AddEnergy;
...
void CHARACTER_MACHINE::CalculateCurseDamage()
{
	...
	Character.CurseDamageMin = Character.Work_CurseDamageMin;
	Character.CurseDamageMax = Character.Work_CurseDamageMax;
...
{
	Character.Defense = Character.Work_Defense;
...
void CHARACTER_MACHINE::CalculateNextExperince()
{
	...
	//	Character.NextExperience = static_cast<DWORD>(baseExperience);
	//}
	//else
	//{
	//	Character.Experience = Character.NextExperience;
...
bool CHARACTER_ATTRIBUTE::CheckMasterLevel()
{
	if (gCharacterManager.IsMasterLevel(CharacterAttribute->Class) && CharacterAttribute->Level >= CharacterAttribute->MaxCharacterLevel)
	{
		return true;
	}
	return false;
...
uint64_t CHARACTER_ATTRIBUTE::character_experience(CharacterExperience Type, int Character_Level)
{
	uint64_t baseExperiencei64 = 0;
