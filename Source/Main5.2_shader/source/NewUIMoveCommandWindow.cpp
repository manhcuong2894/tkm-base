...
const float MOVE_SIZEX = 160.f;
const float MOVE_SIZEY = 360.f;
const float rendimientoX = (WIN_WINDOW_SIZEX / MOVE_SIZEX);
const float rendimientoY = (WIN_WINDOW_SIZEY / MOVE_SIZEY);
...
CNewUIMoveCommandWindow::CNewUIMoveCommandWindow()
{
	...
	m_iTotalSizeView = MOVECOMMAND_MAX_RENDER_TEXTLINE;
	m_iMoveIndex = 2;
	m_iLoadImage = 0;
}
...
void SEASON3B::CNewUIMoveCommandWindow::SetPos(int x, int y)
{
...
#if MAIN_UPDATE > 603
	this->SetTotalSize(15);
#else
	this->SetTotalSize(MOVECOMMAND_MAX_RENDER_TEXTLINE);
#endif
...
#if MAIN_UPDATE > 603
	m_MapNameUISize.x = MOVE_SIZEX;
	m_MapNameUISize.x -= 25;
#else
	m_MapNameUISize.x += 10;
#endif


	m_listMoveInfoData = gMoveCommandDat->GetMoveCommandDatalist();
	m_iRealFontHeight = (FontHeight / g_fScreenRate_y) + 2;

	m_MapNameUISize.y = 60 + (m_iRealFontHeight * this->GetTotalSize());
	...
	m_StartMapNamePos.y = m_Pos.y + 25;
#else
	m_StartMapNamePos.x = m_Pos.x + 2;
	...
	m_iScrollBarHeightPixel = this->GetTotalSize() * m_iRealFontHeight;
	...
	m_iNumPage = 1 + (m_listMoveInfoData.size() / this->GetTotalSize());

	m_iCurPage = 1;

	m_iTotalMoveScrBtnperStep = m_listMoveInfoData.size() - this->GetTotalSize();
	...
	m_iRenderStartTextIndex = 0;
	m_iRenderEndTextIndex = m_iRenderStartTextIndex + this->GetTotalSize();
...
void SEASON3B::CNewUIMoveCommandWindow::SetTotalSize(int total)
{
	m_iTotalSizeView = total;
}

int SEASON3B::CNewUIMoveCommandWindow::GetTotalSize()
{
	return m_iTotalSizeView;
}
...
void SEASON3B::CNewUIMoveCommandWindow::SettingCanMoveMap()
{
	...
	for (int i = 0; i < m_iRenderEndTextIndex; i++, li++)
	{
		...
		if (gCharacterManager.CharacterIsEvo3(CharacterAttribute->Class) && (iReqLevel != CharacterAttribute->MaxCharacterLevel))
		{
...
bool SEASON3B::CNewUIMoveCommandWindow::BtnProcess()
{
	...
	int iRenderFrameX, iRenderFrameY;

#if MAIN_UPDATE > 603
	HEIGHT_SIZE = 20;
	iRenderFrameX = m_Pos.x + 28;
	iRenderFrameY = m_StartMapNamePos.y;
	RenderSizeY = 60 + (HEIGHT_SIZE * this->GetTotalSize());
	RenderSizeX = (m_ScrollBtnPos.x - iRenderFrameX);
#else
	HEIGHT_SIZE = m_iRealFontHeight;
	RenderSizeY = m_MapNameUISize.y;
	RenderSizeX = m_MapNameUISize.x;

	iRenderFrameY = m_StartMapNamePos.y;
	iRenderFrameX = m_StartMapNamePos.x;
#endif
	...
	if (CheckMouseIn(m_Pos.x, m_Pos.y, RenderSizeX, RenderSizeY) && IsPress(VK_LBUTTON))
	{
...
#if MAIN_UPDATE > 603
	RenderSizeX = MOVE_SIZEX + 248.f / rendimientoX;
#endif
	if (CheckMouseIn(iRenderFrameX, iRenderFrameY, RenderSizeX, RenderSizeY))
	{
		RenderSizeX = (m_ScrollBtnPos.x - iRenderFrameX);
		...
		for (int i = 0; i < m_iRenderEndTextIndex; i++, li++)
		{
			...
			iRenderFrameY = m_StartMapNamePos.y + (HEIGHT_SIZE * iCurRenderTextIndex);

			if (CheckMouseIn(iRenderFrameX, iRenderFrameY, RenderSizeX, HEIGHT_SIZE))
			{
				if ((*li)->_bCanMove == true)
				{
					(*li)->_bSelected = true;
					...
					{
#if MAIN_UPDATE > 603
						if (m_iMoveIndex != (*li)->_ReqInfo.index)
						{
							m_iMoveIndex = (*li)->_ReqInfo.index;

							this->LoadImages(m_iMoveIndex);
						}
						else
						{
							if (IsTheMapInDifferentServer(gMapManager->currentMap, m_iMoveIndex))
								SaveOptions();

							SendRequestMoveMap(g_pMoveCommandWindow->GetMoveCommandKey(), m_iMoveIndex);
...
#if MAIN_UPDATE > 603
		iRenderFrameX = m_Pos.x + MOVE_SIZEX + 5;
		int buttonx = 88.f / rendimientoX;
		int buttonposY = m_Pos.y + MOVE_SIZEY - 60.f / rendimientoY;

		if (SEASON3B::IsRelease(VK_LBUTTON) && CheckMouseIn(iRenderFrameX + (MOVE_SIZEX - buttonx) / 2.f, buttonposY, buttonx, 36.f / rendimientoY))
		{
			if (m_iMoveIndex != -1)
			{
				if (IsTheMapInDifferentServer(gMapManager->currentMap, m_iMoveIndex))
...
#else
		if (SEASON3B::IsRelease(VK_LBUTTON) && CheckMouseIn(3, m_MapNameUISize.y - HEIGHT_SIZE - 6, m_MapNameUISize.x - 5, HEIGHT_SIZE))
		{
			g_pNewUISystem->Hide(SEASON3B::INTERFACE_MOVEMAP);
			return true;
		}
...
#if MAIN_UPDATE > 603
	if (CheckMouseIn(m_Pos.x, m_Pos.y, MOVE_SIZEX + 248.f / rendimientoX, MOVE_SIZEY))
		return false;
#else
...
void SEASON3B::CNewUIMoveCommandWindow::RenderMap()
{
#if MAIN_UPDATE > 603
	//788, 1048
	glEnable(GL_SCISSOR_TEST);

	float renderX = (float)(m_Pos.x + MOVE_SIZEX - ((20 - 28) / rendimientoX));
	float renderY = (float)(m_Pos.y + ((5 + 28) / rendimientoY));

	float RenderFrameX = renderX * g_fScreenRate_x;
	float RenderFrameY = renderY * g_fScreenRate_y;
	float RenderSizeX = (202.f / rendimientoX) * g_fScreenRate_x;
	float RenderSizeY = (285.f / rendimientoY) * g_fScreenRate_y;

	glScissor((GLsizei)RenderFrameX, (GLsizei)(WindowHeight - (RenderFrameY + RenderSizeY)), RenderSizeX, RenderSizeY);

	renderX -= (285.f - 202.f) / 2.f;

	EnableAlphaTest();

	glColor4f(0.f, 0.f, 0.f, 1.f);

	RenderColor(renderX, renderY, 285.f, 285.f, 0.0, 0);
	...
	if (m_iLoadImage != 0)
	{
		SEASON3B::RenderImage(BITMAP_move_minimap, renderX, renderY, 285.f, 285.f, 0.0, 0.0, 1.f, 1.f);
	}
...
void SEASON3B::CNewUIMoveCommandWindow::RenderFrame()
{
	...
	SEASON3B::RenderImageF(BITMAP_move_back2, (float)m_Pos.x + MOVE_SIZEX - 20 / rendimientoX, (float)m_Pos.y + 5 / rendimientoY, 248.f / rendimientoX, 414 / rendimientoY, 0.0, 8.f, 964.f, 1522.f);

	SEASON3B::RenderImageF(BITMAP_move_back1, (float)m_Pos.x, (float)m_Pos.y, MOVE_SIZEX, MOVE_SIZEY, 0.0, 0.0, 790.f, 1570.f);
#else
...
void SEASON3B::CNewUIMoveCommandWindow::RenderText()
{
	std::list<CMoveCommandData::MOVEINFODATA*>::iterator li = m_listMoveInfoData.begin();
	...
	for (int i = 0; i < m_iRenderEndTextIndex; i++, li++)
	{
		...
		{
#if MAIN_UPDATE > 603
			float RenderFrameX = iRenderFrameX + (m_ScrollBtnPos.x - iRenderFrameX) / 2.f;

			if ((*li)->_bSelected == true || (*li)->_ReqInfo.index == m_iMoveIndex)
				g_pRenderText->SetTextColor(CLRDW_GOLD);
			else
				g_pRenderText->SetTextColor(CLRDW_WHITE);

			g_pRenderText->RenderText(RenderFrameX, iRenderFrameY, (*li)->_ReqInfo.szMainMapName, 0, HEIGHT_SIZE, RT3_WRITE_CENTER);

			SEASON3B::RenderImageF(BITMAP_move_back2, iRenderFrameX, iRenderFrameY + HEIGHT_SIZE - 3, (m_ScrollBtnPos.x - iRenderFrameX), 6.f, 0.0, 1830.f, 528.f, 19.f);
#else
			g_pRenderText->SetTextColor(CLRDW_WHITE);

			if ((*li)->_bStrife) g_pRenderText->RenderText(m_StrifePos.x, iRenderFrameY, GlobalText[2987], 0, HEIGHT_SIZE, RT3_WRITE_CENTER);

			g_pRenderText->RenderText(m_MapNamePos.x, iRenderFrameY, (*li)->_ReqInfo.szMainMapName, 0, HEIGHT_SIZE, RT3_WRITE_CENTER);

			itoa(iReqLevel, szText, 10);
			g_pRenderText->RenderText(m_ReqLevelPos.x, iRenderFrameY, szText, 0, HEIGHT_SIZE, RT3_WRITE_CENTER);

			itoa((*li)->_ReqInfo.iReqZen, szText, 10);
			g_pRenderText->RenderText(m_ReqZenPos.x, iRenderFrameY, szText, 0, HEIGHT_SIZE, RT3_WRITE_CENTER);

			if ((*li)->_bSelected == true)
			{
				glColor4f(0.8f, 0.8f, 0.1f, 0.6f);
				RenderColor(iRenderFrameX, iRenderFrameY - 1, m_MapNameUISize.x - 22, HEIGHT_SIZE);
	...
	if (m_iMoveIndex != -1)
	{
		const CMoveCommandData::MOVEINFODATA* dataMove = gMoveCommandDat->GetMoveCommandDataByIndex(m_iMoveIndex);

		if (dataMove)
		{
			iReqLevel = dataMove->_ReqInfo.iReqLevel;
			...
			sprintf_s(szText, "%s : %d", GlobalText[935], iReqLevel);

			g_pRenderText->RenderText(iRenderFrameX + (MOVE_SIZEX / 2.f), buttonposY - (m_iRealFontHeight * 4) + 5, szText, 0, 0, RT3_WRITE_CENTER);


			if (dataMove->_ReqInfo.iReqZen > (int)iZen)
				g_pRenderText->SetTextColor(255, 51, 26, 255);
			else
				g_pRenderText->SetTextColor(CLRDW_GOLD);

			iReqLevel = dataMove->_ReqInfo.iReqZen;

			sprintf_s(szText, "%s : %d", GlobalText[936], (DWORD)iReqLevel);

			g_pRenderText->RenderText(iRenderFrameX + (MOVE_SIZEX / 2.f), buttonposY - (m_iRealFontHeight * 3) + 5, szText, 0, 0, RT3_WRITE_CENTER);
		}
	}

	SEASON3B::RenderImageF(BITMAP_move_back2, iRenderFrameX + (MOVE_SIZEX - buttonx) / 2.f, buttonposY, buttonx, 36.f / rendimientoY, 0.0, 1555.f, 329.f, 128.f);
#else
...
void SEASON3B::CNewUIMoveCommandWindow::RenderScrollBar()
{
	...
	RenderImageF(IMAGE_MOVECOMMAND_SCROLL_MIDDLE, m_ScrollBarPos.x, RenderFrameY, MOVECOMMAND_SCROLLBAR_TOP_WIDTH, RenderSizeY, 0.0, 0.0, 7.f, 15.f);

	RenderImageF(IMAGE_MOVECOMMAND_SCROLL_MIDDLE, m_ScrollBarPos.x, RenderFrameY, MOVECOMMAND_SCROLLBAR_TOP_WIDTH, MOVECOMMAND_SCROLLBAR_MIDDLE_HEIGHT, 0.0, 0.0, 7.f, 15.f);

	RenderImageF(IMAGE_MOVECOMMAND_SCROLL_MIDDLE, m_ScrollBarPos.x, RenderFrameY + RenderSizeY - MOVECOMMAND_SCROLLBAR_MIDDLE_HEIGHT, MOVECOMMAND_SCROLLBAR_TOP_WIDTH, MOVECOMMAND_SCROLLBAR_MIDDLE_HEIGHT, 0.0, 0.0, 7.f, 15.f);

	RenderImageF(IMAGE_MOVECOMMAND_SCROLL_TOP, m_ScrollBarPos.x, m_ScrollBarPos.y, MOVECOMMAND_SCROLLBAR_TOP_WIDTH, MOVECOMMAND_SCROLLBAR_TOP_HEIGHT, 0.0, 0.0, 7.f, 3.f);

	RenderImageF(IMAGE_MOVECOMMAND_SCROLL_BOTTOM, m_ScrollBarPos.x, m_ScrollBarPos.y + m_iScrollBarHeightPixel - MOVECOMMAND_SCROLLBAR_TOP_HEIGHT, MOVECOMMAND_SCROLLBAR_TOP_WIDTH, MOVECOMMAND_SCROLLBAR_TOP_HEIGHT, 0.0, 0.0, 7.f, 3.f);
	...
	if (m_bScrollBtnActive == true)
	{
		...
		RenderImage(IMAGE_MOVECOMMAND_SCROLLBAR_ON, m_ScrollBtnPos.x, m_ScrollBtnPos.y, MOVECOMMAND_SCROLLBTN_WIDTH, MOVECOMMAND_SCROLLBTN_HEIGHT);
	...
	{
		RenderImage(IMAGE_MOVECOMMAND_SCROLLBAR_OFF, m_ScrollBtnPos.x, m_ScrollBtnPos.y, MOVECOMMAND_SCROLLBTN_WIDTH, MOVECOMMAND_SCROLLBTN_HEIGHT);
	}
}
...
void SEASON3B::CNewUIMoveCommandWindow::LoadImages()
{
	...
	LoadBitmap("Interface\\newui_scrollbar_m.tga", IMAGE_MOVECOMMAND_SCROLL_MIDDLE, GL_LINEAR, GL_REPEAT);
...
#if MAIN_UPDATE > 603
	LoadBitmap("Interface\\HUD_TEST\\newui_move_back1.dds", BITMAP_move_back1);
	LoadBitmap("Interface\\HUD_TEST\\newui_move_back2.dds", BITMAP_move_back2);
#endif
...
void SEASON3B::CNewUIMoveCommandWindow::LoadImages(int index)
{
	m_iLoadImage = 0;
	...
	if (index != -1)
	{
		int iWorld = this->GetMapIndexFromGate(index);

		if (iWorld != -1)
		{
			char iName[300];
			sprintf_s(iName, "Data\\Interface\\HUD\\NaviMap\\Navimap%02d.ozt", iWorld + 1);
			FILE* pFile = fopen(iName, "rb");

			if (pFile)
			{
				fclose(pFile);

				sprintf_s(iName, "Interface\\HUD\\NaviMap\\Navimap%02d.tga", iWorld + 1);
				LoadBitmap(iName, BITMAP_move_minimap, GL_LINEAR);
				m_iLoadImage = 1;
			}
		}
	}
...
int SEASON3B::CNewUIMoveCommandWindow::GetMapIndexFromGate(int index)
{
	const CMoveCommandData::MOVEINFODATA* dataMove = gMoveCommandDat->GetMoveCommandDataByIndex(index);

	if (dataMove)
	{
		int igate = dataMove->_ReqInfo.iGateNum;

		if (dataMove->_ReqInfo.iGateNum >= 0 && dataMove->_ReqInfo.iGateNum < MAX_GATES)
		{
			return GateAttribute[igate].Map;
		}
	}
