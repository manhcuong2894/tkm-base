...
void SetAttackSpeed()
{
	...
	if (current_skill_id == Skill_Evil_Spirit
		|| current_skill_id == Skill_Evil_Spirit_Strengthener
		|| current_skill_id == Skill_Evil_Spirit_Strengthener2
		)
	{
		magic_speed = CharacterAttribute->MagicSpeed;

		if (magic_speed > 3737 && magic_speed < 6247)
			magic_speed = 3737;
	}
	...
	else if (Hero->GetBaseClass() == Dark_Wizard || Hero->GetBaseClass() == Magic_Gladiator)
	{
		if (/*current_skill_id == Skill_Evil_Spirit
			|| current_skill_id == Skill_Evil_Spirit_Strengthener
			|| current_skill_id == Skill_Evil_Spirit_Strengthener2
			|| */current_skill_id == Skill_Twister
			|| current_skill_id == Skill_Cometfall
			|| current_skill_id == Skill_Decay
			|| current_skill_id == Skill_Decay_Strengthener
			|| current_skill_id == Skill_Ice_Storm
			|| current_skill_id == Skill_Ice_Storm_Strengthener
			|| current_skill_id == Skill_Ice_Storm_Mastery
		...
		else if (current_skill_id == Skill_Flame
			|| current_skill_id == Skill_Flame_Strengthener)
		{
			if (magic_speed > 450 && magic_speed < 480)
				magic_speed = 450;
			else if (magic_speed > 600 && magic_speed < 690)
				magic_speed = 600;
			else if (magic_speed > 891)
				magic_speed = 891;
		}
		else if (current_skill_id == Skill_Fire_Slash
			|| current_skill_id == Skill_Fire_Slash_Strengthener
			|| current_skill_id == Skill_Fire_Slash_Mastery
			)
		{
	...
	{
		if (current_skill_id == Skill_Triple_Shot)
		{
			if (attack_speed > 508)
				attack_speed = 500;
		}
		else if (current_skill_id == Skill_Ice_Arrow
			|| current_skill_id == Skill_Penetration)
		{
			if (attack_speed > 400)
				attack_speed = 400;
		}
	...
	float MagicSpeed2 = magic_speed * 0.0019f;

	BMD* pPlayer = gmClientModels->GetPlayerModel();

	if (pPlayer)
	{
		pPlayer->GetAction(PLAYER_ATTACK_FIST)->PlaySpeed = 0.6f + AttackSpeed1;

		for (int i = PLAYER_ATTACK_SWORD_RIGHT1; i <= PLAYER_ATTACK_RIDE_CROSSBOW; i++)
		{
			pPlayer->GetAction(i)->PlaySpeed = 0.25f + AttackSpeed1;
		}

		pPlayer->GetAction(PLAYER_ATTACK_SKILL_SWORD1)->PlaySpeed = 0.30f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_SKILL_SWORD2)->PlaySpeed = 0.30f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_SKILL_SWORD3)->PlaySpeed = 0.27f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_SKILL_SWORD4)->PlaySpeed = 0.30f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_SKILL_SWORD5)->PlaySpeed = 0.24f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_SKILL_WHEEL)->PlaySpeed = 0.24f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_ONETOONE)->PlaySpeed = 0.25f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_SKILL_SPEAR)->PlaySpeed = 0.30f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_SKILL_RIDER)->PlaySpeed = 0.3f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_SKILL_RIDER_FLY)->PlaySpeed = 0.3f + AttackSpeed1;

		pPlayer->GetAction(PLAYER_ATTACK_TWO_HAND_SWORD_TWO)->PlaySpeed = 0.25f + AttackSpeed1;

		for (int i = PLAYER_ATTACK_BOW; i <= PLAYER_ATTACK_FLY_CROSSBOW; i++)
			pPlayer->GetAction(i)->PlaySpeed = 0.30f + AttackSpeed1;

		for (int i = PLAYER_ATTACK_RIDE_BOW; i <= PLAYER_ATTACK_RIDE_CROSSBOW; i++)
			pPlayer->GetAction(i)->PlaySpeed = 0.30f + AttackSpeed1;

		pPlayer->GetAction(PLAYER_SKILL_ELF1)->PlaySpeed = 0.25f + MagicSpeed1;

		for (int i = PLAYER_SKILL_HAND1; i <= PLAYER_SKILL_WEAPON2; i++)
		{
			pPlayer->GetAction(i)->PlaySpeed = 0.29f + MagicSpeed2;
		}

		pPlayer->GetAction(PLAYER_SKILL_TELEPORT)->PlaySpeed = 0.30f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_FLASH)->PlaySpeed = 0.40f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_INFERNO)->PlaySpeed = 0.60f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_HELL)->PlaySpeed = 0.50f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_RIDE_SKILL)->PlaySpeed = 0.30f + MagicSpeed2;

		pPlayer->GetAction(PLAYER_SKILL_HELL_BEGIN)->PlaySpeed = 0.50f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_ATTACK_STRIKE)->PlaySpeed = 0.25f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_RIDE_STRIKE)->PlaySpeed = 0.2f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_RIDE_HORSE_SWORD)->PlaySpeed = 0.25f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_RIDE_ATTACK_FLASH)->PlaySpeed = 0.40f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_ATTACK_RIDE_ATTACK_MAGIC)->PlaySpeed = 0.3f + MagicSpeed2;

		pPlayer->GetAction(PLAYER_FENRIR_ATTACK)->PlaySpeed = 0.25f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_FENRIR_ATTACK_DARKLORD_STRIKE)->PlaySpeed = 0.2f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_FENRIR_ATTACK_DARKLORD_SWORD)->PlaySpeed = 0.25f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_FENRIR_ATTACK_DARKLORD_FLASH)->PlaySpeed = 0.40f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_FENRIR_ATTACK_TWO_SWORD)->PlaySpeed = 0.25f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_FENRIR_ATTACK_MAGIC)->PlaySpeed = 0.37f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_FENRIR_ATTACK_CROSSBOW)->PlaySpeed = 0.30f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_FENRIR_ATTACK_SPEAR)->PlaySpeed = 0.25f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_FENRIR_ATTACK_ONE_SWORD)->PlaySpeed = 0.25f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_FENRIR_ATTACK_BOW)->PlaySpeed = 0.30f + AttackSpeed1;
		...
		pPlayer->GetAction(PLAYER_ATTACK_ONE_FLASH)->PlaySpeed = 0.4f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_RUSH)->PlaySpeed = 0.3f + AttackSpeed1;
		pPlayer->GetAction(PLAYER_ATTACK_DEATH_CANNON)->PlaySpeed = 0.2f + AttackSpeed1;

		pPlayer->GetAction(PLAYER_SKILL_SLEEP)->PlaySpeed = 0.3f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_SLEEP_UNI)->PlaySpeed = 0.3f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_SLEEP_DINO)->PlaySpeed = 0.3f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_SLEEP_FENRIR)->PlaySpeed = 0.3f + MagicSpeed2;

		pPlayer->GetAction(PLAYER_SKILL_LIGHTNING_ORB)->PlaySpeed = 0.4f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_LIGHTNING_ORB_UNI)->PlaySpeed = 0.25f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_LIGHTNING_ORB_DINO)->PlaySpeed = 0.25f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_LIGHTNING_ORB_FENRIR)->PlaySpeed = 0.25f + MagicSpeed2;

		pPlayer->GetAction(PLAYER_SKILL_CHAIN_LIGHTNING)->PlaySpeed = 0.25f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_CHAIN_LIGHTNING_UNI)->PlaySpeed = 0.15f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_CHAIN_LIGHTNING_DINO)->PlaySpeed = 0.15f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_CHAIN_LIGHTNING_FENRIR)->PlaySpeed = 0.15f + MagicSpeed2;

		pPlayer->GetAction(PLAYER_SKILL_DRAIN_LIFE)->PlaySpeed = 0.25f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_DRAIN_LIFE_UNI)->PlaySpeed = 0.25f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_DRAIN_LIFE_DINO)->PlaySpeed = 0.25f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_DRAIN_LIFE_FENRIR)->PlaySpeed = 0.25f + MagicSpeed2;

		pPlayer->GetAction(PLAYER_SKILL_GIGANTICSTORM)->PlaySpeed = 0.55f + MagicSpeed1;
		pPlayer->GetAction(PLAYER_SKILL_FLAMESTRIKE)->PlaySpeed = 0.69f + MagicSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_LIGHTNING_SHOCK)->PlaySpeed = 0.35f + MagicSpeed2;

		pPlayer->GetAction(PLAYER_SKILL_SUMMON)->PlaySpeed = 0.25f;
		pPlayer->GetAction(PLAYER_SKILL_SUMMON_UNI)->PlaySpeed = 0.25f;
		pPlayer->GetAction(PLAYER_SKILL_SUMMON_DINO)->PlaySpeed = 0.25f;
		pPlayer->GetAction(PLAYER_SKILL_SUMMON_FENRIR)->PlaySpeed = 0.25f;


		pPlayer->GetAction(PLAYER_SKILL_BLOW_OF_DESTRUCTION)->PlaySpeed = 0.3f;
		pPlayer->GetAction(PLAYER_RECOVER_SKILL)->PlaySpeed = 0.33f;
		pPlayer->GetAction(PLAYER_SKILL_SWELL_OF_MP)->PlaySpeed = 0.2f;

		pPlayer->GetAction(PLAYER_ATTACK_SKILL_FURY_STRIKE)->PlaySpeed = 0.38f;
		pPlayer->GetAction(PLAYER_SKILL_VITALITY)->PlaySpeed = 0.34f;
		pPlayer->GetAction(PLAYER_SKILL_HELL_START)->PlaySpeed = 0.30f;
		pPlayer->GetAction(PLAYER_ATTACK_TELEPORT)->PlaySpeed = 0.28f;
		pPlayer->GetAction(PLAYER_ATTACK_RIDE_TELEPORT)->PlaySpeed = 0.3f;
		pPlayer->GetAction(PLAYER_ATTACK_DARKHORSE)->PlaySpeed = 0.3f;
		pPlayer->GetAction(PLAYER_FENRIR_ATTACK_DARKLORD_TELEPORT)->PlaySpeed = 0.3f;
		pPlayer->GetAction(PLAYER_ATTACK_REMOVAL)->PlaySpeed = 0.28f;
		pPlayer->GetAction(PLAYER_SKILL_THRUST)->PlaySpeed = 0.4f + AttackSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_STAMP)->PlaySpeed = 0.4f + AttackSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_GIANTSWING)->PlaySpeed = 0.4f + AttackSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_DARKSIDE_READY)->PlaySpeed = 0.3f + AttackSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_DARKSIDE_ATTACK)->PlaySpeed = 0.3f + AttackSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_DRAGONKICK)->PlaySpeed = 0.4f + AttackSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_DRAGONLORE)->PlaySpeed = 0.3f + AttackSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_ATT_UP_OURFORCES)->PlaySpeed = 0.35f;
		pPlayer->GetAction(PLAYER_SKILL_HP_UP_OURFORCES)->PlaySpeed = 0.35f;
		pPlayer->GetAction(PLAYER_RAGE_FENRIR_ATTACK_RIGHT)->PlaySpeed = 0.25f + AttackSpeed2;
		pPlayer->GetAction(PLAYER_SKILL_PHOENIX_SHOT)->PlaySpeed = 0.74;
	}
}
...
void SetPlayerAttack(CHARACTER* c)
{
	...
	if (o->Type == MODEL_PLAYER)
	{
		...
		else if ((sc_type_helper == MODEL_HELPER + 2 || o->movementType == Movement::Uniria || sc_type_helper == MODEL_HELPER + 3 || o->movementType == Movement::Dinorant) && !c->SafeZone)
		{
			if (sc_type_weapon01 < MODEL_SPEAR || sc_type_weapon01 >= MODEL_SPEAR + 5)
			{
				if (sc_type_weapon01 < MODEL_SPEAR + 5 || sc_type_weapon01 >= MODEL_BOW)
				{
					if (gCharacterManager.GetEquipedBowType(c) == BOWTYPE_BOW)
					{
						SetAction(o, PLAYER_ATTACK_RIDE_BOW, 1);
					}
					else if (gCharacterManager.GetEquipedBowType(c) == BOWTYPE_CROSSBOW)
					{
						SetAction(o, PLAYER_ATTACK_RIDE_CROSSBOW, 1);
					}
					else if (sc_type_weapon01 == -1)
					{
						if (c->GetBaseClass() == CLASS_RAGEFIGHTER)
							...
							SetAction(o, PLAYER_ATTACK_RIDE_SWORD, 1);
					}
					else if (c->GetBaseClass() == CLASS_RAGEFIGHTER)
					{
					...
					{
						SetAction(o, PLAYER_ATTACK_RIDE_TWO_HAND_SWORD, 1);
					}
					else
					{
						SetAction(o, PLAYER_ATTACK_RIDE_SWORD, 1);
				...
				{
					SetAction(o, PLAYER_ATTACK_RIDE_SCYTHE, 1);
				}
			}
			else
			{
				SetAction(o, PLAYER_ATTACK_RIDE_SPEAR, 1);
			}
		}
		else if ((sc_type_helper == MODEL_HELPER + 4 || o->movementType == Movement::Horse) && !c->SafeZone)
		{
			...
			{
				SetAction(o, PLAYER_ATTACK_RIDE_HORSE_SWORD, 1);
			}
		}
		else
		{
			if (sc_type_weapon01 != -1 || sc_type_weapon02 != -1)
			{
				if (sc_type_weapon01 < MODEL_SWORD || sc_type_weapon01 >= MODEL_MACE + MAX_ITEM_INDEX)
				{
					if (sc_type_weapon02 < MODEL_SWORD || sc_type_weapon02 >= MODEL_MACE + MAX_ITEM_INDEX)
					{
						if (sc_type_weapon01 < MODEL_STAFF || sc_type_weapon01 >= MODEL_STAFF + MAX_ITEM_INDEX)
						{
							if (sc_type_weapon01 != MODEL_SPEAR + 1 && sc_type_weapon01 != MODEL_SPEAR + 2)
							{
								if (sc_type_weapon01 < MODEL_SPEAR || sc_type_weapon01 >= MODEL_SPEAR + MAX_ITEM_INDEX)
								{
									...
									{
										if (c->Wing.Type == -1)
											SetAction(o, PLAYER_ATTACK_BOW, 1);
										else
											SetAction(o, PLAYER_ATTACK_FLY_BOW, 1);
									}
									else if (gCharacterManager.GetEquipedBowType(c) == 2)
									{
										if (c->Wing.Type == -1)
											SetAction(o, PLAYER_ATTACK_CROSSBOW, 1);
										else
											SetAction(o, PLAYER_ATTACK_FLY_CROSSBOW, 1);
									}
									else
									{
										SetAction(o, PLAYER_ATTACK_FIST, 1);
									}
								}
								else
								{
									SetAction(o, c->SwordCount % 3 + PLAYER_ATTACK_SCYTHE1, 1);
								}
							}
							else
							{
								SetAction(o, PLAYER_ATTACK_SPEAR1, 1);
							}
						}
						else if (GMItemMng->find((sc_type_weapon01 - MODEL_ITEM))->TwoHand)
						{
						...
						{
							SetAction(o, (rand() % 2) + PLAYER_ATTACK_SWORD_RIGHT1, 1);
						}
					}
					else
					{
						SetAction(o, (rand() % 2) + PLAYER_ATTACK_SWORD_LEFT1, 1);
					}
				}
				else if (GMItemMng->find((sc_type_weapon01 - MODEL_ITEM))->TwoHand)
				{
					...
					{
						SetAction(o, c->SwordCount % 3 + PLAYER_ATTACK_TWO_HAND_SWORD1, 1);
					}
					else
					{
						SetAction(o, PLAYER_ATTACK_TWO_HAND_SWORD_TWO, 1);
					}
				}
				else if (sc_type_weapon02 < MODEL_SWORD || sc_type_weapon02 >= MODEL_MACE + MAX_ITEM_INDEX)
				{
					SetAction(o, c->SwordCount % 2 + PLAYER_ATTACK_SWORD_RIGHT1, 1);
				}
				else
				{
					...
					{
					case 0:
						SetAction(o, PLAYER_ATTACK_SWORD_RIGHT1, 1);
						break;
					case 1:
						SetAction(o, PLAYER_ATTACK_SWORD_LEFT1, 1);
						break;
					case 2:
						SetAction(o, PLAYER_ATTACK_SWORD_RIGHT2, 1);
						break;
					case 3:
						SetAction(o, PLAYER_ATTACK_SWORD_LEFT2, 1);
						break;
					}
				}
			}
...
void SetPlayerDie(CHARACTER* c)
{
	...
	if (c->Object.AnimationFrame == 0.f)
	{
		...
		else
		{
			...
			if (pModel->Sounds[4] != -1)
			{
				int offset = 0;
				if (o->SubType == 9)
					offset = 5;

				PlayBuffer(SOUND_MONSTER + offset + pModel->Sounds[4], o);
			}
		}
		c->Object.AnimationFrame = 0.001;
	}
}
	...
	case Skill_Fire_Slash:
		if (o->Type == MODEL_PLAYER)
		{
			...
			if (o->AnimationFrame >= 3.f)
			{
				...
				gmClientModels->GetPlayerModelAction(PLAYER_ATTACK_SKILL_WHEEL)->PlaySpeed = 0.54f + AttackSpeed1;
				c->AttackTime = 15;
			}
		}
		g_iLimitAttackTime = 15;
...
void PlayerStopAnimationSetting(CHARACTER* c, OBJECT* o)
{
	if (o->CurrentAction != PLAYER_DIE1 && o->CurrentAction != PLAYER_DIE2)
	{
		if (o->CurrentAction == PLAYER_STOP_FLY_BOW || (o->CurrentAction < PLAYER_WALK_MALE
			...
			|| o->CurrentAction >= PLAYER_ATTACK_FIST
...
void CreateWeaponBlur(CHARACTER* c, OBJECT* o, BMD* b)
{
	if ((o->AnimationFrame >= 3.f || (o->CurrentAction == PLAYER_ATTACK_TWO_HAND_SWORD_TWO)))
	{
		...
		if (o->Type == MODEL_PLAYER)
		{
			...
			{
				if ((o->CurrentAction >= PLAYER_ATTACK_SWORD_RIGHT1 && o->CurrentAction <= PLAYER_ATTACK_TWO_HAND_SWORD3) || o->CurrentAction == PLAYER_ATTACK_TWO_HAND_SWORD_TWO)
				{
					BlurType = 1;
					...
					else if (o->CurrentAction == PLAYER_ATTACK_TWO_HAND_SWORD3 || o->CurrentAction == PLAYER_ATTACK_TWO_HAND_SWORD_TWO)
					{
	...
	if (b)
	{
		...
		if (c->AttackTime >= g_iLimitAttackTime)
		{
			...
			case Skill_Flame_Strengthener2:
				...
				break;
			case Skill_Twister:
				CreateEffect(MODEL_STORM, o->Position, o->Angle, Light, 0, o, o->PKKey, FindHotKey(Skill_Twister));
...
void MoveCharacterVisual(CHARACTER* c, OBJECT* o)
{
	...
	if (b->NumActions == 0)
	{
		...
		VectorAddDelta(o->OBB.StartPos, o->Position, o->OBB.StartPos);
...
void RenderLinkObject(float x, float y, float z, CHARACTER* c, PART_t* f, int Type, int Level, int Option1, bool Link, bool Translate, int RenderType, bool bRightHandItem)
{
	...
	if (pModel == NULL
		...
		|| Type == (MODEL_WING + 135)
		//|| Type == (ITEM_WING + 130)
		|| gCharacterManager.GetEquipedBookType(Type)
	...
	if ((c->Skill == Skill_Penetration || c->Skill == Skill_Penetration_Strengthener)
		...
		&& pObject->CurrentAction >= PLAYER_ATTACK_FIST
	...
	case MODEL_SWORD + 19:
		...
		if (((pObject->CurrentAction<PLAYER_WALK_MALE || pObject->CurrentAction>PLAYER_RUN_RIDE_WEAPON) &&
			(pObject->CurrentAction<PLAYER_ATTACK_SKILL_SWORD1 || pObject->CurrentAction>PLAYER_ATTACK_SKILL_SWORD5)))
		{
	...
	case MODEL_BOW + 24:
	{
		...
		if (pObject->CurrentAction >= PLAYER_ATTACK_FIST && pObject->CurrentAction <= PLAYER_RIDE_SKILL)
		{
			Vector(0.2f, 0.8f, 0.5f, vLight);
			for (int i = 0; i < 8; i++)
			{
	...
	gItemEffect->RenderEffectVisual(Type, pModel, Object);
...
void RenderCharacter(CHARACTER* c, OBJECT* o, int Select)
{
	...
	else if (c->MonsterIndex == 71 || c->MonsterIndex == 74 || c->MonsterIndex == 301)
	{
		if (!c->Object.m_pCloth)
		{
			...
			pCloth[0].Create(&(c->Object), 19, 0.0f, 10.0f, 0.0f, 5, 15, 30.0f, 300.0f, iTex, iTex, PCT_RUBBER2 | PCT_MASK_ALPHA);
			c->Object.m_pCloth = (void*)pCloth;
			c->Object.m_byNumCloth = 1;
		}
		CPhysicsCloth* pCloth = (CPhysicsCloth*)c->Object.m_pCloth;
	...
	else if (c->MonsterIndex == 77)
	{
		...
		{
			CPhysicsCloth* pCloth = new CPhysicsCloth[1];
			pCloth[0].Create(o, 10, 0.0f, -10.0f, 0.0f, 5, 12, 15.0f, 240.0f, BITMAP_PHO_R_HAIR, BITMAP_PHO_R_HAIR, PCT_RUBBER2 | PCT_MASK_ALPHA);
	...
	if (!Bind)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (w->Type != -1 && weapon02Kind2 != ItemKind2::ARROW && weapon02Kind2 != ItemKind2::BOLT && weapon02Kind2 != ItemKind2::QUIVER && w->Type != MODEL_HELPER + 5)
			{
				...
				if (o->CurrentAction != PLAYER_ATTACK_BOW
					...
					&& o->CurrentAction != PLAYER_ATTACK_FLY_CROSSBOW)
				{
					...
					case MODEL_MACE + 2:
						...
						{
							w->CurrentAction = 2;
							PlayerAction = gmClientModels->GetPlayerModelAction(PLAYER_ATTACK_SWORD_RIGHT1);
							w->PlaySpeed = PlayerAction->PlaySpeed;
						}
						break;
					case MODEL_MACE + 5:
				...
				else
				{
					if (w->Type == (MODEL_BOW + 23))
						...
						w->CurrentAction = 0;
					PlayerAction = gmClientModels->GetPlayerModelAction(PLAYER_ATTACK_BOW);
					w->PlaySpeed = PlayerAction->PlaySpeed;
				}
	...
	case MODEL_PLAYER:
		...
		{
			if (c->GetBaseClass() == CLASS_DARK_LORD || o->CurrentAction == PLAYER_ATTACK_RIDE_ATTACK_FLASH || o->CurrentAction == PLAYER_FENRIR_ATTACK_DARKLORD_FLASH)
			{
	...
	default:
		if (!RenderHellasMonsterVisual(c, o, b)
			&& !battleCastle::RenderBattleCastleMonsterVisual(c, o, b)
			&& !M31HuntingGround::RenderHuntingGroundMonsterVisual(c, o, b)
			&& !M34CryingWolf2nd::RenderCryingWolf2ndMonsterVisual(c, o, b)
			&& !M34CryWolf1st::RenderCryWolf1stMonsterVisual(c, o, b)
			&& !M33Aida::RenderAidaMonsterVisual(c, o, b)
			&& !M37Kanturu1st::RenderKanturu1stMonsterVisual(c, o, b)
			&& !M38Kanturu2nd::Render_Kanturu2nd_MonsterVisual(c, o, b)
			&& !M39Kanturu3rd::RenderKanturu3rdMonsterVisual(c, o, b)
			&& !SEASON3A::CGM3rdChangeUp::Instance().RenderBalgasBarrackMonsterVisual(c, o, b)
			&& !g_CursedTemple->RenderMonsterVisual(c, o, b)
			&& !SEASON3B::GMNewTown::RenderMonsterVisual(c, o, b)
			&& !SEASON3C::GMSwampOfQuiet::RenderMonsterVisual(c, o, b)
			)
		{
			TheMapProcess().RenderMonsterVisual(c, o, b);
		}
	}

	gMonsterEffect->RenderEffectVisual(b, c, o);
...
static std::vector<CHARACTER*> g_vCharactersToRender;

void RenderCharactersClient()
{
	g_vCharactersToRender.clear();

	for (int i = 0; i < MAX_CHARACTERS_CLIENT; ++i)
	{
		CHARACTER* pCharacter = GetCharacterDataUnsafe(i);
		OBJECT* to = &pCharacter->Object;

		if (!pCharacter->GetLive())
			continue;

		if (SceneFlag == MAIN_SCENE)
		{
			bool Visible = gCullingFace.SphereInFrustum(to->Position, to->GetCollision(), 0.0);

			pCharacter->SetVisible(Visible);
		}

		if (!pCharacter->GetVisible())
			continue;

		g_vCharactersToRender.push_back(pCharacter);
	}

	for (CHARACTER* pCharacter : g_vCharactersToRender)
	{
		OBJECT* to = &pCharacter->Object;

		if (SceneFlag == MAIN_SCENE)
		{
			// Excluir personajes invisibles por buffs en Castle Siege
			if (pCharacter != Hero && battleCastle::IsBattleCastleStart() && g_isCharacterBuff(to, eBuff_Cloaking))
			{
				if (!battleCastle::AreOnSameTeam(Hero, pCharacter))
				{
					if (Hero->TargetCharacter == pCharacter->Key)
						Hero->TargetCharacter = -1;
					continue;
				}
			}

			// Evitar render especial del Hero GM (CtlCode 0x04)
			if (pCharacter == Hero && (Hero->CtlCode & 0x04) && SceneFlag == MAIN_SCENE)
			{
				to->OBB.StartPos[0] = 1000.f;
				VectorSet(to->OBB.XAxis, 1.f, 1.f, 1.f);
				continue;
			}
		}
		...
		{
			int index = gmCharacters->GetCharacterIndex(pCharacter);
			bool isSelected = (index == SelectedCharacter || index == SelectedNpc);

			RenderCharacter(pCharacter, to, isSelected);

			if (to->Type == MODEL_PLAYER)
				battleCastle::CreateBattleCastleCharacter_Visual(pCharacter, to);
		}
	}

	if (SceneFlag == MAIN_SCENE && (gMapManager->InBattleCastle() || World == WD_31HUNTING_GROUND))
	{
	...
	/*for (int i = 0; i < MAX_CHARACTERS_CLIENT; ++i)
	{
		...
		if (pCharacter != Hero && battleCastle::IsBattleCastleStart() == true && g_isCharacterBuff(to, eBuff_Cloaking))
		{
		...
		if (pCharacter == Hero && (0x04 & Hero->CtlCode) && SceneFlag == MAIN_SCENE)
		{
			to->OBB.StartPos[0] = 1000.0000;
			to->OBB.XAxis[0] = 1.00000;
			to->OBB.XAxis[1] = 1.00000;
			to->OBB.XAxis[2] = 1.00000;
			continue;
		}
		...
		if (to->Live)
		{
			if (to->Visible)
			{
				RenderCharacter(pCharacter, to, (i == SelectedCharacter || i == SelectedNpc));
...
void GetBytePart(int partIndex, PART_t* f, uint32_t packedData, BYTE SkinModelIndex)
{
...
void ChangeCharacterExt(int Key, uint32_t* Equipment, CHARACTER* pCharacter, OBJECT* pHelper)
{
	CHARACTER* c;
	if (pCharacter == NULL)
...
bool IsBackItem(CHARACTER* c, int iType)
{
	...
	return (iType >= MODEL_SWORD && iType < MODEL_HELM && (!gCharacterManager.GetEquipedBookType(iType)));
}
