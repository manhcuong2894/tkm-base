...
#ifdef test_main_dev
	if (false == m_pNewInventoryCtrl->Create(g_pNewUI3DRenderMng, g_pNewItemMng, this, x + 22, y + 50, 8, 15))
	{
...
#ifdef test_main_dev
	m_BtnRepair.SetPos(m_Pos.x + 54, m_Pos.y + 384);
	m_BtnRepairAll.SetPos(m_Pos.x + WIN_WINDOW_SIZEX - 74, m_Pos.y + 384);
#else
...
#ifdef test_main_dev
	m_BtnRepair.ChangeButtonImgState(true, BITMAP_btn_repair_00, false);
	m_BtnRepair.ChangeButtonInfo(m_Pos.x + 54, m_Pos.y + 384, 20, 20);

	m_BtnRepairAll.ChangeButtonImgState(true, BITMAP_btn_repair_00, false);
	m_BtnRepairAll.ChangeButtonInfo(m_Pos.x + WIN_WINDOW_SIZEX - 74, m_Pos.y + 384, 20, 20);

	m_BtnRepair.CurSize(69.f, 72.f);
	m_BtnRepairAll.CurSize(69.f, 72.f);
#else
	if (gmProtect->checkold_school())
	{
