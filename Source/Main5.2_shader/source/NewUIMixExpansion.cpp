...
const POINT& SEASON3B::CNewUIMixExpansion::GetPos() const
{
	return m_Pos;
}
...
{
	if (m_iNumItemCnt > 0 || CNewUIInventoryCtrl::GetPickedItem() != NULL)
	{
...
void SEASON3B::CNewUIMixExpansion::SetMixState(int iMixState)
{
	m_iMixState = iMixState;
}

int SEASON3B::CNewUIMixExpansion::GetMixState()
{
	return m_iMixState;
}
...
bool SEASON3B::CNewUIMixExpansion::ButtonProcess()
{
	if (this->m_iNumMixIndex != -1)
	{
		...
		if (this->GetMixState() != CNewUIMixInventory::MIX_FINISHED)
		{
			if (m_BtnMix.UpdateMouseEvent())
			{
	...
	if (SEASON3B::CheckMouseIn(m_Pos.x + 25, m_Pos.y + 215, 160, 160) == true)
	{
		if (g_pMyInventory->GetRepairMode() != 1 && EquipmentItem == false)
		{
			...
			if (pPickedItem)
			{
				...
				{
					if (SEASON3B::IsPress(VK_LBUTTON) && this->GetMixState() == CNewUIMixInventory::MIX_READY)
					{
						...
						else
						{
							...
							if (InvenIndex != -1 && m_iDstIndex != -1)
							{
								...
								SendRequestEquipmentItem(0, iSourceIndex + MAX_EQUIPMENT_INDEX, pItemObj, 23, m_iDstIndex);
							}
						}
					}
				}
				...
				if (SEASON3B::IsPress(VK_RBUTTON) && m_iTargetItem != -1 && m_iTargetItem == m_iRenderItem && !this->IsItemAutoMove())
				{
					if (m_pdwItemCheckBox[m_iTargetItem] != 0)
					{
						...
						if (m_iDstIndex != -1)
						{
							...
							SendRequestEquipmentItem(23, m_iTargetItem, pItemObj, 0, m_iDstIndex + MAX_EQUIPMENT_INDEX);
...
void SEASON3B::CNewUIMixExpansion::SendRequestInfo(int index, BYTE Type)
{
	if (Type == 0)
	{
		this->SetMixState(CNewUIMixInventory::MIX_READY);
		if (index < (int)m_InfoBtnCnt.size())
		{
	...
	{
		this->SetMixState(CNewUIMixInventory::MIX_REQUESTED);
		SendRequestMixState(1);
	}
}
...
void SEASON3B::CNewUIMixExpansion::ReceiveInventory(BYTE* ReceiveBuffer)
{
	...
	this->SetMixState(CNewUIMixInventory::MIX_FINISHED);

	//g_ConsoleDebug->Write(5, "recibiendo datos: %d", Data->Value);
...
bool SEASON3B::CNewUIMixExpansion::IsMixItem(ITEM* pItemObj)
{
	...
	for (int i = 0; i < (int)mixRecipient.size(); i++)
	{
