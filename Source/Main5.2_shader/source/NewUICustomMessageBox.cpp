	...
	else
	{
		...
		if (pPickedItem)
		{
			...
			else if (pPickedItem->GetOwnerInventory() == g_pMyShopInventory->GetInventoryCtrl())
			{
				...
				SendRequestEquipmentItem(REQUEST_EQUIPMENT_MYSHOP, MAX_EQUIPMENT_INDEX + iSourceIndex, pItemObj, REQUEST_EQUIPMENT_MYSHOP, INVENTORY_SHOP_INDEX + iTargetIndex);
			}

			AddPersonalItemPrice(INVENTORY_SHOP_INDEX + iTargetIndex, 0, iInputZen, 0, 0, 0, g_IsPurchaseShop);
		}
		else
		{
			iSourceIndex = g_pMyShopInventory->GetSourceIndex();
			iSourceIndex = INVENTORY_SHOP_INDEX + iSourceIndex;
...
CALLBACK_RESULT SEASON3B::CPersonalShopProItemValueMsgBoxLayout::ProcessOk(CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pPickedItem)
	{
		...
		AddPersonalItemPrice(INVENTORY_SHOP_INDEX + iTargetIndex, pMsgBox->GetPersonalType(), iInputZen, iInputJoB, iInputJoS, iInputJoC, g_IsPurchaseShop);
	}
	else
	{
		iSourceIndex = g_pMyShopInventory->GetSourceIndex();
