...
void SEASON3B::CNewUIMasterSkillTree::RenderText()
{
	...
	sprintf_s(Buffer, GlobalText[1746], Master_Level_Data.nMLevel);

	g_pRenderText->RenderText(x + 275, y + 11, Buffer, 0, 0, 1, 0);

	sprintf_s(Buffer, GlobalText[1747], Master_Level_Data.nMLevelUpMPoint);

	g_pRenderText->RenderText(x + 372, y + 11, Buffer, 0, 0, 1, 0);

	if (Master_Level_Data.lNext_MasterLevel_Experince != 0)
	{
		uint64_t Experience = Master_Level_Data.lMasterLevel_Experince;
		uint64_t NexExperience = Master_Level_Data.lNext_MasterLevel_Experince;
		uint64_t baseExperience = Master_Level_Data.lBase_MasterLevel_Experince;
		//90-80=10
		double fNeedExp = NexExperience - baseExperience;
		if (fNeedExp > 0)
		{
			double fExp = Experience - baseExperience;

			sprintf_s(Buffer, GlobalText[3335], (fExp / fNeedExp) * 100.0);
		}
		else
		{
			sprintf_s(Buffer, GlobalText[3335], 100.0);
		}
	...
	sprintf_s(Buffer, GlobalText[this->categoryTextIndex], this->CategoryPoint[0]);

	g_pRenderText->RenderText(x + 92, y + 40, Buffer, 0, 0, 1, 0);

	sprintf_s(Buffer, GlobalText[this->categoryTextIndex + 1], this->CategoryPoint[1]);

	g_pRenderText->RenderText(x + 302, y + 40, Buffer, 0, 0, 1, 0);

	sprintf_s(Buffer, GlobalText[this->categoryTextIndex + 2], this->CategoryPoint[2]);
