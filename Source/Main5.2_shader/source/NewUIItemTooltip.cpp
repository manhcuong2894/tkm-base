...
void CNewUIItemTooltip::OpenItemTooltipText(const char* filename)
{
	...
	g_ConsoleDebug->Write(5, "SIZE: %d | sizeof: %d", Size, sizeof(_ITEM_TOOLTIP_TEXT));
	if (Size == 0)
	{
		SendMessage(gwinhandle->GethWnd(), WM_DESTROY, 0, 0);
	}
	...
	if (tooltipdata)
	{
		...
		if (ItemType != ITEM_HELPER + 4 && ItemType != ITEM_HELPER + 5)
		{
			if (pItem->Kind3 == ItemKind3::PET_GAIN_EXPERIENCE)
			{
				...
				{
					RequestPetInfo(pItem->x, pItem->y, pItem, 1);
			...
			if ((InvenType == SEASON3B::TOOLTIP_TYPE_MY_SHOP || InvenType == SEASON3B::TOOLTIP_TYPE_PURCHASE_SHOP) && !IsPersonalShopBan(pItem))
			{
				...
				int price = 0;
				char Text[100] = {0, };
				int indexInv = pItem->x + (COL_PERSONALSHOP_INVEN * pItem->y) + INVENTORY_SHOP_INDEX;
			...
			for (int i = 0; i < pItem->SpecialNum && (pItem->Type < ITEM_HELPER + 109 || pItem->Type > ITEM_HELPER + 115 || IsEnableOptionWing(pItem->Type)); i++)
			{
				...
				else if (pItem->Special[i] != Skill_Earthshake
					&& pItem->Special[i] != Skill_Dark_Horse_Strengthener
					&& pItem->Special[i] != Skill_Earthshake_Mastery
					&& pItem->Special[i] != Skill_Earthshake_Strengthener)
				{
...
void CNewUIItemTooltip::SetTooltipValueText(char* Description, char* information, int NextLine, int* color, int NumText, ITEM* pItem, Script_Item* pItemAtrr, int iItemLevel)
{
	...
	case 21: //-- "Level: %d"
		...
		break;
	case 22: //-- "Increase %d%% of Damage"
		{
			BYTE Damage = 0;
			PET_INFO* pPetInfo = giPetManager::GetPetInfo(pItem);

			if (pPetInfo->m_dwPetType != PET_TYPE_DARK_SPIRIT)
				Damage = ((5 + (CharacterAttribute->Dexterity / 20) + pPetInfo->m_wLevel * 2) & 0xFF);
			else
				Damage = 0;

			sprintf(information, Description, Damage);
		}
		break;
	case 23: //-- "Absorb %d%% additional damage"
		{
			BYTE Damage = 0;
			PET_INFO* pPetInfo = giPetManager::GetPetInfo(pItem);

			if (pPetInfo->m_dwPetType != PET_TYPE_DARK_SPIRIT)
				Damage = (30 + pPetInfo->m_wLevel) / 2;
			else
				Damage = 0;
			sprintf(information, Description, Damage);
		}
