...
protected:
	uint16_t m_wSize;
	uint8_t m_byBuffer[MAX_SPE_BUFFERSIZE];
public:
	__forceinline void Init(uint8_t byType, uint8_t byHeadCode)
	{
		Clear();
		AddData(&byType, sizeof(uint8_t), FALSE);
		switch (byType)
		{
		case 0xC1:
		...
		AddData(&byHeadCode, sizeof(uint8_t), FALSE);
	}
	__forceinline void* End(void)
	{
		...
		case 0xC1:
			...
			break;
		case 0xC2:
			memcpy(&m_byBuffer[1], ((uint8_t*)&m_wSize) + 1, 1);
...
protected:
	...
	{
		uint8_t byXorFilter[SIZE_SPE_XORFILTER] =
		{
...
public:
	__forceinline void AddData(void* pSrc, uint16_t wSize, BOOL bXor = TRUE)
	{
	...
	__forceinline void AddNullData(uint16_t wSize)
	{
		uint8_t byBuffer[MAX_SPE_BUFFERSIZE];
