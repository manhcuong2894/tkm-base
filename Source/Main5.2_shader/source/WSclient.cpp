...
#endif //WINDOWMODE
extern uint32_t TotalPacketSize;
extern int g_iKeyPadEnable;
extern char DebugTextCount;
extern short g_shCameraLevel;
extern uint8_t DebugText[MAX_DEBUG_MAX][256];
...
uint8_t Version[SIZE_PROTOCOLVERSION] = { '1' + 1, '0' + 2, '4' + 3, '0' + 4, '5' + 5 };
uint8_t Serial[SIZE_PROTOCOLSERIAL + 1] = { "TbYehR2hFUPBKgZj" };
...
uint8_t g_byPacketSerialSend = 0;
uint8_t g_byPacketSerialRecv = 0;
BOOL g_bGameServerConnected = FALSE;

uint32_t g_dwLatestMagicTick = 0;
...
uint8_t g_byPacketAfter_EquipmentItem[256];
void ReceiveTradeExit(uint8_t* ReceiveBuffer);
...
uint8_t HeroSoccerTeam = 0;
int  SoccerTime;
char SoccerTeamName[2][8 + 1];
bool SoccerObserver = false;

static  const   uint8_t    NOT_GET_ITEM = 0xff;
static  const   uint8_t    GET_ITEM_ZEN = 0xfe;
static  const   uint8_t    GET_ITEM_MULTI = 0xfd;
extern int ItemKey;
...
void ReceiveServerList(uint8_t* ReceiveBuffer)
{
	...
	uint8_t Value2 = *(ReceiveBuffer + Offset++);
...
void ReceiveServerConnect(uint8_t* ReceiveBuffer)
{
...
void ReceiveServerConnectBusy(uint8_t* ReceiveBuffer)
{
...
void ReceiveServerVersion(uint8_t* ReceiveBuffer)
{
...
void ReceiveServerUpdate(uint8_t* ReceiveBuffer)
{
...
void ReceiveJoinServer(uint8_t* ReceiveBuffer)
{
...
void ReceiveConfirmPassword(uint8_t* ReceiveBuffer)
{
...
void ReceiveConfirmPassword2(uint8_t* ReceiveBuffer)
{
...
void ReceiveChangePassword(uint8_t* ReceiveBuffer)
{
...
void ReceiveCharacterList(uint8_t* ReceiveBuffer)
{
...
void ReceiveCharacterCard_New(uint8_t* ReceiveBuffer)
{
...
void ReceiveCreateCharacter(uint8_t* ReceiveBuffer)
{
...
void ReceiveDeleteCharacter(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_SUBCODE Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
...
BOOL ReceiveLogOut(uint8_t* ReceiveBuffer, BOOL bEncrypted)
{
	LogOut = false;
	LPPHEADER_DEFAULT_SUBCODE Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
...
uint64_t DeserializeUInt64FromBytes(const uint8_t* inBytes)
{
	uint64_t value = 0;
...
BOOL ReceiveJoinMapServer(uint8_t* ReceiveBuffer, BOOL bEncrypted)
{
	LPPRECEIVE_JOIN_MAP_SERVER Data = (LPPRECEIVE_JOIN_MAP_SERVER)ReceiveBuffer;

	MouseLButton = false;

	CharacterAttribute->Experience = 0;
	CharacterAttribute->BaseExperience = 0;
	CharacterAttribute->NextExperience = 0;
	Master_Level_Data.lMasterLevel_Experince = 0;
	Master_Level_Data.lBase_MasterLevel_Experince = 0;
	Master_Level_Data.lNext_MasterLevel_Experince = 0;

	if (CharacterAttribute->CheckMasterLevel())
	{
		Master_Level_Data.lMasterLevel_Experince = DeserializeUInt64FromBytes(Data->btMExp);
		Master_Level_Data.lBase_MasterLevel_Experince = DeserializeUInt64FromBytes(Data->btMBaseExp);
		Master_Level_Data.lNext_MasterLevel_Experince = DeserializeUInt64FromBytes(Data->btMNextExp);
	}
	else
	{
		CharacterAttribute->Experience = (DWORD)DeserializeUInt64FromBytes(Data->btMExp);
		CharacterAttribute->BaseExperience = (DWORD)DeserializeUInt64FromBytes(Data->btMBaseExp);
		CharacterAttribute->NextExperience = (DWORD)DeserializeUInt64FromBytes(Data->btMNextExp);
	}
...
void ReceiveRevival(uint8_t* ReceiveBuffer)
{
	MouseLButton = false;
	Teleport = false;
	...
	{
		o = &pCharacter->Object;
		uint8_t BackUpGuildStatus = pCharacter->GuildStatus;
		uint8_t BackUpGuildType = pCharacter->GuildType;
		uint8_t BackUpGuildRelationShip = pCharacter->GuildRelationShip;
		uint8_t byBackupEtcPart = pCharacter->EtcPart;
...
void ReceiveMagicList(uint8_t* ReceiveBuffer)
{
	...
	for (int i = 0; i < MAX_SKILLS; i++)
	{
		...
		if (SkillType != 0)
		{
			...
			uint8_t SkillUseType = SkillAttribute[SkillType].SkillUseType;
...
BOOL ReceiveInventory(uint8_t* ReceiveBuffer, BOOL bEncrypted)
{
	for (int i = 0; i < MAX_NEW_EQUIPMENT; i++)
	{
		CharacterMachine->Equipment[i].Type = -1;
	...
	if (bEncrypted)
	{
		...
		for (int i = 0; i < Data->Value; i++)
		{
			...
			{
				if (target_index < MAX_EQUIPMENT_INDEX || target_index >= INVENTORY_SHOP_INDEX)
				{
					if (target_index >= INVENTORY_SHOP_INDEX && target_index < INVENTORY_SHOP_MAX)
					{
						g_pMyShopInventory->InsertItem(target_index - INVENTORY_SHOP_INDEX, Data2->Item);
					}
				}
				else
				{
...
void ReceiveDeleteInventory(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_SUBCODE Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
	if (Data->SubCode != 0xff)
	{
		...
		{
			if (itemindex < MAX_EQUIPMENT_INDEX || itemindex >= INVENTORY_SHOP_INDEX)
			{
				if (itemindex >= INVENTORY_SHOP_INDEX && itemindex < INVENTORY_SHOP_MAX)
				{
					g_pMyShopInventory->DeleteItem(itemindex - INVENTORY_SHOP_INDEX);
				}
			}
			else
			{
...
void ReceiveTradeInventory(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_SUBCODE_WORD Data = (LPPHEADER_DEFAULT_SUBCODE_WORD)ReceiveBuffer;
	int Offset = sizeof(PHEADER_DEFAULT_SUBCODE_WORD);
...
void ReceiveChat(uint8_t* ReceiveBuffer)
{
	if (SceneFlag == LOG_IN_SCENE)
	{
	...
	else
	{
		...
		memcpy(ID, Data->Name, MAX_ID_SIZE);
		ID[MAX_ID_SIZE] = 0;
		...
		else
		{
			...
			{
#ifdef WISSPER_CHAT_BLOCKED
				if (g_pChatInputBox->CheckBlocked(ID) == false)
				{
					gmHeadChat->AssignChat(ID, Text);
				...
				gmHeadChat->AssignChat(ID, Text);
				g_pChatListBox->AddText(ID, Text, SEASON3B::TYPE_CHAT_MESSAGE);
#endif // WISSPER_CHAT_BLOCKED
			}
		}
	}
}

void ReceiveChatWhisper(uint8_t* ReceiveBuffer)
{
	...
	memcpy(ID, (char*)Data->Name, MAX_ID_SIZE);
...
void ReceiveChatWhisperResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveChatKey(uint8_t* ReceiveBuffer)
{
...
void ReceiveNotice(uint8_t* ReceiveBuffer)
{
...
void ReceiveMoveCharacter(uint8_t* ReceiveBuffer)
{
...
void ReceiveMovePosition(uint8_t* ReceiveBuffer)
{
...
BOOL ReceiveTeleport(uint8_t* ReceiveBuffer, BOOL bEncrypted)
{
	SEASON3B::CNewUIInventoryCtrl::BackupPickedItem();
...
void ReceiveEquipment(uint8_t* ReceiveBuffer)
{
...
void ReceiveChangePlayer(uint8_t* ReceiveBuffer)
{
	...
	if (pCharacter)
	{
		...
		uint8_t Level = Data->Item[1] & 0xF;
		uint8_t Option = Data->Item[3] & 0x3F;
		uint8_t ExtOption = Data->Item[4];
...
void ReceiveCreatePlayerViewport(uint8_t* ReceiveBuffer, int Size)
{
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		if (!FindText(Temp, "webzen", false))
		{
			...
			uint8_t BackUpGuildStatus = 0;
			uint8_t BackUpGuildType = 0;
			uint8_t BackUpGuildRelationShip = 0;
			uint8_t BackUpGuildMasterKillCount = 0;
			uint8_t EtcPart = 0;
			uint8_t BackupCtlcode = 0;
		...
		Offset += (sizeof(PCREATE_CHARACTER) - (sizeof(uint8_t) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
	}
...
void ReceiveCreateTransformViewport(uint8_t* ReceiveBuffer)
{
	LPPWHEADER_DEFAULT_WORD Data = (LPPWHEADER_DEFAULT_WORD)ReceiveBuffer;
	int Offset = sizeof(PWHEADER_DEFAULT_WORD);
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		uint8_t byBackUpGuildStatus = 0;
		uint8_t byBackUpGuildType = 0;
		uint8_t byBackUpGuildRelationShip = 0;
		uint8_t byBackUpGuildMasterKillCount = 0;
		uint8_t byEtcPart = 0;
		uint8_t byBackupCtlcode = 0;
		...
		Offset += (sizeof(PCREATE_TRANSFORM) - (sizeof(uint8_t) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
	}
...
void ReceiveCreateMonsterViewport(uint8_t* ReceiveBuffer)
{
	LPPWHEADER_DEFAULT_WORD Data = (LPPWHEADER_DEFAULT_WORD)ReceiveBuffer;
	int Offset = sizeof(PWHEADER_DEFAULT_WORD);
	for (int i = 0; i < Data->Value; i++)
	{
		...
		uint8_t bMyMob = (Data2->TypeH) & 0x80;
		uint8_t byBuildTime = (Data2->TypeH & 0x70) >> 4;
		...
		Offset += (sizeof(PCREATE_MONSTER) - (sizeof(uint8_t) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
	}
}

void ReceiveCreateSummonViewport(uint8_t* ReceiveBuffer)
{
	LPPWHEADER_DEFAULT_WORD Data = (LPPWHEADER_DEFAULT_WORD)ReceiveBuffer;
	int Offset = sizeof(PWHEADER_DEFAULT_WORD);
	...
	for (int i = 0; i < Data->Value; i++)
	{
		...
		Offset += (sizeof(PCREATE_SUMMON) - (sizeof(uint8_t) * (MAX_BUFF_SLOT_INDEX - Data2->s_BuffCount)));
	}

	g_ConsoleDebug->Write(MCD_RECEIVE, "0x1F [ReceiveCreateSummonViewport(%d)]", Data->Value);
}

void ReceiveDeleteCharacterViewport(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
...
void ReceiveDamage(uint8_t* ReceiveBuffer)
{
...
void ReceiveAttackDamage(uint8_t* ReceiveBuffer)
{
...
void ReceiveAction(uint8_t* ReceiveBuffer, int Size)
{
...
void ReceiveSkillStatus(uint8_t* ReceiveBuffer)
{
...
void ReceiveMagicFinish(uint8_t* ReceiveBuffer)
{
	...
	if (so->Type == MODEL_PLAYER)
	{
		if (gCharacterManager.GetBaseClass(c->Class) == CLASS_ELF && !c->SafeZone)
		{
			...
			if (EquipedBowType == BOWTYPE_BOW)
			{
				...
				{
					SetAction(so, PLAYER_ATTACK_RIDE_BOW, true);
				}
				...
				{
					if (c->Wing.Type != -1)
						SetAction(so, PLAYER_ATTACK_FLY_BOW);
					else
						SetAction(so, PLAYER_ATTACK_BOW);
				}
			}
			else if (EquipedBowType == BOWTYPE_CROSSBOW)
			{
				...
				{
					SetAction(so, PLAYER_ATTACK_RIDE_CROSSBOW, true);
				}
				else if ((sc_type_helper == MODEL_HELPER + 3 || so->movementType == Movement::Dinorant) && !c->SafeZone)
				{
					SetAction(so, PLAYER_ATTACK_RIDE_CROSSBOW, true);
				...
				{
					if (c->Wing.Type != -1)
						SetAction(so, PLAYER_ATTACK_FLY_CROSSBOW);
					else
						SetAction(so, PLAYER_ATTACK_CROSSBOW);
				}
			}
		}
	}
...
void SetPlayerHighBow(CHARACTER* sc)
{
	...
	if (EquipedBowType == BOWTYPE_BOW)
	{
		...
		{
			SetAction(so, PLAYER_ATTACK_RIDE_BOW_UP, true);
		}
		...
		{
			if (sc->Wing.Type != -1)
				SetAction(so, PLAYER_ATTACK_FLY_BOW_UP);
			else
				SetAction(so, PLAYER_ATTACK_BOW_UP);
		}
	}
	else if (EquipedBowType == BOWTYPE_CROSSBOW)
	{
		if ((sc_type_helper == MODEL_HELPER + 37 || so->movementType == Movement::Fenrir) && !sc->SafeZone)
		{
			SetAction(so, PLAYER_ATTACK_RIDE_CROSSBOW_UP, true);
		}
		else if ((sc_type_helper == MODEL_HELPER + 2 || so->movementType == Movement::Uniria) && !sc->SafeZone)
		{
			SetAction(so, PLAYER_ATTACK_RIDE_CROSSBOW_UP, true);
		...
		{
			if (sc->Wing.Type != -1)
				SetAction(so, PLAYER_ATTACK_FLY_CROSSBOW_UP);
			else
				SetAction(so, PLAYER_ATTACK_CROSSBOW_UP);
		}
	}
}

BOOL ReceiveMonsterSkill(uint8_t* ReceiveBuffer, int Size, BOOL bEncrypted)
{
...
BOOL ReceiveMagic(uint8_t* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	if (bEncrypted)
	{
			...
			if (sc && tc)
			{
				...
				if ((signed int)MagicNumber > Skill_Killing_Blow)
				{
					if ((signed int)MagicNumber > Skill_Fire_Burst_Strengthener)
					{
						...
						LABEL_71:
							...
							{
								SetAction(so, PLAYER_ATTACK_RIDE_STRIKE, true);
							}
							...
							{
								SetAction(so, PLAYER_ATTACK_STRIKE, true);
							}
							sc->AttackTime = 1;
						...
						LABEL_247:
							...
							{
								SetAction(so, PLAYER_ATTACK_RIDE_ATTACK_MAGIC, true);
							}
							else if (sc_type_helper == MODEL_HELPER + 3 || so->movementType == Movement::Dinorant)
							{
								SetAction(so, PLAYER_ATTACK_RIDE_ATTACK_MAGIC, true);
						...
						LABEL_343:
							if (sc == Hero)
							{
								...
								{
									SendRequestAction(AT_RAGEBUFF_1, ((uint8_t)((so->Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
								}
								else
								{
									SendRequestAction(AT_RAGEBUFF_2, ((uint8_t)((so->Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
								}
							}
							{
								OBJECT* _pObj = to;
					...
					else
					{
						...
						LABEL_89:
							SetAction(so, PLAYER_ATTACK_SKILL_SWORD1, 1);
							sc->AttackTime = 1;
							PlayBuffer(SOUND_SKILL_SWORD1, 0, 0);
							break;
						case Skill_Lunge_Strengthener:
						LABEL_90:
							SetAction(so, PLAYER_ATTACK_SKILL_SWORD2, 1);
							sc->AttackTime = 1;
						...
						LABEL_92:
							SetAction(so, PLAYER_ATTACK_SKILL_SWORD4, 1);
							sc->AttackTime = 1;
						...
						LABEL_93:
							if (sc->SwordCount % 2)
								SetAction(so, PLAYER_ATTACK_TWO_HAND_SWORD3, 1);
							else
								SetAction(so, PLAYER_ATTACK_SKILL_SWORD5, 1);
							++sc->SwordCount;
							sc->AttackTime = 1;
							PlayBuffer(SOUND_SKILL_SWORD4, 0, 0);
							break;
						...
						LABEL_97:
							SetAction(so, PLAYER_ATTACK_TWO_HAND_SWORD_TWO, 1);
							sc->AttackTime = 1;
							PlayBuffer(SOUND_SKILL_SWORD4, 0, 0);
							break;
						case Skill_Death_Stab_Strengthener:
						LABEL_102:
							SetAction(so, PLAYER_ATTACK_ONETOONE, true);
						...
						case Skill_Dragon_Slasher:
							goto LABEL_378;
						case Skill_Charge:
							SetAction(so, PLAYER_ATTACK_RUSH, 1);
							sc->AttackTime = 1;
							PlayBuffer(SOUND_SKILL_SWORD2, 0, 0);
							break;
				...
				else
				{
					...
					case Skill_Lunge:
						goto LABEL_90;
					case Skill_Uppercut:
						SetAction(so, PLAYER_ATTACK_SKILL_SWORD3, 1);
						sc->AttackTime = 1;
					...
					case Skill_Impale:
						if (sc_type_helper == MODEL_HELPER + 37 || so->movementType == Movement::Fenrir)
							...
							SetAction(so, PLAYER_ATTACK_SKILL_SPEAR, true);
						sc->AttackTime = 1;
						break;
					...
					case Skill_Mana_Rays:
						SetAction(so, PLAYER_ATTACK_DEATH_CANNON, 1);
						so->AnimationFrame = 0.0;
						sc->AttackTime = 1;
						break;
					case Skill_Starfall:
					...
					case Skill_Spiral_Slash:
						SetAction(so, PLAYER_ATTACK_ONE_FLASH, 1);
						sc->AttackTime = 1;
						PlayBuffer(SOUND_SKILL_SWORD2, 0, 0);
						break;
					case Skill_Stun:
					...
					case Skill_Cancel_Stun:
						...
						{
							SetAction(so, PLAYER_ATTACK_REMOVAL, true);
						}
						if (sc->SkillSuccess)
					...
					case Skill_Summon:
						...
						{
							SetAction(so, PLAYER_ATTACK_RIDE_TELEPORT, true);
						}
						else if ((sc_type_helper == MODEL_HELPER + 3 || so->movementType == Movement::Dinorant) && !sc->SafeZone)
						{
							SetAction(so, PLAYER_ATTACK_RIDE_TELEPORT, true);
						...
						{
							SetAction(so, PLAYER_ATTACK_TELEPORT, true);
						}
...
BOOL ReceiveMagicContinue(uint8_t* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	if (bEncrypted)
	{
...
// ChainLightning
void ReceiveChainMagic(uint8_t* ReceiveBuffer)
{
...
void ReceiveMagicPosition(uint8_t* ReceiveBuffer, int Size)
{
...
void ReceiveSkillCount(uint8_t* ReceiveBuffer)
{
...
BOOL ReceiveDieExp(uint8_t* ReceiveBuffer, BOOL bEncrypted)
{
...
BOOL ReceiveDieExpLarge(uint8_t* ReceiveBuffer, BOOL bEncrypted)
{
...
{
	uint8_t positionX = (uint8_t)(o->Position[0] / TERRAIN_SCALE);
	uint8_t positionY = (uint8_t)(o->Position[1] / TERRAIN_SCALE);
...
void ReceiveDie(uint8_t* ReceiveBuffer, int Size)
{
...
void ReceiveCreateItemViewport(uint8_t* ReceiveBuffer)
{
	LPPWHEADER_DEFAULT_WORD Data = (LPPWHEADER_DEFAULT_WORD)ReceiveBuffer;
	int Offset = sizeof(PWHEADER_DEFAULT_WORD);
...
void ReceiveDeleteItemViewport(uint8_t* ReceiveBuffer)
{
	LPPWHEADER_DEFAULT_WORD Data = (LPPWHEADER_DEFAULT_WORD)ReceiveBuffer;
	int Offset = sizeof(PWHEADER_DEFAULT_WORD);
	for (int i = 0; i < Data->Value; i++)
...
void ReceiveGetItem(uint8_t* ReceiveBuffer)
{
	...
	if (Data->Result != NOT_GET_ITEM)
	{
		...
		else
		{
			...
			{
				if (Data->Result >= MAX_EQUIPMENT_INDEX && Data->Result < INVENTORY_SHOP_INDEX)
...
void ReceiveDropItem(uint8_t* ReceiveBuffer)
{
	...
	if (Data->KeyH)
	{
		...
		else if (Data->KeyL < INVENTORY_SHOP_INDEX)
		{
...
BOOL ReceiveEquipmentItem(uint8_t* ReceiveBuffer, BOOL bEncrypted)
{
	...
	if (bEncrypted)
	{
		...
		if (Data->SubCode == 0xFF)
		{
			...
			if (g_pMixInvenExpansion->IsItemAutoMove())
			{
				g_pMixInvenExpansion->ProcessItemAutoMoveFailure();
			}
		...
		else
		{
			...
			if (pPickedItem)
			{
				if (g_pMyInventory->IsInvenctrlSame(pPickedItem->GetOwnerInventory()) == -1)
				{
					...
					{
						iSourceIndex += INVENTORY_SHOP_INDEX;
					}
				}
				else
				{
			...
			if (iSourceIndex >= INVENTORY_SHOP_INDEX && iSourceIndex < INVENTORY_FULL_SIZE)
			{
				...
				if (GetPersonalItemPrice(iSourceIndex, price, g_IsPurchaseShop))
				{
					...
					if (Data->Index >= INVENTORY_SHOP_INDEX)
					{
			...
			if (g_pMixInvenExpansion->IsItemAutoMove())
			{
				if (Data->SubCode == 0)
				{
					g_pMixInvenExpansion->ProcessItemAutoMoveSuccess();
				}
			}
			...
			if (Data->SubCode)
			{
				...
				else if (Data->SubCode == 23) //-- MixExpansion
				{
					...
					g_pMixInvenExpansion->InsertItem(Data->Index, Data->Item);
				}
			}
			else
			{
				...
				else
				{
					if (itemindex < MAX_EQUIPMENT_INDEX || itemindex >= INVENTORY_SHOP_INDEX)
					{
						if (itemindex >= INVENTORY_SHOP_INDEX && itemindex < INVENTORY_SHOP_MAX)
						{
							g_pMyShopInventory->InsertItem(itemindex - INVENTORY_SHOP_INDEX, Data->Item);
						}
					}
					else
					{
...
void ReceiveModifyItem(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_SUBCODE_ITEM Data = (LPPHEADER_DEFAULT_SUBCODE_ITEM)ReceiveBuffer;
...
void ReceivePostChatItem(uint8_t* ReceiveBuffer)
{
...
BOOL ReceiveTalk(uint8_t* ReceiveBuffer, BOOL bEncrypted)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	...
	case 0x14:
	{
		...
		uint8_t* cursedtempleenterinfo = (&Data->Value) + 1;
	...
	case 0x23:
	{
		...
		uint8_t* pbtRemainTime = (&Data->Value) + 1;
...
void ReceiveBuy(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_ITEM Data = (LPPHEADER_DEFAULT_ITEM)ReceiveBuffer;
	if (Data->Index != 255)
	{
		if (Data->Index >= MAX_EQUIPMENT_INDEX && Data->Index < INVENTORY_SHOP_INDEX)
		{
...
void ReceiveSell(uint8_t* ReceiveBuffer)
{
...
void ReceiveRepair(uint8_t* ReceiveBuffer)
{
...
void ReceiveLevelUp(uint8_t* ReceiveBuffer)
{
	LPPRECEIVE_LEVEL_UP Data = (LPPRECEIVE_LEVEL_UP)ReceiveBuffer;
	CharacterAttribute->Level = Data->Level;

	CharacterAttribute->LevelUpPoint = Data->LevelPoint;
	CharacterAttribute->Life = Data->LifeMax;
	CharacterAttribute->LifeMax = Data->LifeMax;

	CharacterAttribute->Mana = Data->ManaMax;
	CharacterAttribute->ManaMax = Data->ManaMax;

	CharacterAttribute->Shield = Data->ShieldMax;
	CharacterAttribute->ShieldMax = Data->ShieldMax;
	...
	CharacterAttribute->AddPoint = Data->AddPoint;
	CharacterAttribute->MaxAddPoint = Data->MaxAddPoint;
	CharacterAttribute->wMinusPoint = Data->MinusPoint;
	CharacterAttribute->wMaxMinusPoint = Data->MaxMinusPoint;

	unicode::t_char szText[256] = { 0, };

	DWORD iExp = CharacterAttribute->NextExperience - CharacterAttribute->Experience;
	...
	CharacterAttribute->BaseExperience = Data->BaseExperience;
	...
	if (gmProtect->HelperActiveAlert && CharacterAttribute->Level == gmProtect->HelperActiveLevel)
	{
...
void ReceiveAddPoint(uint8_t* ReceiveBuffer)
{
...
void ReceiveLife(uint8_t* ReceiveBuffer)
{
	...
	{
	case 0xff:
		CharacterAttribute->Life = Data->curAtt1;
		CharacterAttribute->Shield = Data->curAtt2;
		break;
	case 0xfe:
		CharacterAttribute->LifeMax = Data->curAtt1;
		CharacterAttribute->ShieldMax = Data->curAtt2;
		break;
	case 0xfd:
		EnableUse = 0;
		break;
	default:
	{
		int itemindex = Data->type - MAX_EQUIPMENT_INDEX;
...
void ReceiveMana(uint8_t* ReceiveBuffer)
{
	...
	{
	case 0xff:
		CharacterAttribute->Mana = Data->curAtt1;
		CharacterAttribute->SkillMana = Data->curAtt2;
		break;
	case 0xfe:
		CharacterAttribute->ManaMax = Data->curAtt1;
		CharacterAttribute->SkillManaMax = Data->curAtt2;
		break;
	default:
		break;
	}
...
void ReceivePK(uint8_t* ReceiveBuffer)
{
...
void ReceiveDurability(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_VALUE_KEY Data = (LPPHEADER_DEFAULT_VALUE_KEY)ReceiveBuffer;
...
BOOL ReceiveHelperItem(uint8_t* ReceiveBuffer, BOOL bEncrypted)
{
...
void ReceiveWeather(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
...
void ReceiveEvent(uint8_t* ReceiveBuffer)
{
...
void ReceiveSummonLife(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_SUBCODE Data = (LPPHEADER_DEFAULT_SUBCODE)ReceiveBuffer;
	SummonLife = Data->Value;
}

BOOL ReceiveTrade(uint8_t* ReceiveBuffer, BOOL bEncrypted)
{
	LPPCHATING Data = (LPPCHATING)ReceiveBuffer;
	g_pTrade->ProcessToReceiveTradeRequest(Data->Name);
	return (TRUE);
}

void ReceiveTradeResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveTradeYourInventoryDelete(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	g_pTrade->ProcessToReceiveYourItemDelete(Data->Value);
}

void ReceiveTradeYourInventory(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_ITEM Data = (LPPHEADER_DEFAULT_ITEM)ReceiveBuffer;
	g_pTrade->ProcessToReceiveYourItemAdd(Data->Index, Data->Item);
}

void ReceiveTradeMyGold(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	g_pTrade->ProcessToReceiveMyTradeGold(Data->Value);
}

void ReceiveTradeYourGold(uint8_t* ReceiveBuffer)
{
...
void ReceiveTradeYourResult(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
...
void ReceivePing(uint8_t* ReceiveBuffer)
{
	gsteady_clock->runtime_recv_ping(*(int*)(ReceiveBuffer + 4));
}

void ReceiveStorageGold(uint8_t* ReceiveBuffer)
{
...
void ReceiveStorageExit(uint8_t* ReceiveBuffer)
{
	g_ConsoleDebug->Write(MCD_RECEIVE, "0x82 [ReceiveStorageExit]");
}

void ReceiveStorageStatus(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
...
void ReceiveParty(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_KEY Data = (LPPHEADER_DEFAULT_KEY)ReceiveBuffer;
...
void ReceivePartyResult(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	switch (Data->Value)
	{
...
void ReceivePartyList(uint8_t* ReceiveBuffer)
{
...
void ReceivePartyInfo(uint8_t* ReceiveBuffer)
{
...
void ReceivePartyLeave(uint8_t* ReceiveBuffer)
{
	PartyNumber = 0;
	g_pChatListBox->AddText("", GlobalText[502], SEASON3B::TYPE_ERROR_MESSAGE);
...
void ReceivePartyGetItem(uint8_t* ReceiveBuffer)
{
	...
	if (c && Hero != c)
	{
		...
		if ((Data->ItemInfo & 0x10000))
			sprintf(itemName, "%s ", GlobalText[620]);
		else if ((Data->ItemInfo & 0x20000))
			sprintf(itemName, "%s ", GlobalText[1089]);
		...
		if ((Data->ItemInfo & 0x02000))
			strcat(itemName, GlobalText[176]);

		if ((Data->ItemInfo & 0x08000))
			strcat(itemName, GlobalText[177]);

		if ((Data->ItemInfo & 0x04000))
			strcat(itemName, GlobalText[178]);
...
void ReceiveGuild(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_KEY Data = (LPPHEADER_DEFAULT_KEY)ReceiveBuffer;
...
void ReceiveGuildResult(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	switch (Data->Value)
	{
	case 0:
		g_pChatListBox->AddText("", GlobalText[503], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 1:
		g_pChatListBox->AddText("", GlobalText[504], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 2:
		g_pChatListBox->AddText("", GlobalText[505], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 3:
		g_pChatListBox->AddText("", GlobalText[506], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 4:
		g_pChatListBox->AddText("", GlobalText[507], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 5:
		g_pChatListBox->AddText("", GlobalText[508], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 6:
		g_pChatListBox->AddText("", GlobalText[509], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 7:
		g_pChatListBox->AddText("", GlobalText[510], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 0xA1:
		g_pChatListBox->AddText("", GlobalText[2992], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 0xA2:
		g_pChatListBox->AddText("", GlobalText[2995], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 0xA3:
		g_pChatListBox->AddText("", GlobalText[2996], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	}
}

void ReceiveGuildList(uint8_t* ReceiveBuffer)
{
...
void ReceiveGuildLeave(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	switch (Data->Value)
	{
...
void ReceiveCreateGuildInterface(uint8_t* ReceiveBuffer)
{
	g_pNewUISystem->Show(SEASON3B::INTERFACE_NPCGUILDMASTER);
}

void ReceiveCreateGuildMasterInterface(uint8_t* ReceiveBuffer)
{
}

void ReceiveDeleteGuildViewport(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_KEY Data = (LPPHEADER_DEFAULT_KEY)ReceiveBuffer;
...
void ReceiveCreateGuildResult(uint8_t* ReceiveBuffer)
{
	...
	{
	case 0:
		g_pChatListBox->AddText("", GlobalText[516], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 2:
		g_pChatListBox->AddText("", GlobalText[517], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 3:
		g_pChatListBox->AddText("", GlobalText[518], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 4:
		g_pChatListBox->AddText("", GlobalText[940], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 5:
		g_pChatListBox->AddText("", GlobalText[941], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 6:
		g_pChatListBox->AddText("", GlobalText[942], SEASON3B::TYPE_ERROR_MESSAGE);
		break;
	case 1:
...
void ReceiveDeclareWar(uint8_t* ReceiveBuffer)
{
...
void ReceiveDeclareWarResult(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	switch (Data->Value)
	{
...
void ReceiveGuildBeginWar(uint8_t* ReceiveBuffer)
{
	LPPRECEIVE_WAR Data = (LPPRECEIVE_WAR)ReceiveBuffer;
	EnableGuildWar = true;
	...
	SendRequestAction(AT_RUSH1, ((uint8_t)((Hero->Object.Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
...
void ReceiveGuildEndWar(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	char Text[100];
	int Win = 0;
	...
	case 2:
		...
		SendRequestAction(AT_WIN1, ((uint8_t)((Hero->Object.Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
		break;
	case 0:
		SetActionClass(Hero, &Hero->Object, PLAYER_CRY1, AT_CRY1);
		SendRequestAction(AT_CRY1, ((uint8_t)((Hero->Object.Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
		break;
	}
...
void ReceiveGuildWarScore(uint8_t* ReceiveBuffer)
{
...
void ReceiveGuildIDViewport(uint8_t* ReceiveBuffer)
{
	LPPWHEADER_DEFAULT_WORD Data = (LPPWHEADER_DEFAULT_WORD)ReceiveBuffer;
	int Offset = sizeof(PWHEADER_DEFAULT_WORD);
	for (int i = 0; i < Data->Value; ++i)
...
void ReceiveGuildInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveGuildAssign(uint8_t* ReceiveBuffer)
{
...
void ReceiveGuildRelationShip(uint8_t* ReceiveBuffer)
{
...
void ReceiveGuildRelationShipResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveBanUnionGuildResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveUnionViewportNotify(uint8_t* ReceiveBuffer)
{
...
void ReceiveUnionList(uint8_t* ReceiveBuffer)
{
...
void ReceiveSoccerTime(uint8_t* ReceiveBuffer)
{
	LPPRECEIVE_SOCCER_TIME Data = (LPPRECEIVE_SOCCER_TIME)ReceiveBuffer;
	SoccerTime = Data->Time;
}

void ReceiveSoccerScore(uint8_t* ReceiveBuffer)
{
...
void ReceiveSoccerGoal(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	char Text[100];
...
void Receive_Master_Level_Exp(uint8_t* ReceiveBuffer)
{
...
void Receive_Master_LevelUp(uint8_t* ReceiveBuffer)
{
	LPPMSG_MASTERLEVEL_UP Data = (LPPMSG_MASTERLEVEL_UP)ReceiveBuffer;
	Master_Level_Data.nMLevel = Data->Level;
	//Master_Level_Data.nAddMPoint = Data->nAddMPoint;
	Master_Level_Data.nMLevelUpMPoint = Data->LevelUpPoint;
...
void Receive_Master_LevelGetSkill(uint8_t* ReceiveBuffer)
{
#if MASTER_SKILL_TREE == 1
...
void Receive_Master_LevelSkillTree(uint8_t* ReceiveBuffer)
{
#if MASTER_SKILL_TREE == 1
...
BOOL ReceiveNewJoinMapServer(uint8_t* ReceiveBuffer)
{
	...
	CharacterAttribute->AddPoint = Data->MinusPoint;
	CharacterAttribute->MaxAddPoint = Data->MaxMinusPoint;
...
BOOL ReceiveNewCalculate(uint8_t* ReceiveBuffer)
{
...
void ReceiveCharacterCoin(uint8_t* ReceiveBuffer)
{
...
void GCBuyConfirmRecv(uint8_t* ReceiveBuffer) // OK
{
...
void GCCharacterServerInfoRecv(uint8_t* ReceiveBuffer)
{
...
void ReceiveServerCommand(uint8_t* ReceiveBuffer)
{
...
void ReceiveMix(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_ITEM Data = (LPPHEADER_DEFAULT_ITEM)ReceiveBuffer;
...
void ReceiveMixExit(uint8_t* ReceiveBuffer)
{
	g_ConsoleDebug->Write(MCD_RECEIVE, "0x87 [ReceiveMixExit]");
}

void ReceiveGemMixResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveGemUnMixResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveMoveToDevilSquareResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveDevilSquareOpenTime(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	if (0 == Data->Value)
	{
...
void ReceiveDevilSquareCountDown(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
	...
	{
		if (((uint8_t)(Data->Value + 1) >= TYPE_MATCH_DOPPELGANGER_ENTER_CLOSE && (uint8_t)(Data->Value + 1) <= TYPE_MATCH_DOPPELGANGER_CLOSE) || (uint8_t)(Data->Value + 1) == TYPE_MATCH_NONE)
		{
			matchEvent::StartMatchCountDown((uint8_t)(Data->Value + 1));
		}
	}
	else
	{
...
void ReceiveDevilSquareRank(uint8_t* ReceiveBuffer)
{
...
void ReceiveMoveToEventMatchResult(uint8_t* ReceiveBuffer)
{
	g_pNewUISystem->HideAll();
...
void ReceiveEventZoneOpenTime(uint8_t* ReceiveBuffer)
{
...
void ReceiveMoveToEventMatchResult2(uint8_t* ReceiveBuffer)
{
	g_pNewUISystem->HideAll();
...
void ReceiveSetAttribute(uint8_t* ReceiveBuffer)
{
...
void ReceiveMatchGameCommand(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelRequest(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelStart(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelEnd(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelScore(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelHP(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelChannelList(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelWatchRequestReply(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelWatcherJoin(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelWatchEnd(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelWatcherQuit(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelWatcherList(uint8_t* ReceiveBuffer)
{
	g_DuelMgr.RemoveAllDuelWatchUser();
...
void ReceiveDuelResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveDuelRound(uint8_t* ReceiveBuffer)
{
...
void ReceiveCreateShopTitleViewport(uint8_t* ReceiveBuffer)
{
...
void ReceiveShopTitleChange(uint8_t* ReceiveBuffer)
{
...
void ReceiveSetPriceResult(uint8_t* ReceiveBuffer)
{
	...
	if (Header->byResult != 0x01 && g_IsPurchaseShop == PSHOPWNDTYPE_SALE)
	{
		...
		RemovePersonalItemPrice(INVENTORY_SHOP_INDEX + g_pMyShopInventory->GetTargetIndex(), PSHOPWNDTYPE_SALE);
...
void ReceiveCreatePersonalShop(uint8_t* ReceiveBuffer)
{
...
void ReceiveDestroyPersonalShop(uint8_t* ReceiveBuffer)
{
...
void ReceivePersonalShopItemList(uint8_t* ReceiveBuffer)
{
	...
	if (Header->byResult == 0x01)
	{
		...
		for (int i = 0; i < Header->byCount; i++, pShopItem++)
		{
			...
			{
				int itemindex = pShopItem->byPos - (INVENTORY_SHOP_INDEX);
...
void ReceiveRefreshItemList(uint8_t* ReceiveBuffer)
{
	LPGETPSHOPITEMLIST_HEADERINFO Header = (LPGETPSHOPITEMLIST_HEADERINFO)ReceiveBuffer;
...
void ReceivePurchaseItem(uint8_t* ReceiveBuffer)
{
...
void NotifySoldItem(uint8_t* ReceiveBuffer)
{
...
void NotifyClosePersonalShop(uint8_t* ReceiveBuffer)
{
...
void ReceiveDisplayEffectViewport(uint8_t* ReceiveBuffer)
{
...
void ReceiveFriendList(uint8_t* ReceiveBuffer)
{
	g_pWindowMgr->Reset();
...
void ReceiveAddFriendResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveRequestAcceptAddFriend(uint8_t* ReceiveBuffer)
{
...
void ReceiveDeleteFriendResult(uint8_t* ReceiveBuffer)
{
	LPFS_FRIEND_RESULT Data = (LPFS_FRIEND_RESULT)ReceiveBuffer;
	char szName[MAX_ID_SIZE + 1] = { 0 };
	strncpy(szName, (const char*)Data->Name, MAX_ID_SIZE);
...
void ReceiveFriendStateChange(uint8_t* ReceiveBuffer)
{
...
void ReceiveLetterSendResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveLetter(uint8_t* ReceiveBuffer)
{
	//g_pLetterList->ClearLetterList();
...
void ReceiveLetterText(uint8_t* ReceiveBuffer)
{
	...
	{
		uint8_t ServerClass = Data->Equipment[EQUIPMENT_HELPER] & 0x7F;
...
void ReceiveLetterDeleteResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveCreateChatRoomResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveChatRoomInviteResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveChatRoomConnectResult(DWORD dwWindowUIID, uint8_t* ReceiveBuffer)
{
...
void ReceiveChatRoomUserStateChange(DWORD dwWindowUIID, uint8_t* ReceiveBuffer)
{
...
void ReceiveChatRoomUserList(DWORD dwWindowUIID, uint8_t* ReceiveBuffer)
{
...
void ReceiveChatRoomChatText(DWORD dwWindowUIID, uint8_t* ReceiveBuffer)
{
...
void ReceiveChatRoomNoticeText(DWORD dwWindowUIID, uint8_t* ReceiveBuffer)
{
	LPFS_CHAT_TEXT Data = (LPFS_CHAT_TEXT)ReceiveBuffer;
	Data->Msg[99] = '\0';
	if (Data->Msg[0] == '\0')
...
void ReceiveOption(uint8_t* ReceiveBuffer)
{
	...
	uint8_t byQLevel, byWLevel, byELevel, byRLevel;
	...
	uint8_t wChatListBoxSize = (Data->ChatLogBox >> 4) * 3;
	uint8_t wChatListBoxBackAlpha = Data->ChatLogBox & 0x0F;
...
void ReceiveEventChipInfomation(uint8_t* ReceiveBuffer)
{
...
void ReceiveEventChip(uint8_t* ReceiveBuffer)
{
...
void ReceiveBuffState(uint8_t* ReceiveBuffer)
{
...
void ReceiveMutoNumber(uint8_t* ReceiveBuffer)
{
...
void ReceiveServerImmigration(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
...
void ReceiveScratchResult(uint8_t* ReceiveBuffer)
{
...
void ReceivePlaySoundEffect(uint8_t* ReceiveBuffer)
{
...
void ReceiveEventCount(uint8_t* ReceiveBuffer)
{
...
void ReceiveQuestHistory(uint8_t* ReceiveBuffer)
{
...
void ReceiveQuestState(uint8_t* ReceiveBuffer)
{
...
void ReceiveQuestResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveQuestPrize(uint8_t* ReceiveBuffer)
{
	...
	case 201:
	{
		...
		if (c)
		{
			...
			uint8_t byClass = gCharacterManager.ChangeServerClassTypeToClientClassType(Data->m_byNumber);

			if (gCharacterManager.GetStepClass(byClass) != 2)
				break;
	...
	case 204:
	{
		...
		if (pCharacter)
		{
			...
			if (gCharacterManager.GetStepClass(byClass) == 3 || gCharacterManager.GetStepClass(byClass) == 4)
			{
				...
				g_pMasterSkillTree->SetMasterType(Hero->GetCharacterClass());
			}
		}
	}
	break;
...
void ReceiveQuestMonKillInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveQuestLimitResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveQuestByItemUseEP(uint8_t* ReceiveBuffer)
{
...
void ReceiveQuestByEtcEPList(uint8_t* ReceiveBuffer)
{
...
void ReceiveQuestByNPCEPList(uint8_t* ReceiveBuffer)
{
	LPPMSG_NPCTALK_QUESTLIST pData = (LPPMSG_NPCTALK_QUESTLIST)ReceiveBuffer;
...
void ReceiveQuestQSSelSentence(uint8_t* ReceiveBuffer)
{
...
void ReceiveQuestQSRequestReward(uint8_t* ReceiveBuffer)
{
	LPPMSG_NPC_QUESTEXP_INFO pData = (LPPMSG_NPC_QUESTEXP_INFO)ReceiveBuffer;
...
void ReceiveQuestCompleteResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveQuestGiveUp(uint8_t* ReceiveBuffer)
{
...
void ReceiveProgressQuestList(uint8_t* ReceiveBuffer)
{
...
void ReceiveProgressQuestRequestReward(uint8_t* ReceiveBuffer)
{
	LPPMSG_NPC_QUESTEXP_INFO pData = (LPPMSG_NPC_QUESTEXP_INFO)ReceiveBuffer;
	g_QuestMng.SetQuestRequestReward(ReceiveBuffer);
...
void ReceiveProgressQuestListReady(uint8_t* ReceiveBuffer)
{
...
void ReceiveGensJoining(uint8_t* ReceiveBuffer)
{
#if MAIN_UPDATE > 303
...
void ReceiveGensSecession(uint8_t* ReceiveBuffer)
{
#if MAIN_UPDATE > 303
...
void ReceivePlayerGensInfluence(uint8_t* ReceiveBuffer)
{
#if MAIN_UPDATE > 303
...
void ReceiveOtherPlayerGensInfluenceViewport(uint8_t* ReceiveBuffer)
{
#if MAIN_UPDATE > 303
...
void ReceiveNPCDlgUIStart(uint8_t* ReceiveBuffer)
{
...
void ReceiveReward(uint8_t* ReceiveBuffer)
{
#if MAIN_UPDATE > 303
...
void ReceivePetCommand(uint8_t* ReceiveBuffer)
{
...
void ReceivePetAttack(uint8_t* ReceiveBuffer)
{
...
void ReceivePetInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveWTTimeLeft(uint8_t* ReceiveBuffer)
{
...
void ReceiveWTMatchResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveWTBattleSoccerGoalIn(uint8_t* ReceiveBuffer)
{
...
void ReceiveChangeMapServerInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveChangeMapServerResult(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT Data = (LPPHEADER_DEFAULT)ReceiveBuffer;
...
void ReceiveBCStatus(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCReg(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCGiveUp(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCRegInfo(uint8_t* ReceiveBuffer)
{
	...
	case 0x01:
	{
		...
		DWORD dwMarkCount;
		uint8_t* pMarkCount = (uint8_t*)&dwMarkCount;
...
void ReceiveBCRegMark(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCNPCBuy(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCNPCRepair(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCNPCUpgrade(uint8_t* ReceiveBuffer)
{
...
void ReceiveInfoCastleNpc(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCGetTaxInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCChangeTaxRate(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCWithdraw(uint8_t* ReceiveBuffer)
{
...
void ReceiveTaxInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveHuntZoneEnter(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCNPCList(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCDeclareGuildList(uint8_t* ReceiveBuffer)
{
...
void ReceiveBCGuildList(uint8_t* ReceiveBuffer)
{
...
void ReceiveGateState(uint8_t* ReceiveBuffer)
{
...
void ReceiveGateOperator(uint8_t* ReceiveBuffer)
{
...
void ReceiveGateCurrentState(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrownSwitchState(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrownRegist(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrownState(uint8_t* ReceiveBuffer)
{
	LPPRECEIVE_CROWN_STATE pData = (LPPRECEIVE_CROWN_STATE)ReceiveBuffer;
...
void ReceiveBattleCastleRegiment(uint8_t* ReceiveBuffer)
{
	LPPRECEIVE_CROWN_STATE pData = (LPPRECEIVE_CROWN_STATE)ReceiveBuffer;
...
void ReceiveBattleCasleSwitchInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveBattleCastleStart(uint8_t* ReceiveBuffer)
{
	LPPRECEIVE_CROWN_STATE pData = (LPPRECEIVE_CROWN_STATE)ReceiveBuffer;
...
void ReceiveBattleCastleProcess(uint8_t* ReceiveBuffer)
{
...
void ReceiveKillCount(uint8_t* ReceiveBuffer)
{
...
void ReceiveBuildTime(uint8_t* ReceiveBuffer)
{
...
void ReceiveCastleGuildMark(uint8_t* ReceiveBuffer)
{
	LPPRECEIVE_CASTLE_FLAG pData = (LPPRECEIVE_CASTLE_FLAG)ReceiveBuffer;

	uint8_t GuildMark[64];
	for (int i = 0; i < 64; i++)
	{
		if (i % 2 == 0)
...
void ReceiveCastleHuntZoneInfo(uint8_t* RecevieBuffer)
{
...
void ReceiveCastleHuntZoneResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveCatapultState(uint8_t* ReceiveBuffer)
{
...
void ReceiveCatapultFire(uint8_t* ReceiveBuffer)
{
...
void ReceiveCatapultFireToMe(uint8_t* ReceiveBuffer)
{
...
void ReceivePreviewPort(uint8_t* ReceiveBuffer)
{
	LPPWHEADER_DEFAULT_WORD pData = (LPPWHEADER_DEFAULT_WORD)ReceiveBuffer;
	int Offset = sizeof(PWHEADER_DEFAULT_WORD);
	for (int i = 0; i < pData->Value; i++)
	{
		...
		Offset += (sizeof(PRECEIVE_PREVIEW_PORT) - (sizeof(uint8_t) * (MAX_BUFF_SLOT_INDEX - pData2->s_BuffCount)));
	}
}

void ReceiveMapInfoResult(uint8_t* ReceiveBuffer)
{
	LPPRECEIVE_MAP_INFO_RESULT pData = (LPPRECEIVE_MAP_INFO_RESULT)ReceiveBuffer;
}

void ReceiveGuildCommand(uint8_t* ReceiveBuffer)
{
...
void ReceiveGuildMemberLocation(uint8_t* ReceiveBuffer)
{
...
void ReceiveGuildNpcLocation(uint8_t* ReceiveBuffer)
{
	if (g_pSiegeWarfare->GetCurSiegeWarType() != TYPE_GUILD_COMMANDER)
		return;
...
void ReceiveMatchTimer(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrywolfInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrywolStateAltarfInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrywolfAltarContract(uint8_t* ReceiveBuffer)
{
	...
	else if (pData->bResult == 1)
	{
		...
		SendRequestAction(AT_HEALING1, ((uint8_t)((Hero->Object.Angle[2] + 22.5f) / 360.f * 8.f + 1.f) % 8));
	}
}

void ReceiveCrywolfLifeTime(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrywolfTankerHit(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrywolfBenefitPlusChaosRate(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrywolfBossMonsterInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrywolfPersonalRank(uint8_t* ReceiveBuffer)
{
...
void ReceiveCrywolfHeroList(uint8_t* ReceiveBuffer)
{
...
void ReceiveKanturu3rdStateInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveKanturu3rdEnterBossMap(uint8_t* ReceiveBuffer)
{
...
void ReceiveKanturu3rdCurrentState(uint8_t* ReceiveBuffer)
{
...
void ReceiveKanturu3rdState(uint8_t* ReceiveBuffer)
{
...
void ReceiveKanturu3rdResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveKanturu3rdTimer(uint8_t* ReceiveBuffer)
{
...
void RecevieKanturu3rdMayaSKill(uint8_t* ReceiveBuffer)
{
...
void RecevieKanturu3rdLeftUserandMonsterCount(uint8_t* ReceiveBuffer)
{
...
void ReceiveCursedTempleEnterInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveCursedTempleEnterResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveCursedTempleState(uint8_t* ReceiveBuffer)
{
...
void ReceiveRaklionStateInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveRaklionCurrentState(uint8_t* ReceiveBuffer)
{
...
void TranslateChattingProtocol(DWORD dwWindowUIID, int HeadCode, uint8_t* ReceiveBuffer, int Size, BOOL bEcrypted);
...
void ProtocolCompiler(CWsctlc* pSocketClient, int iTranslation, int iParam)
{
	...
	while (1)
	{
		uint8_t* ReceiveBuffer = pSocketClient->GetReadMsg();
		...
		std::vector<uint8_t> dataDec(MAX_SPE_BUFFERSIZE_);

		uint8_t* byDec = dataDec.data();
		...
		else if (ReceiveBuffer[0] == 0xC3 || ReceiveBuffer[0] == 0xC4)
		{
			...
			if (ReceiveBuffer[0] == 0xC3)
			{
				...
				pHeader->Code = 0xC1;
				pHeader->Size = (uint8_t)iSize;
				HeadCode = pHeader->HeadCode;
				ReceiveBuffer = (uint8_t*)pHeader;
			}
			else
			{
				LPPWMSG_HEADER pHeader = (LPPWMSG_HEADER)byDec;
				pHeader->Code = 0xC2;
				pHeader->SizeH = (uint8_t)(iSize / 256);
				pHeader->SizeL = (uint8_t)(iSize % 256);
				HeadCode = pHeader->HeadCode;
				ReceiveBuffer = (uint8_t*)pHeader;
			}
			Size = iSize;
...
bool ReceiveRegistedLuckyCoin(uint8_t* ReceiveBuffer)
{
...
bool ReceiveRegistLuckyCoin(uint8_t* ReceiveBuffer)
{
...
bool ReceiveRequestExChangeLuckyCoin(uint8_t* ReceiveBuffer)
{
...
bool ReceiveEnterDoppelGangerEvent(uint8_t* ReceiveBuffer)
{
...
bool ReceiveDoppelGangerMonsterPosition(uint8_t* ReceiveBuffer)
{
...
bool ReceiveDoppelGangerState(uint8_t* ReceiveBuffer)
{
...
bool ReceiveDoppelGangerIcewalkerState(uint8_t* ReceiveBuffer)
{
...
bool ReceiveDoppelGangerTimePartyState(uint8_t* ReceiveBuffer)
{
...
bool ReceiveDoppelGangerResult(uint8_t* ReceiveBuffer)
{
...
bool ReceiveDoppelGangerMonsterGoal(uint8_t* ReceiveBuffer)
{
...
bool ReceiveMoveMapChecksum(uint8_t* ReceiveBuffer)
{
...
bool ReceiveRequestMoveMap(uint8_t* ReceiveBuffer)
{
...
bool ReceiveEnterEmpireGuardianEvent(uint8_t* ReceiveBuffer)
{
...
bool ReceiveRemainTickEmpireGuardian(uint8_t* ReceiveBuffer)
{
...
bool ReceiveResultEmpireGuardian(uint8_t* ReceiveBuffer)
{
...
// (0xD2)(0x01)
bool ReceiveIGS_CashPoint(uint8_t* pReceiveBuffer)
{
...
// (0xD2)(0x02)
bool ReceiveIGS_ShopOpenResult(uint8_t* pReceiveBuffer)
{
	...
	if ((uint8_t)Data->byShopOpenResult == 0)
	{
		return false;
	}
...
// (0xD2)(0x03)
bool ReceiveIGS_BuyItem(uint8_t* pReceiveBuffer)
{
	LPPMSG_CASHSHOP_BUYITEM_ANS Data = (LPPMSG_CASHSHOP_BUYITEM_ANS)pReceiveBuffer;

	switch ((uint8_t)Data->byResultCode)
	{
	case -2:
	{
...
//----------------------------------------------------------------------------
// (0xD2)(0x04)
bool ReceiveIGS_SendItemGift(uint8_t* pReceiveBuffer)
{
...
// (0xD2)(0x06)
bool ReceiveIGS_StorageItemListCount(uint8_t* pReceiveBuffer)
{
...
// (0xD2)(0x0D)
bool ReceiveIGS_StorageItemList(uint8_t* pReceiveBuffer)
{
...
// (0xD2)(0x0E)
bool ReceiveIGS_StorageGiftItemList(uint8_t* pReceiveBuffer)
{
...
//----------------------------------------------------------------------------
// (0xD2)(0x07)
bool ReceiveIGS_SendCashGift(uint8_t* pReceiveBuffer)
{
...
//----------------------------------------------------------------------------
// (0xD2)(0x08)
bool ReceiveIGS_PossibleBuy(uint8_t* pReceiveBuffer)
{
...
// (0xD2)(0x09)
bool ReceiveIGS_LeftCountItem(uint8_t* pReceiveBuffer)
{
...
// (0xD2)(0x0C)
bool ReceiveIGS_UpdateScript(uint8_t* pReceiveBuffer)
{
...
// (0xD2)(0x13)
bool ReceiveIGS_EventItemlistCnt(uint8_t* pReceiveBuffer)
{
...
//(0xD2)(0x14)
bool ReceiveIGS_EventItemlist(uint8_t* pReceiveBuffer)
{
...
// (0xD2)(0x15)
bool ReceiveIGS_UpdateBanner(uint8_t* pReceiveBuffer)
{
...
#ifdef PBG_ADD_SECRETBUFF
bool ReceiveFatigueTime(uint8_t* pReceiveBuffer)
{
...
bool ReceiveEquippingInventoryItem(uint8_t* pReceiveBuffer)
{
...
bool ReceivePeriodItemListCount(uint8_t* pReceiveBuffer)
{
	LPPMSG_PERIODITEMEX_ITEMCOUNT Data = (LPPMSG_PERIODITEMEX_ITEMCOUNT)pReceiveBuffer;
	return true;
}

bool ReceivePeriodItemList(uint8_t* pReceiveBuffer)
{
...
#ifdef PBG_ADD_NEWCHAR_MONK_SKILL
BOOL ReceiveStraightAttack(uint8_t* ReceiveBuffer, int Size, BOOL bEncrypted)
{
...
void ReceiveDarkside(uint8_t* ReceiveBuffer)
{
...
void ReceiveHelperDataRecv(uint8_t* ReceiveBuffer)
{
...
void ReceiveHelperResult(uint8_t* ReceiveBuffer)
{
...
void ReceiveInventoryHoly(uint8_t* ReceiveBuffer)
{
	LPPHEADER_DEFAULT_SUBCODE_WORD Data = (LPPHEADER_DEFAULT_SUBCODE_WORD)ReceiveBuffer;
	...
	{
		uint8_t* btData = (ReceiveBuffer + offset);
...
void ReceiveCharacterLifeInfo(uint8_t* ReceiveBuffer)
{
...
void ReceiveCharacterBaseInfo(uint8_t* ReceiveBuffer)
{
	LPPMSG_CHARACTER_BASE_INFO Data = (LPPMSG_CHARACTER_BASE_INFO)ReceiveBuffer;

	CharacterAttribute->MagicSpeed = Data->MagicSpeed;
	CharacterAttribute->AttackSpeed = Data->AttackSpeed;
}

void UpdateInventoryExtend(uint8_t* ReceiveBuffer)
{
...
void settings_reset_manager(uint8_t* ReceiveBuffer, uint8_t type)
{
	if (type == 0)
	{
...
void settings_invasion_kill(uint8_t* ReceiveBuffer)
{
...
void settings_invasion_manager(uint8_t* ReceiveBuffer)
{
...
BOOL TranslateProtocol(int HeadCode, uint8_t* ReceiveBuffer, int Size, BOOL bEncrypted)
{
	switch (HeadCode)
	{
	case 0xF1:
	...
	case 0x4F:
	{
		...
		case 0x01:
			...
			break;
		case 0x02:
			if (g_pMixInvenExpansion)
				g_pMixInvenExpansion->ReceiveMixOptions(ReceiveBuffer);
			break;
		case 0x03:
			//g_pMixInvenExpansion->GetRecipeAdvice(ReceiveBuffer);
			break;
		case 0x04:
			if (g_pMixInvenExpansion)
				g_pMixInvenExpansion->ReceiveMixRecipients(ReceiveBuffer);
			break;
		case 0x05:
			if (g_pMixInvenExpansion)
				g_pMixInvenExpansion->ReceiveInventory(ReceiveBuffer);
			break;
		}
	}
	break;
	...
	case 0xDE:
	{
		...
		if (ReceiveBuffer[0] == 0xC1)
		{
			subcode = *(uint8_t*)(ReceiveBuffer + 3);
		}
		else
		{
			subcode = *(uint8_t*)(ReceiveBuffer + 4);
		}
