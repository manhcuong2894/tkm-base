...
float MoveHumming(vec3_t Position, vec3_t Angle, vec3_t TargetPosition, float Turn, bool Fvg)
{
	...
	float turn = (Fvg) ? AdvanceByDelta(Turn) : Turn;
...
bool PathFinding2(int sx, int sy, int tx, int ty, PATH_t* a, float fDistance, int iDefaultWall)
{
	...
	if (!Success)
	{
		if (((TerrainWall[TERRAIN_INDEX_REPEAT(sx, sy)] & TW_SAFEZONE)
			|| (TerrainWall[TERRAIN_INDEX_REPEAT(tx, ty)] & TW_SAFEZONE))
			&& !(TerrainWall[TERRAIN_INDEX_REPEAT(tx, ty)] & TW_CHARACTER))
		{
			Wall = TW_NOMOVE;
		}
