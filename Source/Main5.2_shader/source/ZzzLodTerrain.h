...
void RenderTerrainAlphaBitmap(int Texture,float xf,float yf,float SizeX,float SizeY,vec3_t Light,float Rotation=0.f,float Alpha=1.f, float Height=5.f, bool terrainback = true);
...
inline int MapFileEncryptV16(uint8_t* pbyDst, uint8_t* pbySrc, int iSize)
{
	if (pbyDst)
	{
		uint8_t xorKey[16] = {
		...
		{
			uint8_t key = xorKey[i % 16];
			pbyDst[i] = (pbySrc[i] ^ key) + wMapKey;
			wMapKey = (pbyDst[i] + key + i) & 0xFF;
		}
	}
