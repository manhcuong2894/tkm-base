...
void SEASON3B::CNewUIChatLogWindow::LoadImages()
{
	...
	LoadBitmap("Interface\\newui_scrollbar_m.tga", IMAGE_SCROLL_MIDDLE, GL_LINEAR, GL_REPEAT);
				...
				if (!pMsgText->GetID().empty())
				{
					...
					if (SEASON3B::CheckMouseIn(ptRenderPos.x, ptRenderPos.y, RealTextSize.cx, RealTextSize.cy))
					{
						...
						if (MouseRButton && strID.empty() == false)
						{
							...
							g_pChatInputBox->SetWhsprIdBlocked(strID.c_str());
#else
...
#if jdk_dev_chat_fx
	gfxChat::SetPosition(x, y);

	if (gmProtect->MenuSwitch != 0 && gmProtect->MenuTypeButton != 2)
		gfxChat::SetPositionView(0, 168);
	else
		gfxChat::SetPositionView(0, 145);
...
{
#ifdef WISSPER_CHAT_BLOCKED
	if (g_pChatInputBox->CheckBlocked(strID.c_str()))
	{
		return;
	}
#endif // WISSPER_CHAT_BLOCKED
...
bool SEASON3B::CNewUIChatLogWindow::Render()
{
	...
	if (m_bShowChatLog)
	{
		if (g_pChatInputBox->RenderSystemMessage())
		{
...
bool SEASON3B::CNewUIChatLogWindow::GetMsgVisible(int MsgType)
{
	return m_msgEnable[MsgType];
}
