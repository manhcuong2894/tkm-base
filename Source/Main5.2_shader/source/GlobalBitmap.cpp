...
bool CGlobalBitmap::LoadImage(GLuint uiBitmapIndex, const std::string& filename, GLuint uiFilter, GLuint uiWrapMode)
{
	...
	if (0 == _stricmp(ext.c_str(), "jpg"))
		...
		return OpenDDS_DXT5(uiBitmapIndex, filename);
...
bool CGlobalBitmap::OpenDDS_DXT5(GLuint uiBitmapIndex, const std::string& filename)
{
	std::string filename_dxt;
	ExchangeExt(filename, "ddx5", filename_dxt);

	gli::texture texture = gli::load(filename_dxt);

	if (texture.empty())
	{
		//g_ConsoleDebug->Write(5, "no se pudo: %s", filename.c_str());
		return false;
	}

	gli::gl GL(gli::gl::PROFILE_GL33);
	gli::gl::format format = GL.translate(texture.format(), texture.swizzles());
	glm::ivec3 extent = texture.extent(0);

	BITMAP_t* pNewBitmap = new BITMAP_t;
	memset(pNewBitmap, 0, sizeof(BITMAP_t));

	pNewBitmap->BitmapIndex = uiBitmapIndex;
	pNewBitmap->Width = (float)extent.x;
	pNewBitmap->Height = (float)extent.y;
	pNewBitmap->output_width = extent.x;
	pNewBitmap->output_height = extent.y;
	...
	// Configuración de filtros para mipmaps
	...
	glTexStorage2D(GL_TEXTURE_2D, static_cast<GLint>(texture.levels()), format.Internal, extent.x, extent.y);

	for (size_t level = 0; level < texture.levels(); ++level)
	{
		glm::tvec3<GLsizei> Extent(texture.extent(level));
		glCompressedTexSubImage2D(GL_TEXTURE_2D, static_cast<GLint>(level), 0, 0, Extent.x, Extent.y, format.Internal, static_cast<GLsizei>(texture.size(level)), texture.data(0, 0, level));
	}
