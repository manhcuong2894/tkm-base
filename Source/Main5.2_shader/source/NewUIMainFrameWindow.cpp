...
int SEASON3B::CNewUIMainFrameWindow::RenderExperience(float x, float y, float width, float height)
{
	...
	if (gCharacterManager.IsMasterLevel(CharacterAttribute->Class) == true && CharacterAttribute->Level >= CharacterAttribute->MaxCharacterLevel)
	{
		...
		Experience = Master_Level_Data.lMasterLevel_Experince;
		baseExperience = Master_Level_Data.lBase_MasterLevel_Experince;
		NextExperience = Master_Level_Data.lNext_MasterLevel_Experince;

		fNeedExp = NextExperience - baseExperience;
		fExp = Experience - baseExperience;

		if (Experience < baseExperience)
		{
			fExp = 0.f;
		}
		...
		{
			fExpBarNum = ((double)fExp / (double)fNeedExp) * 10.0;
		}
	...
	else
	{
		...
		Experience = (uint64_t)CharacterAttribute->Experience;
		baseExperience = (uint64_t)CharacterAttribute->BaseExperience;
		NextExperience = (uint64_t)CharacterAttribute->NextExperience;
	...
	float x_Next = x;
	double Exp_width = width;
	double fProgress = fExpBarNum - (uint64_t)fExpBarNum;
	...
	if (SEASON3B::CheckMouseIn(x, y, width, height) == true)
	{
		char strTipText[256];
		x_Next = x + (width / 2) - 40;
		sprintf(strTipText, GlobalText[1748], Experience, NextExperience);
...
bool SEASON3B::CNewUISkillList::UpdateMouseEvent()
{
	...
	{
		int32_t bySkillType = CharacterAttribute->Skill[i];
		...
		if (bySkillType && (bySkillType < AT_SKILL_STUN || bySkillType > AT_SKILL_REMOVAL_BUFF))
		{
			...
			uint8_t bySkillUseType = SkillAttribute[bySkillType].SkillUseType;
	...
	if (!gmProtect->checkold_school())
	{
		...
		if (SEASON3B::CheckMouseIn(x, y, width, height))
		{
			...
			for (int i = 0, iIndex = iStartIndex; i < 5; ++i, iIndex++)
			{
				if (gmProtect->checknew_school())
				{
					x = pos_center() + (width * i) + 331.5;
				}
				else
				{
					x = pos_center() + (width * i) + 222.0;
				}
...
{
#if MAIN_UPDATE > 603
	if ((int32_t)CharacterAttribute->SkillNumber > 0 && m_bSkillList)
	{
		float x, y, width, height;
	...
	if (!gmProtect->checkold_school())
	{
		...
		if ((int32_t)CharacterAttribute->SkillNumber > 0 && m_bSkillList)
		{
			...
			for (int i = 0; i < MAX_MAGIC; ++i)
			{
				int bySkillType = CharacterAttribute->Skill[i];
				...
				if (bySkillType && (bySkillType < AT_SKILL_STUN || bySkillType > AT_SKILL_REMOVAL_BUFF))
				{
					...
					if (bySkillUseType != SKILL_USE_TYPE_MASTER && bySkillUseType != SKILL_USE_TYPE_MASTERLEVEL)
					{
						...
						if (i == Hero->CurrentSkill)
							SEASON3B::RenderImage(IMAGE_SKILLBOX_USE, (int)x, (int)y, width, height);
						else
							SEASON3B::RenderImage(IMAGE_SKILLBOX, (int)x, (int)y, width, height);
						RenderSkillIcon(i, (int)x + 6, (int)y + 6, 20, 28);
					}
				}
			}
		}
	...
	{
		int32_t bySkillNumber = CharacterAttribute->SkillNumber;
...
bool SEASON3B::CNewUISkillList::SkillListShow()
{
	return m_bSkillList;
}
