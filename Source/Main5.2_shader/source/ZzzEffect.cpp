...
bool CheckCharacterRange(OBJECT* so, float Range, short PKKey, BYTE Kind = 0)
{
	...
	{
		CHARACTER* c = GetCharacterDataUnsafe(i);
		OBJECT* o = &c->Object;
...
bool AttackCharacterRange(int Index, vec3_t Position, float Range, BYTE Serial, short PKKey, WORD SkillSerialNum)
{
		...
		if (o->Live)
		{
			if (o->Visible)
			{
				if (c != Hero && (c->Dead > 0) != 1)
				{
					...
					{
						if ((Skill == Skill_Twister
							|| Skill == Skill_Evil_Spirit
							|| Skill == Skill_Evil_Spirit_Strengthener
							|| Skill == Skill_Evil_Spirit_Strengthener2) && !c->m_bFixForm)
						{
							c->StormTime = 10;
						}
	...
	else
	{
		...
		{
			if (Skill != Skill_Multi_Shot
				&& Skill != Skill_Multi_Shot_Strengthener
				&& Skill != Skill_Multi_Shot_Mastery)
			{
		...
		{
			SendRequestMagicAttack(Skill, (int)(Position[0] / TERRAIN_SCALE), (int)(Position[1] / TERRAIN_SCALE), (BYTE)SkillSerialNum, Count, DamageKey, SkillSerialNum);
		}
		result = true;
	}
	return result;
...
void CheckTargetRange(OBJECT* o)
{
	...
	if (to != NULL && to->Live && to->Visible)
	{
		...
		{
			o->LifeTime = 1;
			o->LifeTimeNormal = o->LifeTime;
			o->LifeTimeWork = standlimit(1);
			vec3_t Light;
...
void CreateEffect(int Type, vec3_t Position, vec3_t Angle, vec3_t Light, int SubType, OBJECT* Owner, short PKKey, WORD SkillIndex, WORD Skill, WORD SkillSerialNum, float Scale, short int sTargetIndex)
{
	for (int icntEffect = 0; icntEffect < MAX_EFFECTS; icntEffect++)
	{
		...
		if (!o->Live)
		{
			...
			o->LifeTimeWork = standlimit((int)o->LifeTime);
			return;
		}
	}
}
...
void CreateEffect(OBJECT* o, int Type, vec3_t Position, vec3_t Angle, vec3_t Light, int SubType, OBJECT* Owner, short PKKey, WORD SkillIndex, WORD Skill, WORD SkillSerialNum, float Scale, short int sTargetIndex)
{
	...
	case MODEL_FENRIR_THUNDER:
	{
		if (o->SubType == 0)
		{
			...
			BMD* p_b = GetModelDataUnsafe(o->Owner->Type);
			int irandom = rand() % 30;
	...
	case MODEL_PROTECTGUILD:
	{
		...
		BMD* b = GetModelDataUnsafe(o->Owner->Type);
	...
	case MODEL_BLOW_OF_DESTRUCTION:
	{
		...
		else if (o->SubType == 2)
		{
			...
			if (o->Owner->m_sTargetIndex < 0)
				break;
			CHARACTER* pCharacter = GetCharacterDataUnsafe(o->Owner->m_sTargetIndex);
			if (pCharacter)
			{
	...
	case MODEL_EFFECT_FLAME_STRIKE:
		if (o->SubType == 0)
		{
			o->Alpha = 0;
			o->LifeTime = 35;
			o->Velocity = GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_FLAMESTRIKE].PlaySpeed;
			o->AI = 0;
			o->m_iAnimation = rand();
		}
		break;
		...
		else	// SubType == 1
		{
			...
			case MODEL_EMPIREGUARDIANBOSS_FRAMESTRIKE:
			{
				if (o->SubType == 1)
				{
					...
					BMD* b = GetModelDataUnsafe(o->Type);
					//b->Velocity = 10.f;
	...
	case MODEL_WOLF_HEAD_EFFECT:
	{
		...
		{
			o->Scale = 1.0f;
			o->LifeTime = 10.0f / GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_THRUST].PlaySpeed;
		}
		else if (o->SubType == 1)
		{
			o->Velocity = 0.1f;
	...
	{
		VectorCopy(Position, o->Position);
		o->Velocity = GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_STAMP].PlaySpeed * 2.0f;
		o->LifeTime = 100;
		GetModelDataUnsafe(MODEL_DOWN_ATTACK_DUMMY_L)->Actions[0].PlaySpeed = GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_STAMP].PlaySpeed;
		GetModelDataUnsafe(MODEL_DOWN_ATTACK_DUMMY_R)->Actions[0].PlaySpeed = GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_STAMP].PlaySpeed;
	}
	break;
	...
	case MODEL_SHOCKWAVE01:
	{
		...
		if (o->SubType == 1)
		{
			o->Velocity = GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_DRAGONLORE].PlaySpeed;
		...
		{
			o->Velocity = GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_DARKSIDE_READY].PlaySpeed * 2.0f;
			o->LifeTime = 5.0f / o->Velocity;
			o->Scale = Scale;
			vec3_t vPos, vDir;
			VectorCopy(o->Position, vPos);
		...
		{
			o->Velocity = GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_GIANTSWING].PlaySpeed * 2.0f;
			o->LifeTime = 5.0f / o->Velocity;
	...
	case MODEL_SHOCKWAVE_SPIN01:
	{
		o->Scale = Scale;
		VectorCopy(Light, o->Light);
		o->Velocity = GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_GIANTSWING].PlaySpeed;
	...
	case MODEL_DRAGON_KICK_DUMMY:
	{
		...
		// ani keyframe 10 : 19
		o->LifeTime = 200;
		GetModelDataUnsafe(MODEL_DRAGON_KICK_DUMMY)->Actions[0].PlaySpeed = GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_DRAGONKICK].PlaySpeed;
		o->Velocity = GetModelDataUnsafe(MODEL_DRAGON_KICK_DUMMY)->Actions[0].PlaySpeed * 2.0f;
		VectorCopy(Light, o->Light);
	}
	break;
	case BITMAP_LAVA:
	...
	case MODEL_DRAGON_LOWER_DUMMY:
	{
		...
		GetModelDataUnsafe(o->Type)->Actions[o->CurrentAction].PlaySpeed = 0.03f;
		o->m_iAnimation = 0;
	}
	break;
	...
	case MODEL_VOLCANO_OF_MONK:
	{
		if (o->SubType == 1)
		{
			...
			VectorCopy(Position, o->Position);
			o->Position[2] = RequestTerrainHeight(o->Position[0], o->Position[1]);
			GetModelDataUnsafe(o->Type)->Actions[0].PlaySpeed = 0.1f;
			o->LifeTime = 100;
			o->Velocity = 0.6f;
			vec3_t vLight;
			Vector(1.0f, 1.0f, 1.0f, vLight);
	...
	case MODEL_WIND_SPIN03:
	{
		...
		CHARACTER* pCharacter = GetCharacterDataUnsafe(o->m_sTargetIndex);
...
void MoveParticle(OBJECT* o, int Turn)
{
	if (Turn)
	{
		...
		VectorAddDelta(o->Position, Position, o->Position);
	}
	else
	{
		VectorAddDelta(o->Position, o->Direction, o->Position);
	}
}
	...
	else
	{
		if (rand() % 2 == 0)
		{
			if (o->Owner == &Hero->Object)
			{
				...
				if (Skill == Skill_Penetration && o->Type == MODEL_ARROW_BOMB)
				{
...
void MoveEffect(OBJECT* o, int iIndex)
{
	...
	case MODEL_DRAGON:
	{
		...
		if (o->LifeTime <= 10)
		{
			o->LifeTime = 1000;
			o->LifeTimeNormal = o->LifeTime;
			o->LifeTimeWork = standlimit(1000);
		}
	...
	case MODEL_ARROW_AUTOLOAD:
	{
		...
		{
			OBJECT* pOwn = o->Owner;
			if (o->LifeTimeWork % standlimit(10))
			{
		...
		else if (o->SubType == 1)
		{
			...
			VectorAddDelta(o->Position, pOwner->Position, o->Position);
	...
	case MODEL_BLADE_SKILL:
	{
		...
		else if (o->SubType == 1)
		{
			...
			if (o->LifeTime >= 3 && 0 == o->LifeTimeNormal % (2))
			{
				VectorCopy(o->Owner->Angle, Angle);
				int Ran = -1;
				CHARACTER* pChar = GetCharacterDataUnsafe(o->PKKey);
	...
	case MODEL_NEWYEARSDAY_EVENT_YUT:
		...
		o->Position[2] += AdvanceByDelta(o->Gravity * 1.5f);
		o->Gravity -= AdvanceByDelta(1.5f);
	...
	case BITMAP_FIRE_CURSEDLICH:
		if (o->SubType == 0)
		{
			...
			BMD* pModel = GetModelDataUnsafe(pObject->Type);
			if (pModel)
			{
		...
		else if (o->SubType == 2)
		{
			...
			BMD* pModel = GetModelDataUnsafe(o->Owner->Type);
	...
	case MODEL_SUMMONER_WRISTRING_EFFECT:
	{
		...
		BMD* b = GetModelDataUnsafe(o->Type);
		if (b) b->PlayAnimation(&o->AnimationFrame, &o->PriorAnimationFrame, &o->PriorAction, o->Velocity / 5.f, o->Position, o->Angle);
	}
	break;
	case MODEL_SUMMONER_EQUIP_HEAD_SAHAMUTT:
		if (o->SubType == 0)
	...
	case MODEL_SUMMONER_SUMMON_SAHAMUTT:
	{
		...
		BMD* pModel = GetModelDataUnsafe(o->Type);
		if (pModel)
		{
	...
	case MODEL_DESAIR:
	{
		...
		if (oj->Live == true)
		{
			...
			if (o->LifeTimeWork % standlimit(10) == 0)
			{
	...
	case BITMAP_GATHERING:
		...
		for (int j = 0; j < 3; ++j)
		{
			...
			if (o->SubType == 1)
			{
				if (0 == (o->LifeTimeNormal % 2))
					CreateJointSync(BITMAP_JOINT_THUNDER, Position, o->Position, Angle, 3, NULL, 10.f, 10, 10);
				else
	...
	case BITMAP_JOINT_THUNDER:
	{
		...
		{
			BMD* pTargetModel = GetModelDataUnsafe(o->Owner->Type);
	...
	case BITMAP_FLAME:
		if (o->SubType == 0)
		{
			...
			if (o->Owner == &Hero->Object && (o->LifeTimeNormal % (20)) == 0)
			{
				o->LifeTime = (o->LifeTime._runvalue / 20) * 19.9f;
				o->LifeTimeNormal = (o->LifeTimeNormal / 20) * 19.9;
				o->LifeTimeWork = standlimit(o->LifeTime);
	...
	case MODEL_CHAIN_LIGHTNING:
	{
		...
		case 2:
		{
			...
			CHARACTER* pTargetChar = GetCharacterDataUnsafe(FindCharacterIndex(o->m_sTargetIndex));
			...
			BMD* pSourceModel = GetModelDataUnsafe(pSourceObj->Type);
			BMD* pTargetModel = GetModelDataUnsafe(pTargetObj->Type);
	...
	case MODEL_LIGHTNING_SHOCK:
	{
		...
		if (o->SubType == 0)
		{
			...
			BMD* pOwnerModel = GetModelDataUnsafe(pOwner->Type);
		...
		else if (o->SubType == 1)
		{
			...
			for (int i = 0; i < 6; ++i)
			{
				...
				AngleMatrix(vAngle, Matrix);
				VectorRotate(vDirection, Matrix, vPosition);
				VectorAddDelta(vPosition, o->Position, vPosition);
	...
	case MODEL_SKILL_WHEEL2:
		...
		if (gMapManager->InHellas() && o->LifeTimeWork % standlimit(4))
		{
			int PositionX = (int)(o->Position[0] / TERRAIN_SCALE);
			int PositionY = (int)(o->Position[1] / TERRAIN_SCALE);
	...
	case MODEL_SKILL_FURY_STRIKE + 1:
		...
		if (o->LifeTime > 15 && (o->LifeTimeWork % standlimit(3)) == 0)
		{
	...
	case MODEL_STORM:
		...
		case 0:
			...
			if (o->LifeTimeWork % standlimit(15))
				if (o->Owner == &Hero->Object)
					AttackCharacterRange(o->Skill, o->Position, 150.f, o->Weapon, o->PKKey);
			break;
	...
	case MODEL_ARROW_HOLY:
		if (o->SubType == 1)
		{
			...
			if (o->LifeTimeWork == standlimit(13))
		...
		else
		{
			...
			if (o->SubType != 0)
			{
				...
				else if (o->LifeTimeWork == standlimit(30))
				{
					o->AttackPoint[0] = 0;
					o->Kind = 1;
				}
	...
	case MODEL_LACEARROW:
		if (o->Type == MODEL_ARROW_NATURE)
		{
			if (o->SubType == 1)
			{
				...
				if ((size_t)o->LifeTimeWork > standlimit(25) && o->LifeTimeWork % standlimit(2) == 0)
				{
					for (int j = 0; j < 2; j++)
					{
		...
		else if (o->Type == MODEL_LACEARROW)
		{
			...
			if (o->LifeTimeWork % standlimit(2) == 0)
			{
				for (int j = 0; j < 3; j++)
				{
					Vector((float)(rand() % 32 - 16), (float)(rand() % 64 - 32), (float)(rand() % 32 - 16), Position);
	...
	case MODEL_CURSEDTEMPLE_PRODECTION_SKILL:
	{
		...
		BMD* herobmd = GetModelDataUnsafe(o->Owner->Type);
		if (herobmd)
		{
		...
		BMD* effbmd = GetModelDataUnsafe(o->Type);
		if (effbmd)
		{
	...
	case MODEL_ARROW_DARKSTINGER:
	{
		...
		if (pModel)
		{
			...
			if (o->LifeTime == (30) || o->LifeTime == (24))
			{
			...
			if (o->SubType == 4)
			{
				...
				if (Wall != TW_NOGROUND)
				{
					...
					if (o->Position[2] < Height)
					{
						...
						for (int i = 0; i < MAX_CHARACTERS_CLIENT; i++)
						{
							CHARACTER* tc = GetCharacterDataUnsafe(i);
							OBJECT* to = &tc->Object;
							float dx = o->Position[0] - to->Position[0];
							float dy = o->Position[1] - to->Position[1];
							float Distance = sqrtf(dx * dx + dy * dy);
	...
	case MODEL_WARCRAFT:
		...
		break;
	case BITMAP_FIRECRACKERRISE:
		if (0 == o->LifeTimeNormal % (5) && (0 == (rand() % 3)))
		{
	...
	case MODEL_ARROW_DRILL:
		if (o->SubType == 0 || o->SubType == 2)
		{
			...
			if (o->LifeTimeWork == standlimit(1))
			{
				CreateBomb(o->Position, true);
			}
			...
			AngleMatrix(Angle, Matrix);
			VectorRotate(o->Direction, Matrix, Position);
			interpolateres2(o->EyeLeft, Position, o->EyeLeft);
	...
	case MODEL_PIER_PART:
		...
		else if (o->SubType == 2)
		{
			if (o->Owner != NULL)
			{
				...
				if ((o->LifeTimeNormal % 3) == 0)
				{
					Vector(-90.f, 0.f, o->Angle[2], Angle);
					CreateJointSync(BITMAP_JOINT_FORCE, o->Position, o->Position, Angle, 2, NULL, 150.f);
				}
			}
		}
		break;
	...
	case BITMAP_FLARE:
		...
		break;
	case BITMAP_JOINT_THUNDER + 1:
		if (o->LifeTime < 5 && 0 == o->LifeTimeNormal % (2) && o->SubType == 0)
		{
			VectorCopy(o->Position, Position);
	...
	case MODEL_CUNDUN_GHOST:
		...
		else if (o->AnimationFrame > 6.0f)
		{
			if (checkNormalizer)
				o->PKKey += 1;
			o->Position[2] += AdvanceByDelta(o->PKKey * 0.8f);
		...
		else
		{
			...
			if (o->Angle[2] < 45.0f)
				o->Angle[2] += AdvanceByDelta(o->Skill * 0.3f);
			if (o->Angle[2] > 45.0f)
				o->Angle[2] -= AdvanceByDelta(o->Skill * 0.3f);

			if (o->Position[0] < Hero->Object.Position[0] + 300.0f)
				o->Position[0] += AdvanceByDelta(o->Skill * 0.2f);
			if (o->Position[0] > Hero->Object.Position[0] + 300.0f)
				o->Position[0] -= AdvanceByDelta(o->Skill * 0.2f);
			if (o->Position[1] < Hero->Object.Position[1] - 300.0f)
				o->Position[1] += AdvanceByDelta(o->Skill * 0.2f);
			if (o->Position[1] > Hero->Object.Position[1] - 300.0f)
				o->Position[1] -= AdvanceByDelta(o->Skill * 0.2f);
	...
	case BITMAP_JOINT_FORCE:
		if (o->SubType == 0)
		{
			if (o->LifeTime < 11 && 0 == o->LifeTimeNormal % (2))
			{
				Vector(90.f, 0.f, 0.f, o->Angle);
	...
	case MODEL_EFFECT_SKURA_ITEM:
	{
		if ((o->SubType == 0) || (o->SubType == 1))
		{
			if (o->Owner->Live && o->Live)
			{
				...
				BMD* b = GetModelDataUnsafe(MODEL_EFFECT_SKURA_ITEM);
				...
				if (o->LifeTime == (30) || o->LifeTime == (15) || o->LifeTime == (4))
				{
					...
					BMD* characterBMD = GetModelDataUnsafe(o->Owner->Type);
		...
		else if (o->SubType == 1)
		{
			...
			case MODEL_SWORDMAIN01_EMPIREGUARDIAN_BOSS_GAION_:
			{
				...
				if (fCurrentRate >= 0.0f && fCurrentRate <= 0.6f)
				{
					...
					BMD* pBMDSwordModel = GetModelDataUnsafe(o->Type);
					BMD* pOwnerModel = GetModelDataUnsafe(o->Owner->Type);
	...
	case BITMAP_SWORDEFF:
	{
		if (o->Owner->AnimationFrame > GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_GIANTSWING].NumAnimationKeys)
			o->Live = false;
	...
	case MODEL_DRAGON_KICK_DUMMY:
	{
		if (o->Owner->AnimationFrame > GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_DRAGONKICK].NumAnimationKeys
		...
		if (o->Owner->Live)
		{
			...
			if (o->Owner->AnimationFrame > 4 && o->Owner->AnimationFrame < 5)
			{
				...
				pModel = GetModelDataUnsafe(o->Owner->Type);
	...
	case BITMAP_LAVA:
	{
		if (o->Owner->AnimationFrame > GetModelDataUnsafe(o->Owner->Type)->Actions[PLAYER_SKILL_DRAGONLORE].NumAnimationKeys
	...
	case MODEL_PHOENIX_SHOT_EFFECT:
	{
		...
		if (o->LifeTime == 70)
		{
			CHARACTER* pCharacter = GetCharacterDataUnsafe(ow->m_sTargetIndex);
	...
	case MODEL_WIND_SPIN03:
	{
		...
		pCharacter = GetCharacterDataUnsafe(ow->m_sTargetIndex);
		if (!pCharacter)
			goto LABEL_3646;
...
void RenderWheelWeapon(OBJECT* o)
{
	...
	BMD* pModel = GetModelDataUnsafe(Type);
...
{
	BMD* b = GetModelDataUnsafe(MODEL_SPEARSKILL);
	if (b)
	{
...
void RenderEffects(bool bRenderBlendMesh)
{
	...
	for (int i = 0; i < iEffectSize; ++i)
	{
		...
		if (o->Live)
		{
			o->Visible = TestFrustrum2D(o->GetPositionX(), o->GetPositionY(), -200.f);
...
void RenderAfterEffects(bool bRenderBlendMesh)
{
	...
	for (int i = 0; i < iEffectSize; ++i)
	{
		...
		{
			o->Visible = TestFrustrum2D(o->GetPositionX(), o->GetPositionY(), -20.f);
		...
		if (o->Live)
		{
			if (o->Visible)
			{
				...
				case BITMAP_MAGIC + 1:
					...
					else if (o->SubType == 6 && 0 == o->LifeTimeNormal % (25))
					{
						CreateEffect(BITMAP_MAGIC + 1, o->Position, o->Angle, o->Light, 7, o->Owner);
					}
					else if (o->SubType == 8 && 0 == o->LifeTimeNormal % (2))
					{
