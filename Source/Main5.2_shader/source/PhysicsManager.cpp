...
{
	m_vForce[0] += AdvanceByDelta(fXForce);
	m_vForce[1] += AdvanceByDelta(fYForce);
	m_vForce[2] += AdvanceByDelta(fZForce);
}
...
void CPhysicsVertex::Move(float fTime)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		m_vVel[i] += (m_vForce[i] * s_fInvOfMass * fTime) * AdvanceByDelta(1.0);
		m_vPos[i] += (m_vVel[i] * fTime) * AdvanceByDelta(1.0);
	}
}
...
BOOL CPhysicsVertex::KeepLength(CPhysicsVertex* pVertex2, float* pfLength)
{
	...
	if (fDistance > pfLength[1])
	{
		...
		interpolateres2(m_vPos, vDistance, m_vPos);
	}
...
void CPhysicsVertex::AddOneTimeMoveToKeepLength(CPhysicsVertex* pVertex2, float fLength)
{
	...
	interpolateres2(m_vOneTimeMove, vDistance, m_vOneTimeMove);
	VectorAddDelta(pVertex2->m_vOneTimeMove, vDistance, pVertex2->m_vOneTimeMove);

	m_iCountOneTimeMove += AdvanceByDelta(1.0);
	pVertex2->m_iCountOneTimeMove += AdvanceByDelta(1.0);
}
...
void CPhysicsVertex::DoOneTimeMove()
{
	...
	if (m_iCountOneTimeMove > 0)
	{
		for (int i = 0; i < 3; ++i)
		{
			m_vOneTimeMove[i] /= ReduceWithDelta((float)m_iCountOneTimeMove);
		}
		VectorAddDelta(m_vPos, m_vOneTimeMove, m_vPos);
		m_vOneTimeMove[0] = 0.0f;
		m_vOneTimeMove[1] = 0.0f;
		m_vOneTimeMove[2] = 0.0f;
		m_iCountOneTimeMove = 0;
	}
}
...
{
	VectorAddDelta(m_vOneTimeMove, vMove, m_vOneTimeMove);
	m_iCountOneTimeMove += AdvanceByDelta(1.0);
}
...
void CPhysicsCloth::SetFixedVertices(float Matrix[3][4])
{
	...
	for (int iVertex = 0; iVertex < m_iNumHor; ++iVertex)
	{
		...
		BMD* pModel = gmClientModels->GetModel(m_oOwner->Type);

		if (pModel)
		{
			pModel->TransformPosition(Matrix, vPos, vPos2, true);
...
{
	int iSeed = standlimit(101) * (MoveSceneFrame / standlimit(10)) % m_iNumVertices;;
...
void CPhysicsCloth::MoveVertices(float fTime)
{
	...
	vec4_t* bone = m_oOwner->GetBoneTransform(m_iBone);

	if (bone)
	{
		SetFixedVertices(bone);
