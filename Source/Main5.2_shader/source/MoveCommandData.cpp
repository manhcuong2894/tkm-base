...
{
	auto li = std::find_if(m_listMoveInfoData.begin(), m_listMoveInfoData.end(),
		[iIndex](const MOVEINFODATA* data)
		{
			return data->_ReqInfo.index == iIndex;
		});

	return (li != m_listMoveInfoData.end()) ? (*li) : NULL;
}
