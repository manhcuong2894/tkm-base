...
{
	int index = item_info->Type;
	WORD maxDurability = item_info->Durability;

	if (index == GET_ITEM(12, 144) || index == GET_ITEM(12, 146) || item_info->Kind2 == ItemKind2::PENTAGRAM) // Mithril Fragment,Elixir Fragment
	{
	...
	CUSTOM_ITEM_STACK* StackItem = GMProtect->GetItemStack(index, Level);

	int MaxStack = 0;

	if(StackItem) MaxStack = StackItem->MaxStack;
	...
	if (Level >= 5)
	{
		if (Level == 10)
		{
			dur = item_info->Durability + ((Level * 2) - 3);
		}
		else if (Level == 11)
		{
			dur = item_info->Durability + ((Level * 2) - 1);
		}
		else if (Level == 12)
		{
			dur = item_info->Durability + ((Level * 2) + 2);
		}
		else if (Level == 13)
		{
			dur = item_info->Durability + ((Level * 2) + 6);
		}
		else if (Level == 14)
		{
			dur = item_info->Durability + ((Level * 2) + 11);
		}
		else if (Level == 15)
		{
			dur = item_info->Durability + ((Level * 2) + 17);
		}
		else
		{
			dur = item_info->Durability + ((Level * 2) - 4);
		}
	}
	else
	{
		dur = item_info->Durability + Level;
	}

	if (index != GET_ITEM(0, 19) && index != GET_ITEM(2, 13) && index != GET_ITEM(4, 18) && index != GET_ITEM(5, 10) && index != GET_ITEM(5, 36) && item_info->m_byItemSlot != 7) // Sword of Archangel,Scepter of Archangel,Crossbow of Archangel,Staff of Archangel,Stick of Archangel
	{
		if ((pItem->ExtOption % 0x04) != 0)
		{
			dur += 20;
		}
		else if ((pItem->Option1 & 0x3F) != 0)
		{
			dur += 15;
		}
	}
...
void GetSpecialOptionText(int Type, char* Text, WORD Option, WORD Value, int iMana)
{
	switch (Option)
	{
...
void RenderSkillInfo(int sx, int sy, int Type, int SkillNum, int iRenderPoint /*= STRP_NONE*/)
{
	...
	if (CharacterAttribute->Skill[Type] == Skill_Infinity_Arrow)
	{
...
{
	static std::vector<sItemAct> sItem;
...
bool IsExistUndecidedPrice()
{
	...
	for (int i = 0; i < MAX_PERSONALSHOP_INVEN; ++i)
	{
		...
		if (pItem)
		{
			...
			iIndex = INVENTORY_SHOP_INDEX + (pItem->y * COL_PERSONALSHOP_INVEN) + pItem->x;
