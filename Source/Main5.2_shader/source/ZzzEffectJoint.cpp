		...
		if (!o->Live)
		{
			...
			case BITMAP_JOINT_THUNDER:
				...
				case 2:
					...
					GetModelDataUnsafe(MODEL_PLAYER)->TransformPosition(o->Target->BoneTransform[33], BitePosition, o->TargetPosition, true);
					break;
				case 3:
				...
				case 15:
					...
					{
						CHARACTER* tc = GetCharacterDataUnsafe(j);
						OBJECT* to = &tc->Object;
			...
			case BITMAP_FLARE_BLUE:
				...
				else if (o->SubType == 20)
				{
					...
					BMD* b = GetModelDataUnsafe(o->Target->Type);
					if (b)
					{
						vec3_t p;
			...
			case BITMAP_LAVA:
			{
				...
				o->m_bCreateTails = true;
				Vector(1.0f, 1.0f, 1.0f, o->Light);
				o->Velocity = GetModelDataUnsafe(o->Target->Type)->Actions[o->Target->CurrentAction].PlaySpeed;
			}
			break;
			...
			o->MaxTails = standlimit((int)o->MaxTails);

			if (o->MaxTails > MAX_TAILS)
				o->MaxTails = MAX_TAILS;

			o->LifeTimeWork = standlimit((int)o->LifeTime);
			o->LifeTimeNormal = ((int)o->LifeTime);
			return;
		}
	}
}
...
void MoveJoint(JOINT* o, int iIndex)
{
	...
	case BITMAP_JOINT_ENERGY:
		...
		case 57:
			...
			case 57:
			{
				BMD* pModel = GetModelDataUnsafe(o->Target->Type);
				if (pModel)
				{
			...
			case 3:
				...
				CreateParticle(BITMAP_LIGHTNING + 1, o->Position, o->Angle, Light, 0, 1.0, NULL);
				break;
			case 14:
			case 15:
				Luminosity = sinf(WorldTime * 0.002f) * 0.3f + 0.8f;
				Vector(Luminosity, Luminosity * 0.1f, Luminosity * 0.1f, Light);
				CreateParticle(BITMAP_LIGHTNING + 1, o->Position, o->Angle, Light, 4, 1.0, NULL);
				break;
			case 22:
			case 23:
		...
		case 46:
		{
			...
			else
			{
				...
				{
					o->Live = false;
					CreateParticleSync(BITMAP_LIGHTNING + 1, o->Position, o->Angle, o->Light, 1);
			...
			if (o->SubType == 6 || o->SubType == 9)
			{
				CreateParticleSync(BITMAP_LIGHTNING + 1, o->Position, o->Angle, o->Light, 3, 0.05f);
			}
			...
			{
				CreateParticleSync(BITMAP_LIGHTNING + 1, o->Position, o->Angle, o->Light);
			}
		}
		break;
		case 42:
		{
			...
			Vector(Luminosity * 0.4f, Luminosity, Luminosity * 0.8f, Light);
			AddTerrainLight(o->Position[0], o->Position[1], Light, 2, PrimaryTerrainLight);
			CreateParticleSync(BITMAP_LIGHTNING + 1, o->Position, o->Angle, o->Light, 5, 0.12f);
		}
		break;
		case 43:
		{
	...
	case BITMAP_JOINT_HEALING:
		...
		else if (o->SubType == 9)
		{
			...
			o->Position[2] = o->TargetPosition[2] + 10 + (90 - (float)o->LifeTime);
	...
	case BITMAP_JOINT_SPIRIT2:
		if (o->SubType == 0 || o->SubType == 5 || o->SubType == 19)
		{
			if (o->Scale == 80.f)
			{
				...
				if (o->LifeTimeWork % standlimit(15) == 0 && o->Target == &Hero->Object)
				{
			...
			if (o->SubType == 5)
				MoveHumming(o->Position, o->Angle, o->TargetPosition, 2.f, true);
			else
				MoveHumming(o->Position, o->Angle, o->TargetPosition, 10.f, true);
			...
			if (o->SubType != 5)
			{
				...
				o->Direction[0] += AdvanceByDelta(randuin * 0.2f);
				o->Direction[2] += AdvanceByDelta(randuin * 0.8f);
		...
		else if (o->SubType == 3 || o->SubType == 13)
		{
			...
			else if (o->Target != NULL)
			{
				...
				if (o->SubType == 13)
				{
					...
					Distance = MoveHumming(o->Position, o->Angle, Position, (o->Velocity - 20), false);
				}
				else
				{
					Distance = MoveHumming(o->Position, o->Angle, o->Target->Position, (o->Velocity - 20), false);
				}
				if (o->Velocity > 50.f)
					o->Velocity = 50.f;
				else
		...
		else if (o->SubType == 17)
		{
			...
			if (o->LifeTimeWork == standlimit(100))
			{
				for (int i = 0; i < 120; ++i)
				{
					Vector(0.f, 0.f, i * 3.f, Angle);
		...
		else if (o->SubType == 18)
		{
			...
			size_t add = (size_t)(o->LifeTimeWork % standlimit(12));
	...
	case MODEL_SPEARSKILL:
		if (o->m_iChaIndex != -1)
		{
			CHARACTER* pCharacter = GetCharacterDataUnsafe(o->m_iChaIndex);
	...
	case MODEL_FENRIR_SKILL_THUNDER:
		for (int j = 0; j < o->MaxTails; j++)
		{
			...
			Vector(0.f, -o->Velocity, 0.f, Position);
			VectorRotate(Position, Matrix, p);
			VectorAddDelta(o->Position, p, o->Position);
		}
		break;
	case BITMAP_BLUR + 1:
	case BITMAP_JOINT_LASER + 1:
	...
	case BITMAP_JOINT_THUNDER:
				...
				if (o->Target)
				{
					if (j == 0)
					{
						...
						if (pTargetChar)
						{
							...
							BMD* pModel = GetModelDataUnsafe(pSourceObj->Type);
				...
				if (o->Scale == 50.f)
				{
					...
					if (o->SubType == 0 || o->SubType == 27)
					{
						if ((int)WorldTime % 1000 < 500 && rand() % 16 == 0)
						{
							...
							VectorAdd(Position, o->TargetPosition, Position);
							CreateJointSync(BITMAP_JOINT_THUNDER, Position, o->TargetPosition, o->Angle, 28, o->Target, 6.0f + (float)(rand() % 8), -1, 0, 0, 0, o->Light);
						}
					}
				}
				break;
		...
		else if (o->SubType == 15 && o->LifeTimeNormal % (2) == 0)
		{
			...
			for (int i = 0; i < o->MultiUse; ++i)
			{
				if ((i % 15) == 0)
				{
					CHARACTER* tc = GetCharacterDataUnsafe(o->TargetIndex[i / 15]);
					OBJECT* to = &tc->Object;
	...
	case BITMAP_FLARE_BLUE:
		...
		else if (o->SubType == 7 || o->SubType == 11 || o->SubType == 25 || o->SubType == 45 || o->SubType == 46 || o->SubType == 47)
		{
			...
			if (o->SubType != 11 && o->LifeTime == 1 && o->SubType != 45 && o->SubType != 46 && o->SubType != 47)
			{
				...
				CreateParticleSync(BITMAP_EXPLOTION, o->Position, Angle, Light, 1, 0.6f);
			}
		}
		else if (o->SubType == 8)
		{
	...
	case BITMAP_JOINT_FORCE:
		if (o->SubType == 10 || o->SubType == 8)
		{
			...
			for (int j = 0; j < 8; ++j)
			{
				if (o->NumTails < o->MaxTails - 1)
				{
					...
					o->Direction[2] -= AdvanceByDelta(11.f);
				...
				if (o->SubType == 10)
				{
					if (o->Target == &Hero->Object && o->LifeTimeNormal > 18)
					{
						if ((j % 5) == 0)
						{
							if (battleCastle::IsBattleCastleStart())
			...
			if (o->SubType == 10)
			{
				if (o->m_bySkillSerialNum == 0)
				{
					if (o->LifeTime < 10)
						...
						Luminosity = o->LifeTimeNormal / 30.f;
					Vector(Luminosity, Luminosity, Luminosity, o->Light);
				}
				else
				{
		...
		else if (o->SubType >= 2 && o->SubType <= 6)
		{
			...
			if (o->LifeTime < 15)
				o->Direction[2] += AdvanceByDelta(0.5f);
			else
				o->Direction[2] += AdvanceByDelta(o->Direction[0]);
			...
			if ((o->LifeTimeNormal % 5) == 0 && o->SubType == 2)
			{
				CreateEffectSync(MODEL_SKILL_INFERNO, o->StartPosition, o->HeadAngle, o->Light, 6, NULL, 20, 0);
			}
			else if (o->SubType == 4)
			{
				...
				else if (o->SubType >= 1 && o->SubType <= 4 || (o->SubType >= 11 && o->SubType <= 13))
				{
					if (o->Weapon <= 0)
					{
						...
						for (int i = 1; i < o->MultiUse; ++i)
						{
							...
							if (o->SubType == 2 || o->SubType == 4)
							{
								o->TargetPosition[1] -= AdvanceByDelta(20.f);
							}
							...
							o->Velocity -= AdvanceByDelta(2.f);
							o->TargetPosition[0] -= AdvanceByDelta(2.5f);
						}
						o->MultiUse += AdvanceByDelta(2);
					}
					else
			...
			if (o->Position[2] < Height)
			{
				...
				if (o->m_bCreateTails && o->PKKey != 1)
				{
					if (o->SubType < 2)
					{
						if (o->Target != NULL)
						{
							BMD* b = GetModelDataUnsafe(MODEL_SHADOW_BODY);
	...
	case BITMAP_FORCEPILLAR:
	{
		...
		GetModelDataUnsafe(o->Target->Type)->Animation(BoneTransform, o->Target->AnimationFrame, o->Target->PriorAnimationFrame, o->Target->PriorAction, o->Target->Angle, o->Target->HeadAngle, false, false);

		GetModelDataUnsafe(o->Target->Type)->TransformByObjectBone(o->Position, o->Target, 0);
	}
	break;
	case BITMAP_SWORDEFF:
	{
		...
		GetModelDataUnsafe(o->Target->Type)->TransformByObjectBone(o->Position, o->Target, 5);
	}
	break;
	case BITMAP_GROUND_WIND:
	{
	...
	case BITMAP_SWORDEFFOR2_BLUR:
	{
		...
		BMD* pModel = GetModelDataUnsafe(owner->Type);
		...
		if (pModel)
		{
			...
			if (owner->AnimationFrame < 18.0 && owner->AnimationFrame >= 3.0 && checkNormalizer)
			{
				...
				if (mtAction)
				{
					...
					{
						pModel = GetModelDataUnsafe(owner->Type);
						if (!pModel)
							break;
	...
	{
		if (o->LifeTimeWork % standlimit(12))
		{
			MoveJoint(o, iIndex);
		}
	}
...
void RenderJoints(BYTE bRenderOneMore)
{
	...
	for (int i = 0; i < MAX_JOINTS; i++)
	{
		...
		if (o->Live && o->NumTails > 0 && o->RenderFace != 0)
		{
			...
			//if (!SphereInFrustum(o->Position, 10.f))
			//	continue;
			...
			{
				if (o->Type != BITMAP_SMOKE || o->SubType)
				{
					if (o->Type == BITMAP_JOINT_SPARK)
					{
						if (o->SubType == 5) BindTexture(o->TexType);
					}
					else
					{
						glColor3fv(o->Light);
			...
			{
				if ((o->Type != BITMAP_SMOKE || o->SubType || j >= 1)
					&& (o->Type != BITMAP_JOINT_HEALING || o->SubType != 9 && o->SubType != 10 || j != (o->NumTails - 1)))
				{
					if (o->bTileMapping)
					{
						fLight1 = (o->NumTails - (j)) / 16.f;
						fLight2 = (o->NumTails - (j + 1)) / 16.f;
					}
					else if (o->m_byReverseUV == 3)
					{
						fLight1 = 1.f - j / (float)(o->MaxTails - 1);
						fLight2 = 1.f - (j + 1) / (float)(o->MaxTails - 1);
					}
					else
					{
						fLight1 = (o->NumTails - j) / (float)(o->MaxTails - 1);
						fLight2 = (o->NumTails - (j + 1)) / (float)(o->MaxTails - 1);
					}

					fScroll = (float)((int)WorldTime % 1000) * 0.001f;
					...
					{
						fLight1 = (o->NumTails - j) / (float)((o->MaxTails - 1) / 2);
						fLight2 = (o->NumTails - (j + 1)) / (float)((o->MaxTails - 1) / 2);
						fLight1 -= (fScroll);
						fLight2 -= (fScroll);
					}
					...
					if (o->Type != BITMAP_JOINT_FORCE || o->SubType)
					{
						if (o->Type != MODEL_SPEARSKILL || o->SubType && o->SubType != 4 && o->SubType != 9)
						{
							if (o->Type != BITMAP_JOINT_HEALING || (o->SubType != 9 && o->SubType != 10))
							{
								if (o->Type != BITMAP_JOINT_THUNDER + 1 || o->SubType)
								{
									if (o->Type != BITMAP_FLARE + 1 || o->SubType != 6)
									{
										if (o->Type != BITMAP_FLARE + 1 || o->SubType != 8)
										{
											if ((o->Type != BITMAP_FLARE_FORCE || o->SubType < 0 || o->SubType > 4)
												&& (o->SubType < 11 || o->SubType > 13))
											{
												if (o->Type == BITMAP_JOINT_FORCE && o->SubType == 1)
												{
													float fLuminosity = (1.f - (o->NumTails - j) / (float)(o->NumTails)) * 2.f;
													glColor3f(o->Light[0] * fLuminosity, o->Light[1] * fLuminosity, o->Light[2] * fLuminosity);
												}
											}
											else
											{
												float fLuminosity = ((float)((o->NumTails - 1 - j) / (float)(o->MaxTails)) * 2);
												glColor3f(o->Light[0] * fLuminosity, o->Light[1] * fLuminosity, o->Light[2] * fLuminosity);
											}
										}
										else if (j == 0)
						...
						if ((o->RenderFace & RENDER_FACE_ONE))
						{
							VectorCopy(fCurColor, project.Color, 4);
							//glBegin(GL_QUADS);
							//glTexCoord2f(L1, V2);
							...
							//glEnd();
							project.TexCoord[0] = L1;
							project.TexCoord[1] = V2;
							VectorCopy(o->Tails[j][2], project.Position);
							vertexBuffer.push_back(project);

							project.TexCoord[0] = L1;
							project.TexCoord[1] = V1;
							VectorCopy(o->Tails[j][3], project.Position);
							vertexBuffer.push_back(project);

							project.TexCoord[0] = L2;
							project.TexCoord[1] = V1;
							VectorCopy(o->Tails[j + 1][3], project.Position);
							vertexBuffer.push_back(project);

							project.TexCoord[0] = L2;
							project.TexCoord[1] = V2;
							VectorCopy(o->Tails[j + 1][2], project.Position);
							vertexBuffer.push_back(project);
						}
						if ((o->RenderFace & RENDER_FACE_TWO))
						{
							VectorCopy(fCurColor, project.Color, 4);
							if (o->Type == BITMAP_JOINT_THUNDER || o->Type == BITMAP_JOINT_THUNDER + 1)
							{
							...
							project.TexCoord[0] = L1;
							project.TexCoord[1] = V1;
							VectorCopy(o->Tails[j][0], project.Position);
							vertexBuffer.push_back(project);

							project.TexCoord[0] = L1;
							project.TexCoord[1] = V2;
							VectorCopy(o->Tails[j][1], project.Position);
							vertexBuffer.push_back(project);

							project.TexCoord[0] = L2;
							project.TexCoord[1] = V2;
							VectorCopy(o->Tails[j + 1][1], project.Position);
							vertexBuffer.push_back(project);

							project.TexCoord[0] = L2;
							project.TexCoord[1] = V1;
							VectorCopy(o->Tails[j + 1][0], project.Position);
							vertexBuffer.push_back(project);
						}
					}
					else
					{
						//glBegin(GL_QUADS);
						//glTexCoord2f(fLight1, 0.f);
						//glVertex3fv(o->Tails[j][0]);
						//
						//glTexCoord2f(fLight1, 1.f);
						//glVertex3fv(o->Tails[j][1]);
						//
						//glTexCoord2f(fLight2, 1.f);
						//glVertex3fv(o->Tails[j + 1][1]);
						//
						//glTexCoord2f(fLight2, 0.f);
						//glVertex3fv(o->Tails[j + 1][0]);
						//glEnd();

						float Luminosity = (float)(o->MaxTails - j) / (float)(o->MaxTails);
						Luminosity += Luminosity;
						Luminosity *= o->Light[0];

						fCurColor[3] = 1.f;
						VectorSet(fCurColor, Luminosity, Luminosity, Luminosity);
						glColor3f(Luminosity, Luminosity, Luminosity);

						VectorCopy(project.Color, fCurColor, 4);

						project.TexCoord[0] = fLight1;
						project.TexCoord[1] = 0.f;
						VectorCopy(o->Tails[j][0], project.Position);
						vertexBuffer.push_back(project);

						project.TexCoord[0] = fLight1;
						project.TexCoord[1] = 1.f;
						VectorCopy(o->Tails[j][1], project.Position);
						vertexBuffer.push_back(project);

						project.TexCoord[0] = fLight2;
						project.TexCoord[1] = 1.f;
						VectorCopy(o->Tails[j + 1][1], project.Position);
						vertexBuffer.push_back(project);
			...
			if (vertexBuffer.size())
			{
				...
				glVertexPointer(3, GL_FLOAT, sizeof(VertexGPU), reinterpret_cast<char*>(vertexBuffer.data()) + offsetof(VertexGPU, Position));
				glColorPointer(4, GL_FLOAT, sizeof(VertexGPU), reinterpret_cast<char*>(vertexBuffer.data()) + offsetof(VertexGPU, Color));
				glTexCoordPointer(2, GL_FLOAT, sizeof(VertexGPU), reinterpret_cast<char*>(vertexBuffer.data()) + offsetof(VertexGPU, TexCoord));
