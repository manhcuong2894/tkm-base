...
namespace SEASON3B
{
	...
	public:
		...
		void SetEnableSend();
		void SetDisableSend();
		bool RenderSystemMessage();
		int GetRenderWidh();
#ifdef WISSPER_CHAT_BLOCKED
		bool CheckBlocked(const unicode::t_char* pzText);
		void SetWhsprIdBlocked(const unicode::t_char* pzText);
#endif // WISSPER_CHAT_BLOCKED
	protected:
...
#ifdef WISSPER_CHAT_BLOCKED
		void initListBlocked();
		void RenderChatBlocked();
		bool UpdateChatBlocked();
		bool UpdateMouseActionChatBlocked();
#endif // WISSPER_CHAT_BLOCKED
	};
}
