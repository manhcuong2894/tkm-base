...
void CMapManager::LoadWorld(int Map)
{
	...
	sprintf_s(WorldName, "World%d", iMapWorld);
	sprintf_s(FileName, "Data\\%s\\EncTerrain%d.map", WorldName, iMapWorld);
	...
	if (iResult != -1)
	{
		...
		{
			sprintf_s(FileName, "Data\\%s\\CWScript%d.cws", WorldName, iMapWorld);
		...
		if (this->InBattleCastle())
		{
			...
			{
				sprintf_s(FileName, "Data\\%s\\EncTerrain%d.att", WorldName, iMapWorld * 10 + 2);
			}
			else
			{
				sprintf_s(FileName, "Data\\%s\\EncTerrain%d.att", WorldName, iMapWorld);
			}
		}
		else if (this->currentMap == WD_34CRYWOLF_1ST)
		{
			...
			case CRYWOLF_OCCUPATION_STATE_OCCUPIED:
				sprintf_s(FileName, "Data\\%s\\EncTerrain%d.att", WorldName, iMapWorld * 10 + 1);
				break;
		...
		if (iResult != -1)
		{
			sprintf_s(FileName, "Data\\%s\\EncTerrain%d.obj", WorldName, iMapWorld);

			iResult = OpenObjectsEnc(FileName);

			if (iResult != -1)
			{
				sprintf_s(FileName, "%s\\TerrainHeight.bmp", WorldName);
				CreateTerrain(FileName, IsTerrainHeightExtMap(this->currentMap));

				if (this->InBattleCastle())
				{
					if (battleCastle::IsBattleCastleStart())
						sprintf_s(FileName, "%s\\TerrainLight2.jpg", WorldName);
					else
						sprintf_s(FileName, "%s\\TerrainLight.jpg", WorldName);
				}
				else
				{
					if (this->currentMap == WD_34CRYWOLF_1ST)
					{
						...
						case CRYWOLF_OCCUPATION_STATE_PEACE:
							...
							break;
						case CRYWOLF_OCCUPATION_STATE_OCCUPIED:
							sprintf_s(FileName, "%s\\TerrainLight1.jpg", WorldName);
							break;
						case CRYWOLF_OCCUPATION_STATE_WAR:
							sprintf_s(FileName, "%s\\TerrainLight2.jpg", WorldName);
							break;
			...
			{
				char Text[256];
				sprintf_s(Text, "%s file corrupted.", FileName);
