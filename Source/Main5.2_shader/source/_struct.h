...
typedef struct tagITEM
{
	...
	/*+54*/		WORD	SpecialValue[MAX_ITEM_SPECIAL];
	/*+62*/		DWORD	Key;
...
public:
	void Init();
	bool CheckMasterLevel();
	uint64_t character_experience(CharacterExperience Type, int Character_Level);
#ifdef SHUTDOWN_CHARACTER_INFO
...
{
	short		nMLevel;
	uint64_t	lMasterLevel_Experince;
	uint64_t	lBase_MasterLevel_Experince;
	uint64_t	lNext_MasterLevel_Experince;
...
typedef struct
{
	/*+000*/	char strSetName[64];
	/*+064*/	BYTE byStandardOption[6][2];
	/*+076*/	BYTE byStandardOptionValue[6][2];
	/*+088*/	BYTE byExtOption[2];
	/*+090*/	BYTE byExtOptionValue[2];
	/*+092*/	BYTE byOptionCount;
	/*+093*/	BYTE byFullOption[5];
	/*+098*/	BYTE byFullOptionValue[5];
	/*+103*/	BYTE byRequireClass[MAX_CLASS_PLAYER];
}ITEM_SET_OPTION;
//csitemsetoption end
