...
bool CNewUIMixInventory::Update()
{
	if (!m_pNewInventoryCtrl || m_pNewInventoryCtrl->Update())
	{
		if (this->IsVisible())
		{
			...
			int MixInventoryType = g_MixRecipeMgr.GetMixInventoryType();

			if (MixInventoryType == SEASON3A::MIXTYPE_ATTACH_SOCKET || MixInventoryType == SEASON3A::MIXTYPE_DETACH_SOCKET)
			{
				if (m_SocketListBox.GetLineNum())
				{
					if (!g_MixRecipeMgr.GetFirstItemSocketCount())
						m_SocketListBox.Clear();
				}
				else
				{
...
void CNewUIMixInventory::LoadImages()
{
	...
	LoadBitmap("Interface\\newui_scrollbar_m.tga", CNewUIGuardWindow::IMAGE_GUARDWINDOW_SCROLL_MIDDLE, GL_LINEAR, GL_REPEAT);
...
bool CNewUIMixInventory::Mix()
{
	...
	if (g_MixRecipeMgr.GetMixInventoryType() == SEASON3A::MIXTYPE_ATTACH_SOCKET)
	{
		...
		sprintf_s(Text, GlobalText[2655], iSelectedLine, GlobalText[2640 + iSelectedLine]);

		g_pChatListBox->AddText("", Text, SEASON3B::TYPE_PARTY_MESSAGE);
		...
		if (pPickedItem)
		{
			...
			else if (GetMixState() == MIX_READY && g_MixRecipeMgr.IsMixSource(pPickedItem->GetItem()) && pItemObj->ex_src_type == ITEM_EX_SRC_EQUIPMENT)
			{
				...
				if (SEASON3B::IsPress(VK_LBUTTON))
				{
					...
					if (iTargetIndex != -1 && m_pNewInventoryCtrl->CanMove(iTargetIndex, pItemObj))
					{
						SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, iSourceIndex, pItemObj, iCurInventory, iTargetIndex);
						return true;
					}
				}
			}
