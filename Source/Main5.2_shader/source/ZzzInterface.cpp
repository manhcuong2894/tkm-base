...
bool CheckTile(CHARACTER* pCharacter, OBJECT* pObject, float Range)
{
	if (pCharacter && pObject)
	{
		float dx = pObject->Position[0] - (TargetX * TERRAIN_SCALE + TERRAIN_SCALE * 0.5f);
		float dy = pObject->Position[1] - (TargetY * TERRAIN_SCALE + TERRAIN_SCALE * 0.5f);
	...
	do
	{
		...
		if ((_type != MODEL_MONSTER01 + 183
			...
			&& (TerrainWall[Index] >= TW_NOMOVE
				&& !((unsigned int)TerrainWall[Index] & TW_ACTION)
				&& !((unsigned int)TerrainWall[Index] & TW_HEIGHT)
				&& !((unsigned int)TerrainWall[Index] & TW_CAMERA_UP)))
		{
			return false;
		}
...
bool CheckAttack_Fenrir(CHARACTER* pCharacter)
{
	...
	if (::IsStrifeMap(World) && pCharacter != Hero && pCharacter->m_byGensInfluence != Hero->m_byGensInfluence)
	{
	...
	else if (pCharacter->Object.Kind == KIND_PLAYER)
	{
		...
		{
			if (!EnableGuildWar || pCharacter->PK < PVP_MURDERER2 || pCharacter->GuildMarkIndex == -1 || strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[pCharacter->GuildMarkIndex].GuildName))
			{
...
int	getTargetCharacterKey(CHARACTER* c, int selected, int Key)
{
	...
	else
	{
		...
		if (c == Hero && sc)
		{
			...
			else if (!EnableGuildWar || sc->PK < PVP_MURDERER2 || sc->GuildMarkIndex == -1 || strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[sc->GuildMarkIndex].GuildName))
			{
				if (g_DuelMgr.IsDuelEnabled())
				{
				...
				else if (sc->GuildRelationShip != GR_RIVAL && sc->GuildRelationShip != GR_RIVALUNION)
				{
					...
					else if (!::IsStrifeMap(World) || sc == Hero || sc->m_byGensInfluence == Hero->m_byGensInfluence || SEASON3B::IsRepeat(VK_MENU))
					{
					...
					{
						if (strcmp(GuildMark[Hero->GuildMarkIndex].GuildName, GuildMark[c->GuildMarkIndex].GuildName) && !g_pPartyManager->IsPartyMember(SelectedCharacter))
						{
							targetkey = -1;
						}
						else if (SEASON3B::IsRepeat(VK_CONTROL) == true)
...
bool SkillWarrior(CHARACTER* c, ITEM* p)
{
	...
	if (o->Type == MODEL_PLAYER)
	{
		if (o->CurrentAction == PLAYER_DEFENSE1)
			return false;
		if (o->CurrentAction >= PLAYER_ATTACK_SKILL_SWORD1 && o->CurrentAction <= PLAYER_ATTACK_SKILL_SWORD4 || o->CurrentAction == PLAYER_ATTACK_TWO_HAND_SWORD_TWO)
			return false;
	}
...
void UseSkillWarrior(CHARACTER* c, OBJECT* o)
{
	...
	if (Skill <= Skill_Fire_Burst)
	{
		...
		case Skill_Impale:
			if (c->Helper.Type == MODEL_HELPER + 37 || o->movementType == Movement::Fenrir)
				...
				SetAction(o, PLAYER_ATTACK_SKILL_SPEAR, true);
			goto LABEL_40;
		...
		case Skill_Crescent_Moon_Slash:
			SetAction(o, PLAYER_ATTACK_RUSH, 1);
			goto LABEL_40;
		default:
			goto LABEL_34;
		}
	...
	if (Skill > Skill_Cyclone_Strengthener)
	{
		...
		LABEL_19:
			SetAction(o, PLAYER_ATTACK_ONETOONE, 1);
			goto LABEL_40;
		...
		case Skill_Slash_Strengthener:
			SetAction(o, PLAYER_ATTACK_SKILL_SWORD5, 1);
			goto LABEL_40;
		case Skill_Falling_Slash_Strengthener:
			SetAction(o, PLAYER_ATTACK_SKILL_SWORD1, 1);
			goto LABEL_40;
		case Skill_Lunge_Strengthener:
			SetAction(o, PLAYER_ATTACK_SKILL_SWORD2, 1);
			goto LABEL_40;
		default:
			break;
		}
	LABEL_34:
		if (c->Helper.Type != (MODEL_HELPER + 37) || c->SafeZone)
			SetAction(o, g_MovementSkill.m_iSkill + PLAYER_ATTACK_SWORD_LEFT1, 1);
		else
	...
	{
	LABEL_30:
		SetAction(o, PLAYER_ATTACK_SKILL_SWORD4, 1);
		goto LABEL_40;
	}
		...
		if (tc)
		{
			...
			if (tc->Dead == 0)
			{
				...
				if (pCharacter)
				{
					...
					if (bCheckAttack)
					{
						bool bNoneWall = CheckWall(pCha->PositionX, pCha->PositionY, TargetX, TargetY, false);
	...
	if (tc)
	{
		if (tc->Object.Kind == KIND_NPC)
		{
			...
			if (M34CryWolf1st::Get_State_Only_Elf() && M34CryWolf1st::IsCyrWolf1st())
			{
				...
				{
					int NumIndex = tc->Object.Type - MODEL_CRYWOLF_ALTAR1;

					if (Hero->GetBaseClass() == CLASS_ELF && !M34CryWolf1st::Get_AltarState_State(NumIndex))
					{
						BYTE State = (m_AltarState[NumIndex] & 0x0f);
				...
				else
				{
					...
					else if (tc->MonsterIndex < 468 || tc->MonsterIndex > 475)
					{
						...
						else if (gMapManager->IsCursedTemple())
						{
							...
							{
								if (tc->MonsterIndex == 380
									|| (((g_pCursedTempleWindow->GetMyTeam() == SEASON3A::eTeam_Allied && tc->MonsterIndex == 383)
										|| (g_pCursedTempleWindow->GetMyTeam() == SEASON3A::eTeam_Illusion && tc->MonsterIndex == 384)) && g_pCursedTempleWindow->CheckInventoryHolyItem(Hero)))
								{
...
uint8_t MakeSkillSerialNumber(uint8_t* pSerialNumber)
{
...
void AttackElf(CHARACTER* c, int Skill, float Distance)
{
	...
	case Skill_Summon_Satyros:
		if (World != WD_10HEAVEN && gMapManager->InChaosCastle() == false)
			...
			if (Skill > Skill_Twisting_Slash_Strengthener)
			{
				...
				LABEL_120:
					...
					if (CheckTile(c, TargetX, TargetY, Distance))
					{
						...
						{
							SetAction(o, PLAYER_ATTACK_RIDE_STRIKE, true);
						}
						else
						{
							SetAction(o, PLAYER_ATTACK_STRIKE, true);
						}
				...
				LABEL_177:
					if (CheckTile(c, TargetX, TargetY, Distance))
					{
						...
						{
							SetAction(o, PLAYER_ATTACK_RIDE_STRIKE, 1);
						}
						...
						{
							SetAction(o, PLAYER_ATTACK_STRIKE, 1);
						}
				...
				LABEL_196:
					if (CheckTile(c, TargetX, TargetY, Distance))
					{
						...
						if (Skill != Skill_Electric_Spike && Skill != Skill_Electric_Spark_Strengthener)
						{
							if (Skill == Skill_Earthshake || Skill == Skill_Dark_Horse_Strengthener || Skill == Skill_Earthshake_Mastery || Skill == Skill_Earthshake_Strengthener)
							{
								SetAction(o, PLAYER_ATTACK_DARKHORSE, 1);
						...
						{
							SetAction(o, PLAYER_ATTACK_RIDE_ATTACK_FLASH, 1);
						}
						c->Movement = 0;
					}
					else
				...
				LABEL_224:
					...
					if (CheckTile(c, TargetX, TargetY, Distance))
					{
						...
						SetAction(o, PLAYER_ATTACK_SKILL_FURY_STRIKE, 1);
						c->Movement = 0;
					}
					else
					{
...
void Attack(CHARACTER* c)
{
		...
		&& !g_isCharacterBuff((&Hero->Object), eBuff_DuelWatch))
	{
		...
		else
		{
			...
			if (g_pOption->IsAutoAttack() && World != WD_6STADIUM && !gMapManager->InChaosCastle() && Attacking == 2 && SelectedCharacter != -1)
				Success = true;
		...
		if (gSkillManager.DemendConditionCheckSkill(Skill) && Success && c->Dead == 0)
		{
			...
			{
				if (g_pOption->IsAutoAttack() && World != WD_6STADIUM && gMapManager->InChaosCastle() == false)
				{
				...
				if (o->Type == MODEL_PLAYER)
				{
					if (o->AnimationFrame >= 5.f && (
						(o->CurrentAction == PLAYER_ATTACK_SKILL_FURY_STRIKE && Skill != Skill_Rageful_Blow && Skill != Skill_Anger_Blow_Strengthener && Skill != Skill_Anger_Blow_Mastery) ||
						(o->CurrentAction == PLAYER_SKILL_BLOW_OF_DESTRUCTION && Skill != Skill_Strike_of_Destruction && Skill != Skill_Strike_of_Destruction_Strengthe && Skill != Skill_Strike_of_Destruction_Proficien && Skill != Skill_Strike_of_Destruction_Mastery)))
					{
					}
					else
					{
...
void MoveHero()
{
	...
	else if (SelectedCharacter == -1 || !g_isCharacterBuff((&Hero->Object), eBuff_CrywolfHeroContracted))
	{
		if (c->Dead <= 0)
		{
			if (!g_isCharacterBuff(o, eDeBuff_Stun) && !g_isCharacterBuff(o, eDeBuff_Sleep) && o->Live)
			{
				//if (checkNormalizer)
				//	--LoadingWorld;
				...
				if (LoadingWorld <= 30)
				{
					DWORD AddFriendWindow = g_pWindowMgr->GetAddFriendWindow();

					if (AddFriendWindow != 0)
					{
						if (MouseRButtonPush)
						{
							if (!IsStrifeMap(World))
							{
								CUITextInputWindow* pWindow = (CUITextInputWindow*)g_pWindowMgr->GetWindow(AddFriendWindow);
								CHARACTER* pCharacter = gmCharacters->GetCharacter(SelectedCharacter);
					...
					else
					{
						...
						Angle = ((int)(vAngle + o->Angle[2]) + 315) % 360;
					...
					if ((g_pOption->IsAutoAttack() && Attacking != -1 && World != WD_6STADIUM && !gMapManager->InChaosCastle())
					...
					{
						if (++StandTime >= ((int)standlimit(40))
							&& !MouseOnWindow
							&& Hero->Dead == 0
							...
							&& o->CurrentAction != PLAYER_SIT_FEMALE1)
						{
							if (NoAutoAttacking)
							{
								...
								{
									StandTime = 0;
									float vAngle = CreateAngle((float)MouseX, (float)MouseY, (float)HeroX, (float)HeroY);
									HeroAngle = (405 - (int)(vAngle)) % 360;
									...
									if (Angle1 != Angle2)
									{
										...
										BYTE angle_fd = (unsigned __int8)(signed int)(((double)HeroAngle + 22.5) / 360.0 * 8.0 + 1.0) % 8;
										SendRequestAction(AT_STAND1, angle_fd);
									}
								}
							}
						}
					...
					if (!MouseOnWindow && !g_pNewUISystem->CheckMouseUse())
					{
						...
						if (MouseUpdateTime >= (int)(MouseUpdateTimeMax) && !EnableFastInput)
						{
							if (MouseLButtonPush)
							{
								MouseLButtonPush = false;
						...
						if (followCharacter)
						{
							if (followCharacter->Object.Live)
							{
								...
								if (PathFinding2((c->PositionX), (c->PositionY), TargetX, TargetY, &c->Path, 0.0f, TW_CHARACTER))
									SendMove(c, o);
							}
							else
							{
							...
							&& (o->CurrentAction != PLAYER_SHOCK
								...
								&& (o->CurrentAction < PLAYER_SKILL_THRUST || o->CurrentAction > PLAYER_RAGE_UNI_ATTACK_ONE_RIGHT)
								...
								|| o->CurrentAction >= PLAYER_RAGE_FENRIR_WALK && o->CurrentAction <= PLAYER_RAGE_FENRIR_STAND_ONE_LEFT))
						{
							...
							if (!pPickedItem && RightType == -1 && (LeftType >= ITEM_SWORD && LeftType < ITEM_SPEAR
								|| LeftType >= ITEM_STAFF && LeftType < ITEM_SHIELD && (CharacterMachine->Equipment[EQUIPMENT_WEAPON_LEFT].Kind2 != ItemKind2::BOOK)))
							{
							...
							{
								CHARACTER* pCharacter = gmCharacters->GetCharacter(SelectedNpc);
									...
									if (SelectedItem == -1)
									{
										if (!SEASON3B::IsRepeat(VK_SHIFT))
										{
											...
											if (Success && c->Object.Live)
											{
												...
												if (CurrAtt >= TW_NOGROUND && !(CurrAtt & TW_ACTION) && !(CurrAtt & TW_HEIGHT))
													DontMove = true;
												else
													DontMove = false;

												int xPos = (int)(o->Position[0] / TERRAIN_SCALE);
												int yPos = (int)(o->Position[1] / TERRAIN_SCALE);

												g_pNewUIMiniMap->StopMove();

												if (!c->Movement || (abs(c->PositionX - xPos) < 2 && abs(c->PositionY - yPos) < 2))
												{
													if (c->PositionX == TargetX
														&& c->PositionY == TargetY
														&& c->Movement
								...
								else
								{
									...
									else if (PathFinding2((c->PositionX), (c->PositionY), TargetX, TargetY, &c->Path))
									{
										SendMove(c, o);
									}
									else
					...
					Attack(Hero);
					int tempIndex = (int)Hero->Object.Position[1] / (int)TERRAIN_SCALE << 8;
					int Index = ((int)Hero->Object.Position[0] / (int)TERRAIN_SCALE) + tempIndex;
					if (Index >= 0)
					{
						if (Index > 0xffff)
							Index = 0xffff;
				...
				if (CollisionDetectLineToOBB(MousePosition, MouseTarget, o->OBB))
				{
					...
					if (fNewDist < fNearestDist)
					{
						...
						if (World == WD_0LORENCIA || World == WD_2DEVIAS)
						{
							...
							{
								if (TerrainMappingLayer1[Index] != HeroTile && (World == WD_2DEVIAS && HeroTile != 11))
									bCanTalk = FALSE;
							}
						}
						if (bCanTalk == TRUE)
		...
		if (g_iGoalEffect == 1)
		{
			if (c->GuildTeam == 1)
			{
				...
				for (int i = 0; i < 36; ++i)
				{
					...
					CreateJointSync(BITMAP_FLARE, Position, Position, Angle, 22, &c->Object, 35);
				}
			}
		}
...
void RenderPartyHP()
{
	...
	for (size_t i = 0; i < MAX_CHARACTERS_CLIENT; i++)
	{
		...
		if (o->Live && (
#if MAIN_UPDATE > 303
			(o->Kind == KIND_PLAYER && o->Type == MODEL_PLAYER) ||
#endif
