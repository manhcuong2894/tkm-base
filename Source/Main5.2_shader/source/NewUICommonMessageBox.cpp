...
bool SEASON3B::CInfinityArrowCancelMsgBoxLayout::SetLayout()
{
	...
	unicode::_sprintf(strText, "%s%s", SkillAttribute[Skill_Infinity_Arrow].Name, GlobalText[2046]);
	g_iCancelSkillTarget = Skill_Infinity_Arrow;
...
CALLBACK_RESULT SEASON3B::CPersonalShopItemValueCheckMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pPickedItem)
	{
		...
		if (InvenIndex != -1)
		{
			...
			SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, MAX_EQUIPMENT + iSourceIndex, pItemObj, REQUEST_EQUIPMENT_MYSHOP, INVENTORY_SHOP_INDEX + iTargetIndex);
		}
		...
		else if (pPickedItem->GetOwnerInventory() == NULL)
		{
			...
			SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, iSourceIndex, pItemObj, REQUEST_EQUIPMENT_MYSHOP, INVENTORY_SHOP_INDEX + iTargetIndex);
		}
		else if (pPickedItem->GetOwnerInventory() == g_pMyShopInventory->GetInventoryCtrl())
		{
			int iItemPrice = pMsgBox->GetItemValue();
			BYTE byIndex = INVENTORY_SHOP_INDEX + iSourceIndex;

			SendRequestSetSalePrice(byIndex, iItemPrice, 0, 0, 0, 0);

			SendRequestEquipmentItem(REQUEST_EQUIPMENT_MYSHOP, INVENTORY_SHOP_INDEX + iSourceIndex, pItemObj, REQUEST_EQUIPMENT_MYSHOP, INVENTORY_SHOP_INDEX + iTargetIndex);
		}

		AddPersonalItemPrice(INVENTORY_SHOP_INDEX + iTargetIndex, 0, pMsgBox->GetItemValue(), 0, 0, 0, g_IsPurchaseShop);
	}
	else
	{
...
CALLBACK_RESULT SEASON3B::CPersonalShopItemBuyMsgBoxLayout::OkBtnDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
	...
	if (pItem && pCha)
	{
		int sourceIndex = INVENTORY_SHOP_INDEX + g_pPurchaseShopInventory->GetSourceIndex();
