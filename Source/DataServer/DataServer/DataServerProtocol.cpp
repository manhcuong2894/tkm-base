...
void GDRankingPlayerInfoRecv(SDHP_RANKING_CHARACTER_INFO_RECV* lpMsg, int index)
{
	...
	if (gQueryManager.ExecQuery("SELECT Class, Inventory FROM Character WHERE Name='%s'", lpMsg->name) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
	{
		gQueryManager.Close();
	}
	else
	{
		...
		for (int i = 0; i < 12; i++)
		{
			...
			if (Inventory[n][0] == 0xff && (Inventory[n][7] & 0x80) == 0x80 && (Inventory[n][9] & 0xf0) == 0xf0)
			{
				pMsg.Inventory[i][0] = 0xff;
				pMsg.Inventory[i][1] = 0xff;
				pMsg.Inventory[i][2] = 0xff;
				pMsg.Inventory[i][3] = 0xff;
				pMsg.Inventory[i][4] = 0xff;
			}
			else
			{
				pMsg.Inventory[i][0] = Inventory[n][0];
				pMsg.Inventory[i][1] = Inventory[n][1];
				pMsg.Inventory[i][2] = Inventory[n][7];
				pMsg.Inventory[i][3] = Inventory[n][8];
				pMsg.Inventory[i][4] = Inventory[n][9];
			}
		}
	}
