...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) // OK
{
	...
	if (WSAStartup(MAKEWORD(2, 2), &wsa) == 0)
	{
		...
		MaxHWIDConnection = GetPrivateProfileInt("ConnectServerInfo", "MaxHWIDConnection", 1, ".\\Data\\ConnectServer.ini");
	...
	/*HMENU hMenu = LoadMenu(hInstance, MAKEINTRESOURCE(IDC_CONNECTSERVER));
	HMENU hSubMenu = GetSubMenu(hMenu, 1); // Suponiendo que el segundo popup es "Reload"
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) // OK
{
	...
	case WM_COMMAND:
		...
		case IDM_RELOAD_RELOADSERVERLIST:
			gServerList.Load(".\\Data\\ServerList.dat");
			break;
		case IDM_RELOAD_SERVER_VERSION:
			LoadServerVersion();
			break;
		default:
