	...
	for (type_map_item::iterator it = m_ItemInfo.begin(); it != m_ItemInfo.end(); ++it)
	{
		...
		if (info.Index != 14)
		{
			...
			case SET_ITEM_OPTION_ADD_LEADERSHIP:
				info.AttType = 5;
				break;
			case SET_ITEM_OPTION_ADD_ALL_STATS:
				info.AttType = 6;
				break;
			default:
				info.AttType = 0;
...
bool CItemManager::CheckItemMoveToTrade(LPOBJ lpObj, CItem* lpItem, BYTE TargetFlag) // OK
{
	...
	if (lpItem->m_IsPeriodicItem != 0
		|| (gServerInfo.m_SetLuckyReleaseLock != 1 && lpItem->IsLuckyItem() != 0))
	{
		return 0;
	}
...
bool CItemManager::CheckItemMoveToChaos(LPOBJ lpObj, CItem* lpItem, BYTE TargetFlag) // OK
{
	...
	if (gServerInfo.m_SetLuckyReleaseLock != 1)
	{
...
int CItemManager::RepairItem(LPOBJ lpObj, CItem* lpItem, int slot, int type) // OK
{
	...
	if (lpItem->m_IsPeriodicItem != 0 || lpItem->IsPentagramItem() != 0)
	{
		return 0;
	}
	...
	{
		GCPetItemInfoSend(lpObj->Index, (lpItem->m_Index == GET_ITEM(13, 4)), 0, slot, lpItem->m_PetItemLevel, lpItem->m_PetItemExp, (BYTE)lpItem->m_Durability);
	}
...
{
	if (lpObj->m_iMixIndex == 0xFF || lpObj->m_iMixState != 0)
	{
		return 0xFF;
	}
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg, int aIndex) // OK
{
	...
	GCLifeSend(aIndex, 0xFD, (uint32_t)lpObj->Life, lpObj->Shield);
	...
	else if (gCustomTicket->ChangeTicket(lpObj, lpItem->m_Index, lpItem->m_Level) != 0)
	{
		this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
		this->GCItemDeleteSend(aIndex, lpMsg->SourceSlot, 1);
	}
...
#ifdef MOD_PET_EXPERIENCE
			|| lpObj->Inventory[lpMsg->slot].Kind3 == ItemKind3::PET_GAIN_EXPERIENCE
#endif // MOD_PET_EXPERIENCE
			))
		{
