...
void CBloodCastle::SetEntranceZone(int aIndex)
{
	GCMapAttrSend(aIndex, 0, 4, 0, 1, (PMSG_MAP_ATTR*)gBloodCastleEntranceZone);
}
...
void CBloodCastle::CGBloodCastleEnterRecv(PMSG_BLOOD_CASTLE_ENTER_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpMsg->level >= 0 && lpMsg->level <= 7)
	{
		this->SetEntranceZone(aIndex);
	}
}
