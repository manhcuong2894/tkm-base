...
#include "Viewport.h"
#include "Warehouse.h"
#include "CGMMixExpansion.h"
#include "GMHolyItem.h"
#include "ChatManager.h"
#include "ShopManager.h"


void ProtocolCore(uint8_t head, uint8_t* lpMsg, int size, int aIndex, int encrypt, int serial) // OK
{
	PROTECT_START;
	...
	case 0x4F:
		...
		case 0x03:
			gGoblinMix->ReceiveMixInfo(aIndex, lpMsg);
			break;
		case 0x06:
			gGoblinMix->ReceiveMixState(aIndex, lpMsg);
			break;
...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpMsg->message[0] == '~')
	{
		if (OBJECT_RANGE(lpObj->PartyNumber) != 0)
		{
			for (int n = 0; n < MAX_PARTY_USER; n++)
			{
				...
				{
					DataSend(gParty.m_PartyInfo[lpObj->PartyNumber].Index[n], (uint8_t*)&pMsg, pMsg.header.size);
				}
			}
		}
	}
	...
	else if (lpMsg->message[0] == '$')
	{
		if (lpObj->GensFamily != 0)
		{
			for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
			{
				...
				{
					DataSend(n, (uint8_t*)&pMsg, pMsg.header.size);
				}
			}
		}
	}
	...
	{
		DataSend(aIndex, (uint8_t*)&pMsg, pMsg.header.size);
		MsgSendV2(lpObj, (uint8_t*)&pMsg, pMsg.header.size);
	}
}
...
void CGPositionRecv(PMSG_POSITION_RECV* lpMsg, int aIndex) // OK
{
	...
	pMsg.x = (uint8_t)lpObj->TX;

	pMsg.y = (uint8_t)lpObj->TY;
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		if (lpObj->VpPlayer2[n].type == OBJECT_USER)
		{
			...
			{
				DataSend(lpObj->VpPlayer2[n].index, (uint8_t*)&pMsg, pMsg.header.size);
			}
		}
	}
}
...
void CGPetItemInfoRecv(PMSG_PET_ITEM_INFO_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpMsg->flag == 0) // Inventory
	{
		...
		GCPetItemInfoSend(aIndex, lpMsg->type, lpMsg->flag, lpMsg->slot, lpObj->Inventory[lpMsg->slot].m_PetItemLevel, lpObj->Inventory[lpMsg->slot].m_PetItemExp, (uint8_t)lpObj->Inventory[lpMsg->slot].m_Durability);
	}
	else if (lpMsg->flag == 1) // Warehouse
	{
		...
		GCPetItemInfoSend(aIndex, lpMsg->type, lpMsg->flag, lpMsg->slot, lpObj->Warehouse[lpMsg->slot].m_PetItemLevel, lpObj->Warehouse[lpMsg->slot].m_PetItemExp, (uint8_t)lpObj->Warehouse[lpMsg->slot].m_Durability);
	}
	else if (lpMsg->flag == 2) // Trade
	{
		...
		GCPetItemInfoSend(aIndex, lpMsg->type, lpMsg->flag, lpMsg->slot, lpObj->Trade[lpMsg->slot].m_PetItemLevel, lpObj->Trade[lpMsg->slot].m_PetItemExp, (uint8_t)lpObj->Trade[lpMsg->slot].m_Durability);
	}
	else if (lpMsg->flag == 3) // Target Trade
	{
		...
		GCPetItemInfoSend(aIndex, lpMsg->type, lpMsg->flag, lpMsg->slot, lpTarget->Trade[lpMsg->slot].m_PetItemLevel, lpTarget->Trade[lpMsg->slot].m_PetItemExp, (uint8_t)lpTarget->Trade[lpMsg->slot].m_Durability);
	}
	else if (lpMsg->flag == 4) // Chaos Box
	{
		...
		GCPetItemInfoSend(aIndex, lpMsg->type, lpMsg->flag, lpMsg->slot, lpObj->ChaosBox[lpMsg->slot].m_PetItemLevel, lpObj->ChaosBox[lpMsg->slot].m_PetItemExp, (uint8_t)lpObj->ChaosBox[lpMsg->slot].m_Durability);
	}
	else if (lpMsg->flag == 5) // Target Personal Shop
	{
		...
		GCPetItemInfoSend(aIndex, lpMsg->type, lpMsg->flag, lpMsg->slot, lpTarget->Inventory[lpMsg->slot].m_PetItemLevel, lpTarget->Inventory[lpMsg->slot].m_PetItemExp, (uint8_t)lpTarget->Inventory[lpMsg->slot].m_Durability);
	}
	else if (lpMsg->flag == 6) // shop normal
	{
		...
		if (lpObj->TargetShopNumber != -1)
		{
			...
			if (ShopInventory)
			{
				GCPetItemInfoSend(aIndex, lpMsg->type, lpMsg->flag, lpMsg->slot, ShopInventory[lpMsg->slot].m_PetItemLevel, ShopInventory[lpMsg->slot].m_PetItemExp, (uint8_t)ShopInventory[lpMsg->slot].m_Durability);
			}
		}
...
void CGLevelUpPointRecv(PMSG_LEVEL_UP_POINT_RECV* lpMsg, int aIndex) // OK
{
	...
	if (gObjectManager.CharacterLevelUpPointAdd(lpObj, lpMsg->type, 1))
	{
		pMsg.result = 16 + lpMsg->type;
	}

	pMsg.LevelPoint = (uint32_t)lpObj->LevelUpPoint;
	pMsg.LifeMax = (uint32_t)((lpObj->MaxLife + lpObj->AddLife));
	pMsg.ManaMax = (uint32_t)((lpObj->MaxMana + lpObj->AddMana));
	pMsg.ShieldMax = (uint32_t)((lpObj->MaxShield + lpObj->AddShield));
	pMsg.SkillManaMax = (uint32_t)((lpObj->MaxBP + lpObj->AddBP));
	pMsg.Strength = (uint32_t)lpObj->Strength;
	pMsg.Dexterity = (uint32_t)lpObj->Dexterity;
	pMsg.Vitality = (uint32_t)lpObj->Vitality;
	pMsg.Energy = (uint32_t)lpObj->Energy;
	pMsg.Charisma = (uint32_t)lpObj->Leadership;
...
void GCChatTargetSend(LPOBJ lpObj, int aIndex, char* message) // OK
{
	...
	size = ((size > MAX_CHAT_SIZE-1) ? MAX_CHAT_SIZE-1 : size);
	...
	DataSend(lpObj->Index, (uint8_t*)&pMsg, pMsg.header.size);
}
...
void GCEventStateSend(int aIndex, uint8_t state, uint8_t _event) // OK
{
...
void GCEventStateSendToAll(int map, uint8_t state, uint8_t _event) // OK
{
	for (int n = OBJECT_START_USER; n < MAX_OBJECT; n++)
	{
...
void GCServerMsgSend(int aIndex, uint8_t msg) // OK
{
...
void GCWeatherSend(int aIndex, uint8_t weather) // OK
{
...
void GCDamageSend(int aIndex, int bIndex, uint8_t flag, int damage, int type, int ShieldDamage) // OK
{
	...
	{
		DataSend(bIndex, (uint8_t*)&pMsg, pMsg.header.size);
	}
}
...
void GCActionSend(LPOBJ lpObj, uint8_t action, int aIndex, int bIndex) // OK
{
	PMSG_ACTION_SEND pMsg;
...
void GCLifeSend(int aIndex, uint8_t type, uint32_t life, uint32_t shield) // OK
{
	PMSG_LIFE_SEND pMsg;
...
void GCManaSend(int aIndex, uint8_t type, uint32_t mana, uint32_t bp) // OK
{
	PMSG_MANA_SEND pMsg;
...
void GCItemUseSpecialTimeSend(int aIndex, uint8_t number, int time) // OK
{
...
void GCMapAttrSend(int aIndex, uint8_t type, uint8_t attr, uint8_t flag, uint8_t count, PMSG_MAP_ATTR* lpInfo) // OK
{
	uint8_t send[256];
...
void GCPartyItemInfoSend(int aIndex, CItem* lpItem) // OK
{
	...
	pMsg.level = (uint8_t)lpItem->m_Level;
	...
	for (int n = 0; n < MAX_PARTY_USER; n++)
	{
		...
		{
			DataSend(index, (uint8_t*)&pMsg, pMsg.header.size);
		}
	}
}

void GCEffectInfoSend(int aIndex, uint8_t effect) // OK
{
	...
	MsgSendV2(&gObj[aIndex], (uint8_t*)&pMsg, pMsg.header.size);
}
...
void GCGuildMasterQuestionSend(int aIndex) // OK
{
	...
	DataSend(aIndex, (uint8_t*)&pMsg, pMsg.size);
}
...
void GCMonsterDieSend(int aIndex, int bIndex, int experience, int damage, uint8_t flag) // OK
{
...
void GCEventEnterCountSend(int aIndex, uint8_t EventType, uint8_t EnterCount) // OK
{
...
void GCPetItemInfoSend(int aIndex, uint8_t type, uint8_t flag, uint8_t slot, uint8_t level, DWORD experience, uint8_t durability) // OK
{
...
void GCMapServerMoveAuthSend(int aIndex, uint8_t result) // OK
{
...
void GCTaxInfoSend(int aIndex, uint8_t type, uint8_t rate) // OK
{
...
void GCCreationStateSend(int aIndex, uint8_t state) // OK
{
...
void GCElementalDamageSend(int aIndex, int bIndex, uint8_t attribute, int damage) // OK
{
#if(GAMESERVER_UPDATE>=701)
...
void GCCharacterCreationEnableSend(int aIndex, uint8_t flag, uint8_t result) // OK
{
...
void GCLifeUpdateSend(LPOBJ pPlayer) // OK
{
	...
	MsgSendV2(pPlayer, (uint8_t*)&player_body, player_body.header.size);
#endif
}
...
void GCConnectClientSend(int aIndex, uint8_t result) // OK
{
...
void GCConnectAccountSend(int aIndex, uint8_t result) // OK
{
...
void GCConnectAccountSend(int aIndex, uint8_t result, SOCKET socket) // OK
{
	PMSG_CONNECT_ACCOUNT_SEND pMsg;
	...
	DataSendSocket(socket, (uint8_t*)&pMsg, pMsg.header.size);
}

void GCCloseClientSend(int aIndex, uint8_t result) // OK
{
...
void GCCharacterRegenSend(LPOBJ lpObj) // OK
{
	...
	pMsg.X = (uint8_t)lpObj->X;

	pMsg.Y = (uint8_t)lpObj->Y;
	...
	{
		SerializeUInt64ToBytes(lpObj->Experience, pMsg.Experience);
	}
	else
	{
		SerializeUInt64ToBytes(lpObj->MasterExperience, pMsg.Experience);
	}
...
void GCLevelUpSend(LPOBJ lpObj) // OK
{
	...
	pMsg.BaseExperience = lpObj->BaseExperience;
...
void GCServerCommandSend(int aIndex, uint8_t type, uint8_t cmd1, uint8_t cmd2) // OK
{
...
void GCNewCharacterInfoSend(LPOBJ lpObj) // OK
{
	...
	pMsg.Level = (uint16_t)lpObj->Level;
	pMsg.Reset = (uint32_t)lpObj->Reset;
	pMsg.LevelUpPoint = (uint32_t)lpObj->LevelUpPoint;
	...
	pMsg.Life = (uint32_t)(lpObj->Life);
	pMsg.LifeMax = (uint32_t)(lpObj->MaxLife + lpObj->AddLife);
	pMsg.Mana = (uint32_t)(lpObj->Mana);
	pMsg.ManaMax = (uint32_t)(lpObj->MaxMana + lpObj->AddMana);
	pMsg.SkillMana = (uint32_t)(lpObj->BP);
	pMsg.SkillManaMax = (uint32_t)(lpObj->MaxBP + lpObj->AddBP);
	pMsg.Shield = (uint32_t)(lpObj->Shield);
	pMsg.ShieldMax = (uint32_t)(lpObj->MaxShield + lpObj->AddShield);
	...
	{
		SerializeUInt64ToBytes(lpObj->Experience, pMsg.Experience);
		SerializeUInt64ToBytes(lpObj->BaseExperience, pMsg.BaseExperience);
		SerializeUInt64ToBytes(lpObj->NextExperience, pMsg.NextExperience);
	}
	else
	{
		SerializeUInt64ToBytes(lpObj->MasterExperience, pMsg.Experience);
		SerializeUInt64ToBytes(lpObj->MasterBaseExperience, pMsg.BaseExperience);
		SerializeUInt64ToBytes(lpObj->MasterNextExperience, pMsg.NextExperience);
	}
...
void GCNewHealthBarSend(LPOBJ lpObj) // OK
{
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		info.type = (uint8_t)lpTarget->Type;

		info.rate = (uint8_t)((lpTarget->Life * 100) / (lpTarget->MaxLife + lpTarget->AddLife));

		if (lpObj->VpPlayer[n].type == OBJECT_USER)
		{
			info.rate2 = (uint8_t)((lpTarget->Shield * 100) / (lpTarget->MaxShield + lpTarget->AddShield));
		}
		else
		{
			info.rate2 = 0;
...
void GCAnsCastleSiegeState(int iIndex, int iResult, LPSTR lpszGuildName, LPSTR lpszGuildMaster)
{
	...
	pMsgResult.btStartMonth = (uint8_t)tmStateStartDate.wMonth;
	pMsgResult.btStartDay = (uint8_t)tmStateStartDate.wDay;
	pMsgResult.btStartHour = (uint8_t)tmStateStartDate.wHour;
	pMsgResult.btStartMinute = (uint8_t)tmStateStartDate.wMinute;

	pMsgResult.btEndYearH = SET_NUMBERHB(tmStateEndDate.wYear);
	pMsgResult.btEndYearL = SET_NUMBERLB(tmStateEndDate.wYear);
	pMsgResult.btEndMonth = (uint8_t)tmStateEndDate.wMonth;
	pMsgResult.btEndDay = (uint8_t)tmStateEndDate.wDay;
	pMsgResult.btEndHour = (uint8_t)tmStateEndDate.wHour;
	pMsgResult.btEndMinute = (uint8_t)tmStateEndDate.wMinute;
	...
	pMsgResult.btSiegeStartMonth = (uint8_t)tmSiegeStartDate.wMonth;
	pMsgResult.btSiegeStartDay = (uint8_t)tmSiegeStartDate.wDay;
	pMsgResult.btSiegeStartHour = (uint8_t)tmSiegeStartDate.wHour;
	pMsgResult.btSiegeStartMinute = (uint8_t)tmSiegeStartDate.wMinute;
...
void GCAnsRegGuildMark(int iIndex, int iResult, CSP_ANS_GUILDREGMARK* lpMsgResult)
{
	...
	else
	{
		...
		else
		{
			...
			{
				gItemManager.GCItemDurSend(iIndex, lpMsgResult->iItemPos, (uint8_t)gObj[iIndex].Inventory[lpMsgResult->iItemPos].m_Durability, 0);
			}
			else
			{
...
void GCAnsCsNotifyProgress(int iIndex, uint8_t btCastleSiegeState, LPCSTR lpszGuildName)
{
	if (lpszGuildName == NULL)
	{
		return;
...
void GCAnsCsSendCommand(int iCsJoinSize, uint8_t btTeam, uint8_t btX, uint8_t btY, uint8_t btCommand)
{
#if(GAMESERVER_TYPE==1)
