...
{
#if(GAMESERVER_UPDATE>=401)
	if (gServerInfo.m_MasterSkillTree != 0 && (lpObj->Level >= MAX_CHARACTER_LEVEL && lpObj->ChangeUp >= 2))
		return 1;
	else
		return 0;
#else
...
void CMasterSkillTree::CalcMasterLevelNextExperience(LPOBJ lpObj) // OK
{
	...
	lpObj->MasterBaseExperience = this->m_MasterLevelExperienceTable[lpObj->MasterLevel];
