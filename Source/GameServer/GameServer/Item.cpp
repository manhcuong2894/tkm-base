...
void CItem::Convert(int index, BYTE Option1, BYTE Option2, BYTE Option3, BYTE NewOption, BYTE SetOption, BYTE JewelOfHarmonyOption, BYTE ItemOptionEx, BYTE SocketOption[MAX_SOCKET_OPTION], BYTE SocketOptionBonus) // OK
{
	...
	CUSTOM_WING_INFO* infoWing = gCustomWing.findWings007(this->m_Index);
	...
	else
	{
		...
		this->m_SetAddStat = (((this->m_SetOption >> 2) & 3) * 5);
	}
	...
	else if (item_info.Kind2 == ItemKind2::PENTAGRAM)
	{
		...
		|| (this->Kind3 == ItemKind3::PET_GAIN_EXPERIENCE)
#endif // MOD_PET_EXPERIENCE
		)
	{
		this->m_RequireLevel = item_info.Level + (2 * this->m_PetItemLevel);
	}
	...
	if (this->m_Defense > 0)
	{
		...
		else
		{
			...
			{
				this->m_Defense += (this->m_PetItemLevel * 2) + 5;
			}
			else if ((this->m_Index >= GET_ITEM(12, 3) && this->m_Index <= GET_ITEM(12, 6)) || this->m_Index == GET_ITEM(12, 42)) // 2sd Wings,Dark Horse
			{
			...
			else if (infoWing != NULL)
			{
				this->m_Defense += infoWing->GetCustomWingDefense(this->m_Level);
			}
			else
			{
				this->m_Defense += (this->m_Level * 3);
	...
	if (infoWing != NULL)
	{
		...
		{
			this->m_Option3 = Option3;
			this->m_SpecialIndex[SPECIAL_OPTION3] = infoWing->GetWingOptionIndex(0);
			this->m_SpecialValue[SPECIAL_OPTION3] = infoWing->GetWingOptionValue(0);

			if ((this->m_NewOption & 16) != 0)
			{
				this->m_SpecialIndex[SPECIAL_OPTION3] = infoWing->GetWingOptionIndex(1);
				this->m_SpecialValue[SPECIAL_OPTION3] = infoWing->GetWingOptionValue(1);
			}

			if ((this->m_NewOption & 32) != 0)
			{
				this->m_SpecialIndex[SPECIAL_OPTION3] = infoWing->GetWingOptionIndex(2);
				this->m_SpecialValue[SPECIAL_OPTION3] = infoWing->GetWingOptionValue(2);
			}
		}
		...
		for (int i = 0; i < 6; i++)
		{
			...
			{
				this->m_SpecialIndex[SPECIAL_EXCELLENT1 + i] = infoWing->GetWingNewOptionIndex(i);
				this->m_SpecialValue[SPECIAL_EXCELLENT1 + i] = infoWing->GetWingNewOptionValue(i);
			}
			constante *= 2;
		}
	}
