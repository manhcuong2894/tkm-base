...
#include "GameMain.h"
#include "Guild.h"
#include "CGMBattleIce3.h"

CGMBattleIce3 gReiDoMU;
//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

CGMBattleIce3::CGMBattleIce3() // OK
{
...
void CGMBattleIce3::Load(char* path) // OK
{
...
void CGMBattleIce3::MainProc() // OK
{
	...
	if (this->m_RemainTimeWinner > 0)
	{
		if (this->WinnerNumber >= 0)
		{
			...
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(718));
			}
			...
			if (this->m_RemainTimeWinner == 28)
			{
				...
				else
				{
					...
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(719), lpGuildInfo->Name);
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(720), this->WinnerTime);
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(721));
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(722));
			...
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(723));
			}
			if (this->m_RemainTimeWinner == 16)
			{
				if (this->rank1 >= 0)
				{
					...
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(724), 1, lpObj->Name, lpObj->GuildName);
					//Reward
			...
			if (this->m_RemainTimeWinner == 14)
			{
				if (this->rank2 >= 0)
				{
					...
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(724), 2, lpObj->Name, lpObj->GuildName);
					//Reward
			...
			if (this->m_RemainTimeWinner == 12)
			{
				if (this->rank3 >= 0)
				{
					...
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(724), 3, lpObj->Name, lpObj->GuildName);
					//Reward
			...
			if (this->m_RemainTimeWinner == 10)
			{
				if (this->rank4 >= 0)
				{
					...
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(724), 4, lpObj->Name, lpObj->GuildName);
					//Reward
			...
			if (this->m_RemainTimeWinner == 8)
			{
				if (this->rank5 >= 0)
				{
					...
					gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(724), 5, lpObj->Name, lpObj->GuildName);
					//Reward
			...
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(725));
			}
			...
			if (this->m_RemainTimeWinner <= 0)
			{
				gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(501));
		...
		else
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(726));
...
void CGMBattleIce3::ProcState_BLANK() // OK
{
}

void CGMBattleIce3::ProcState_EMPTY() // OK
{
	...
	if (this->m_RemainTime > 0 && this->m_RemainTime <= (this->m_WarningTime * 60))
	{
		...
		if (this->MinutesLeft != minutes)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(502), (MinutesLeft + 1));
		}
	}

	if (this->m_RemainTime > 0 && this->m_RemainTime <= 5)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(504), m_RemainTime);
	}

	if (this->m_RemainTime <= 0)
	{
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(500));

		this->SetState(REI_STATE_START);
	}
}

void CGMBattleIce3::ProcState_START() // OK
{
	if (this->m_RemainTime > 0 && this->m_RemainTime <= 300)
	{
		...
		if (this->MinutesLeft != minutes)
		{
			...
			gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(511), (MinutesLeft + 1));
		}
	}
...
void CGMBattleIce3::SetState(int state) // OK
{
	this->m_State = state;
...
void CGMBattleIce3::SetState_BLANK() // OK
{

}

void CGMBattleIce3::SetState_EMPTY() // OK
{
	this->CheckSync();
}

void CGMBattleIce3::SetState_START() // OK
{
	this->m_Active = 1;
...
void CGMBattleIce3::CheckSync() // OK
{
...
void CGMBattleIce3::CommandReiDoMU(LPOBJ lpObj, char* arg) // OK
{
	...
	if (lpGuildInfo == NULL)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(506));
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(507));
		return;
	}

	if (lpGuildInfo->TotalCount < gServerInfo.m_ReiDoMUGuildMinMembers) //Verifica quantidade de membros na guild
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(509), gServerInfo.m_ReiDoMUGuildMinMembers);
		return;
	}
	...
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(508));
		return;
	}
	...
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GlobalText(503), lpObj->GuildName);

	memcpy(this->m_GuildOwner, lpObj->GuildName, sizeof(this->m_GuildOwner));

	gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GlobalText(510));

	LogAdd(LOG_EVENT, "[KING OF MU] Conquered by %s", lpObj->Name);
}

void CGMBattleIce3::SetGuild(int gIndex) // OK
{
...
void CGMBattleIce3::AddGuild(int gIndex) // OK
{
	for (int n = 0; n < MAX_REI_GUILD; n++)
	{
...
bool CGMBattleIce3::GetGuild(int gIndex) // OK
{
	for (int n = 0; n < MAX_REI_GUILD; n++)
	{
...
void CGMBattleIce3::SetGuildTop() // OK
{
	for (int n = 0; n < MAX_REI_GUILD; n++)
	{
...
void CGMBattleIce3::SetChar(int cIndex) // OK
{
...
void CGMBattleIce3::AddChar(int cIndex) // OK
{
	for (int n = 0; n < MAX_REI_CHAR; n++)
	{
...
bool CGMBattleIce3::GetChar(int cIndex) // OK
{
	for (int n = 0; n < MAX_REI_CHAR; n++)
	{
...
void CGMBattleIce3::CheckChar() // OK
{
	for (int n = 0; n < MAX_REI_CHAR; n++)
	{
...
void CGMBattleIce3::CalcUserRank() // OK
{
	for (int n = 0; n < MAX_REI_CHAR; n++)
	{
		if (this->Char[n].Index < 0)
...
void CGMBattleIce3::GDRankingKingGuildSaveSend(char* name, DWORD Score) // OK
{
...
void CGMBattleIce3::GDRankingKingPlayerSaveSend(int aIndex, DWORD Score) // OK
{
