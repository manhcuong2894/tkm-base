...
void CChaosBox::Wing2Mix(LPOBJ lpObj, int type) // OK
{
	...
	if (this->GetTalismanOfLuckRate(lpObj, &lpObj->ChaosSuccessRate) == 0)
	{
		//pLog->LogMessage(MCD_SEND, "[Wing2Mix: %d][ERROR][GCChaosMixSend: %d]", type, 240);
		this->GCChaosMixSend(lpObj->Index, 240, 0);
		return;
	}
...
void CChaosBox::SocketItemMountSeedSphereMix(LPOBJ lpObj, WORD slotIndex) // OK
{
	if (slotIndex >= MAX_SOCKET_ITEM_OPTION_TABLE)
	{
		this->GCChaosMixSend(lpObj->Index, 0, 0);
		return;
	}
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		else if (gSocketItemType.CheckSocketItemType(lpObj->ChaosBox[n].m_Index) != 0 && lpObj->ChaosBox[n].m_SocketOption[slotIndex] == SOCKET_ITEM_OPTION_EMPTY)
		{
			ItemCount++;
			ItemSlot = n;
		}
	...
	//limite socket
	if (slotIndex >= gSocketItemType.GetSocketItemMaxSocket(lpObj->ChaosBox[ItemSlot].m_Index))
	{
		this->GCChaosMixSend(lpObj->Index, 0, 0);
		return;
	}
	...
	item.m_SocketOption[slotIndex] = SocketOption;
	...
	gLog.Output(LOG_CHAOS_MIX, "[SocketItemMountSeedSphereMix][Success][%s][%s] - [Socket: %d]- (ChaosSuccessRate: %d, ChaosMoney: %d)", lpObj->Account, lpObj->Name, slotIndex, lpObj->ChaosSuccessRate, lpObj->ChaosMoney);
}

void CChaosBox::SocketItemUnMountSeedSphereMix(LPOBJ lpObj, WORD slotIndex) // OK
{
	if (slotIndex >= MAX_SOCKET_ITEM_OPTION_TABLE)
	{
		this->GCChaosMixSend(lpObj->Index, 0, 0);
	...
	for (int n = 0; n < CHAOS_BOX_SIZE; n++)
	{
		...
		if (lpObj->ChaosBox[n].m_SocketOption[slotIndex] != SOCKET_ITEM_OPTION_NONE && lpObj->ChaosBox[n].m_SocketOption[slotIndex] != SOCKET_ITEM_OPTION_EMPTY)
		{
			ItemCount++;
			ItemSlot = n;
		}
	...
	item.m_SocketOption[slotIndex] = SOCKET_ITEM_OPTION_EMPTY;
